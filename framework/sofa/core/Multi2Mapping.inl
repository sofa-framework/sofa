...
template < class In1, class In2, class Out >
void Multi2Mapping<In1,In2,Out>::getConstVecIn1Coord(const VecId &id, helper::vector<const In1VecCoord*> &v) const
{
    for (unsigned int i=0; i<fromModels1.size(); ++i) v.push_back(fromModels1[i]->getVecCoord(id.index));
}
...
template < class In1, class In2, class Out >
void Multi2Mapping<In1,In2,Out>::getConstVecIn1Deriv(const VecId &id, helper::vector<const In1VecDeriv*> &v) const
{
    for (unsigned int i=0; i<fromModels1.size(); ++i) v.push_back(fromModels1[i]->getVecDeriv(id.index));
}
...
template < class In1, class In2, class Out >
void Multi2Mapping<In1,In2,Out>::getConstVecIn2Coord(const VecId &id, helper::vector<const In2VecCoord*> &v) const
{
    for (unsigned int i=0; i<fromModels2.size(); ++i) v.push_back(fromModels2[i]->getVecCoord(id.index));
}
...
template < class In1, class In2, class Out >
void Multi2Mapping<In1,In2,Out>::getVecOutCoord(const VecId &id, helper::vector<OutVecCoord*> &v) const
{
