...
namespace sofa
{
	...
	namespace component
	{
		...
		SofaHAPIHapticsDevice::SofaHAPIHapticsDevice()
			...
			, forceScale(initData(1.0, "forceScale","Default forceScale applied to the force feedback. "))
			...
			, positionTool(initData(Vec3d(0,0,0), "positionTool","Position of the tool in the device end effector frame"))
			, orientationTool(initData(Quat(0,0,0,1), "orientationTool","Orientation of the tool in the device end effector frame"))
			, permanent(initData(false, "permanent" , "Apply the force feedback permanently"))
			, toolSelector(initData(false, "toolSelector", "Switch tools with 2nd button"))
			, toolCount(initData(1, "toolCount", "Number of tools to switch between"))
			, toolIndex(initData(0, "toolIndex", "Current tool index"))
			, toolTransitionSpringStiffness(initData(0.0, "toolTransitionSpringStiffness", "Stiffness of haptic springs when switching instruments (0 to disable)"))
			, driverName(initData(std::string("Any"), "driverName", "Name of the HAPI device driver"))
			, drawDevice(initData(false, "drawDevice", "Visualize the position of the interface in the virtual scene"))
			, drawHandleSize(initData(0.0f, "drawHandleSize", "Visualize the handle direction of the interface in the virtual scene"))
			, drawForceScale(initData(0.0f, "drawForceScale", "Visualize the haptics force in the virtual scene"))
		...
		void SofaHAPIHapticsDevice::draw(const sofa::core::visual::VisualParams* vparams)
		{
			...
			if(drawDevice.getValue())
			{
				...
				if (drawHandleSize.getValue() == 0.0f)
				{
					...
					vparams->drawTool()->drawSpheres(points, 1.0f, sofa::type::Vec<4,float>(0,0,1,1));
				}
				else
				{
					...
					vparams->drawTool()->drawArrow(wposH, wpos, drawHandleSize.getValue()*0.05f, sofa::type::Vec<4,float>(0,0,1,1));
				}
				if (force.norm() > 0 && drawForceScale.getValue() != 0.0f)
				{
					...
					vparams->drawTool()->drawArrow(wpos, wposF, 0.1f, sofa::type::Vec<4,float>(1,0,0,1));
				}
