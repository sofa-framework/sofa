...
*/
template< size_type _spatial_dimensions, typename _Real>
...
public:
    ...
    static const size_type num_cross_terms =  spatial_dimensions==1? 0 : ( spatial_dimensions==2? 1 : spatial_dimensions==3? 3 : 0);
    static const size_type num_quadratic_terms =  2*spatial_dimensions + num_cross_terms;
    static const size_type VSize = spatial_dimensions +  spatial_dimensions * num_quadratic_terms;  // number of entries
    ...
    public:
        ...
        {
            getCenter() = center;
            for(size_type i=0; i<spatial_dimensions; ++i)
            {
                for(size_type j=0; j<spatial_dimensions; ++j)
                {
            ...
            for(size_type i=0; i<spatial_dimensions; ++i)
            {
                for(size_type j=0; j<num_cross_terms; ++j)
                {
                    Frame& quadratic=getQuadratic();
...
//            return QuadraticInv * (v-getCenter());
        ...
        template< size_type N, class Real2 > // N <= VSize
        void operator+=( const Vec<N,Real2>& p ) { for(size_type i=0;i<N;++i) this->elems[i] += (Real)p[i]; }
        template< size_type N, class Real2 > // N <= VSize
        void operator=( const Vec<N,Real2>& p ) { for(size_type i=0;i<N;++i) this->elems[i] = (Real)p[i]; }
    ...
    static Coord interpolate ( const helper::vector< Coord > & ancestors, const helper::vector< Real > & coefs )
    {
        ...
        Coord c;
        for (size_type i = 0; i < ancestors.size(); i++ ) c += ancestors[i] * coefs[i];  // Position and deformation gradient linear interpolation.
        return c;
    }
...
{
    Mat<5,2,Real> M;
    for(size_type i=0;i<2;i++) { M(i,i)=1;  M(i+2,i)=2*p[i];}
    M(4,0)=p[1];     M(4,1)=p[0];
    return M;
}
...
{
    Mat<9,3,Real> M;
    for(size_type i=0;i<3;i++) { M(i,i)=1;  M(i+3,i)=2*p[i];}
    M(6,0)=p[1]; M(6,1)=p[0];
    M(7,1)=p[2]; M(7,2)=p[1];
    M(8,0)=p[2]; M(8,2)=p[0];
    return M;
