...
template< size_type _spatial_dimensions, size_type _strain_size, int _order, typename _Real, char _name >
class BaseStrainTypes
{
public:
    typedef PolynomialBasis<_strain_size,_Real, _spatial_dimensions, _order> Basis;

    static const size_type spatial_dimensions = _spatial_dimensions;   ///< Number of dimensions the frame is moving in, typically 3
    static const size_type strain_size = _strain_size;
    typedef _Real Real;
    ...
    public:
        ...
        static const size_type total_size = VSize;
        typedef Real value_type;

        static size_type size() { return VSize; }
        ...
        Real& operator[](size_type i) { return getVec()[i]; }
        const Real& operator[](size_type i) const    { return getVec()[i]; }
        ...
        StrainVec& getStrainGradient(size_type i) { return v.getGradient()[i]; }
        const StrainVec& getStrainGradient(size_type i) const { return v.getGradient()[i]; }

        StrainVec& getStrainHessian(size_type i,size_type j) { return v.getHessian()(i,j); }
        const StrainVec& getStrainHessian(size_type i,size_type j) const { return v.getHessian()(i,j); }

        StrainVec& getStrainHessian(size_type i) { return v.getHessian().elems[i]; }
        const StrainVec& getStrainHessian(size_type i) const { return v.getHessian().elems[i]; }
    ...
    static Deriv interpolate ( const helper::vector< Deriv > & ancestors, const helper::vector< Real > & coefs )
    {
        ...
        Deriv c;
        for ( size_type i = 0; i < ancestors.size(); i++ )     c += ancestors[i] * coefs[i];
        return c;
    }
...
template<size_type _spatial_dimensions, size_type _material_dimensions, int _order, typename _Real, char _name='E' >
...
public:
    ...
    static const size_type material_dimensions = _material_dimensions; ///< Number of dimensions of the material space (=number of axes of the deformable gradient): 3 for a volume object, 2 for a surface, 1 for a line.
...
template<size_type _spatial_dimensions, size_type _material_dimensions, int _order, typename _Real, char _name='I' >
...
template<size_type _spatial_dimensions, size_type _material_dimensions, int _order, typename _Real, char _name='U' >
...
template<size_type material_dimensions, size_type dim, typename Real>
...
{
    static const size_type strain_size = material_dimensions * (1+material_dimensions) / 2; ///< independent entries in the strain tensor
    typedef Vec<strain_size,Real> StrainVec;    ///< Strain in vector form
    StrainVec s(NOINIT);
    unsigned int ei=0;
    for(size_type j=0; j<material_dimensions; j++)
        for( size_type k=0; k<material_dimensions-j; k++ )
...
template<size_type material_dimensions, typename Real>
static inline Vec<material_dimensions, Real> MatToPrincipalStretches( const  Mat<material_dimensions,material_dimensions, Real>& f )
{
    ...
    for(size_type i=0; i<material_dimensions; i++)
        s[i] = f[i][i];
    return s;
}
...
{
    static const size_type material_dimensions=3;
...
{
    static const size_type material_dimensions=2;
    Mat<material_dimensions,material_dimensions, Real> f(NOINIT);
...
{
    static const size_type material_dimensions=1;
    Mat<material_dimensions,material_dimensions, Real> f(NOINIT);
    f[0][0] = s[0];
    return f;
}
...
static inline Mat<material_dimensions,material_dimensions, Real> PrincipalStretchesToMat( const Vec<material_dimensions, Real>& s )
{
    ...
    {
        f[i][i] = s[i];
        for( size_type j=i+1 ; j<material_dimensions ; ++j )
        {
            f[i][j] = f[j][i] = 0;
        }
    }
...
/// \return 0.5 * ( A + At )
template<size_type N, class Real>
