...
//        vparams->drawTool()->drawSpheres(points, (float)d_drawSize.getValue(), Vec<4,float>(0.2f,0.1f,0.9f,1.0f));
    {
        if( d_fixAll.getValue()==true )
            for (unsigned i=0; i<x.size(); i++ )
            {
                ...
                vparams->drawTool()->drawFrame ( Vector3(), Quat<SReal>(), Vector3 ( 1,1,1 ), Vec4f(0,0,1,1) );
...
void MechanicalObject<defaulttype::TYPEABSTRACTNAME3dTypes>::draw(const core::visual::VisualParams* vparams)
{
    ...
    if (showObject.getValue())
    {
        ...
        for (size_t i = 0; i < this->getSize(); ++i)
        {
            ...
            {
                case 1:
                    vparams->drawTool()->drawFrame ( Vector3(), Quat<SReal>(), Vector3 ( 1,1,1 ), Vec4f(0,1,0,1) );
                    break;
                case 2:
                    vparams->drawTool()->drawFrame ( Vector3(), Quat<SReal>(), Vector3 ( 1,1,1 ), Vec4f(1,0,0,1) );
                    break;
                case 3:
                    vparams->drawTool()->drawFrame ( Vector3(), Quat<SReal>(), Vector3 ( 1,1,1 ), Vec4f(0,0,1,1) );
                    break;
