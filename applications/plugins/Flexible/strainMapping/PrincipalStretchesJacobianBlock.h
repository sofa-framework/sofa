...
template<class InReal, class OutReal, size_type MaterialDimension>
    ...
    */
    ...
    void addmult( OutDeriv& result, const InDeriv& data )
    {
        for( size_type i=0 ; i<spatial_dimensions ; ++i )
            for( size_type j=0 ; j<material_dimensions ; ++j )
                for( size_type k=0 ; k<material_dimensions ; ++k )
    ...
    void addDForce( InDeriv& df, const InDeriv& dx, const OutDeriv& childForce, const SReal& kfactor )
    {
        ...
        else
        {
            ...
            for( size_type k=0 ; k<spatial_dimensions ; ++k ) // line of df
                for( size_type l=0 ; l<material_dimensions ; ++l ) // col of df
                    for( size_type j=0 ; j<material_dimensions ; ++j ) // col of dU & dV
                    {
                        for( size_type i=0 ; i<spatial_dimensions ; ++i ) // line of dU
                            dU[i][j] += _dUOverdF[i*material_dimensions+j][k*material_dimensions+l] * dx.getF()[k][l];

                        for( size_type i=0 ; i<material_dimensions ; ++i ) // line of dV
