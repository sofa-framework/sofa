...
template<size_type matdim,typename Real>
void drawEllipsoid(const defaulttype::Mat<3,matdim,Real> & F, const defaulttype::Vec<3,Real> &p, const float& scale)
{
    ...
    for(size_type i=0; i<3; i++) for(size_type j=0; j<matdim; j++) transformMatrix[4*j+i] = (double)F(i,j)*scale;

    if(matdim==1)
    {
        for(size_type i=0; i<3; i++) for(size_type j=1; j<3; j++) transformMatrix[4*j+i] = 0;
    }
    else if(matdim==2)
    {
        defaulttype::Vec<3,Real> w=cross(F.transposed()[0],F.transposed()[1]); w.normalize();
        for(size_type i=0; i<3; i++)  transformMatrix[8+i]=(double)w[i]*scale*0.01; // arbitrarily small thickness
    }
...
template <size_type L,typename Real>
...
{
    Real n2inv=0; for(size_type i=0; i<L; i++) n2inv+=M[i][0]*M[i][0];
    n2inv=1./n2inv;
    for(size_type i=0; i<L; i++)  Minv[0][i]=M[i][0]*n2inv;
}
...
inline static void invert(defaulttype::Mat<2,3,Real> &Minv, const defaulttype::Mat<3,2,Real> &M)
{
    ...
    w.normalize();
    defaulttype::Mat<3,3,Real> Mc; for(size_type i=0; i<3; i++) {Mc[i][0]=M[i][0]; Mc[i][1]=M[i][1]; Mc[i][2]=w[i];}
    defaulttype::Mat<3,3,Real> Mcinv; invert(Mcinv,Mc);
    for(size_type i=0; i<2; i++) for(size_type j=0; j<3; j++) Minv[i][j]=Mcinv[i][0];
}
...
inline static Mat identity()
{
    ...
    else for(size_type i=0; i<Mat::nbLines; i++) F[i][i]=1.0;
    return F;
}

template <size_type C,size_type L,typename Real>
inline static void identity(defaulttype::Mat<C,L,Real> &F)
{
    F.clear();
    if(L>=C) for(size_type i=0; i<C; i++) F[i][i]=1.0;
    else for(size_type i=0; i<L; i++) F[i][i]=1.0;
}

template <size_type C1,size_type L1,size_type C2,size_type L2,typename Real>
inline static void copy(defaulttype::Mat<C1,L1,Real> &F1, const defaulttype::Mat<C2,L2,Real> &F2)
{
    F1.clear();
    for(size_type c=0; c<C1 && c<C2; c++)
        for(size_type l=0; l<L1 && l<L2; l++)
            F1[c][l]=F2[c][l];
}
