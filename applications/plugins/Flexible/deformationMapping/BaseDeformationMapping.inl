    ...
    if(_sampler) // retrieve initial positions from gauss point sampler (deformation gradient types)
    {
        ...
        if(restPositionSet)     // use custom rest positions defined in state (to set material directions or set residual deformations)
        {
            for(size_t i=0; i<rest.size(); ++i)
                F0[i]=OutDataTypesInfo<Out>::getF(rest[i]);
...
template <class JacobianBlockType>
void BaseDeformationMappingT<JacobianBlockType>::resizeOut(const helper::vector<Coord>& position0, helper::vector<helper::vector<unsigned int> > index,helper::vector<helper::vector<Real> > w, helper::vector<helper::vector<type::Vec<spatial_dimensions,Real> > > dw, helper::vector<helper::vector<defaulttype::Mat<spatial_dimensions,spatial_dimensions,Real> > > ddw, helper::vector<defaulttype::Mat<spatial_dimensions,spatial_dimensions,Real> > F0)
{
    {
        // TODO this must be done before resizeOut() but is done again in Inherit::init();
        // also clean the numerous calls to apply
...
void BaseDeformationMappingT<JacobianBlockType>::draw(const core::visual::VisualParams* vparams)
{
    ...
    {
        helper::vector< type::Vector3 > edge;     edge.resize(2);
