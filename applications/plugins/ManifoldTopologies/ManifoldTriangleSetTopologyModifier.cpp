...
bool ManifoldTriangleSetTopologyModifier::removeTrianglesPreconditions(const sofa::type::vector< Index >& items)
{
...
void ManifoldTriangleSetTopologyModifier::createRemovingTrianglesFutureModifications(const sofa::type::vector< Index >& items)
{
    Triangle vertexTriangle;
    sofa::type::vector<Index> trianglesAroundVertex;
    ...
    for (unsigned int triangleIndex = 0; triangleIndex < items.size(); ++triangleIndex)
    {
        ...
        for (unsigned int vertexIndex = 0; vertexIndex < 3; ++vertexIndex)
        {
            ...
            {
                m_modifications[vertexTriangle[vertexIndex]]=sofa::type::vector<Index>();
...
void ManifoldTriangleSetTopologyModifier::createRemovingEdgesFutureModifications (const sofa::type::vector <Index> items)
{
...
{
    std::map< Index, sofa::type::vector<Index> >::iterator it;
...
void ManifoldTriangleSetTopologyModifier::removeTrianglesPostProcessing(const sofa::type::vector< Index >& edgeToBeRemoved, const sofa::type::vector< Index >& vertexToBeRemoved )
{
...
{
    std::map< Index, sofa::type::vector<Index> >::iterator it;
    sofa::type::vector<Index> vertexshell;
...
void ManifoldTriangleSetTopologyModifier::updateRemovingModifications (const sofa::type::vector< Index >& edgeToBeRemoved,
        const sofa::type::vector< Index >& vertexToBeRemoved)
{
    //???
...
bool ManifoldTriangleSetTopologyModifier::addTrianglesPreconditions( const sofa::type::vector <Triangle> &triangles)
{

    std::map< Index, sofa::type::vector <Triangle> > extremes;
    std::map< Index, Triangle > trianglesList;
    std::map< Index, Triangle >::iterator it;
    sofa::type::vector <Index> listDone;
    ...
    {
        trianglesList.insert ( pair <Index, Triangle> (i, triangles[i]));
    }
    ...
    while ( trianglesList.size() != 0 && allDone == true)
    {
        ...
        for ( it = trianglesList.begin(); it != trianglesList.end(); it++)
        {
            ...
            for (unsigned int vertexIndex = 0; vertexIndex <3; vertexIndex++)
            {
                ...
                {
                    extremes[ (*it).second[vertexIndex] ] = sofa::type::vector <Triangle> ();
                    m_Addmodifications[ (*it).second[vertexIndex] ] = sofa::type::vector <int> ();

                    sofa::type::vector <Index> &trianglesAroundVertex = m_container->getTrianglesAroundVertexForModification((*it).second[vertexIndex]);
            ...
            if (oneDone)
                ...
                for (unsigned int vertexIndex = 0; vertexIndex <3; vertexIndex++)
                {
                    ...
                    {
                        sofa::type::vector <int>::iterator it_vec;
...
void ManifoldTriangleSetTopologyModifier::addTrianglesPostProcessing(const sofa::type::vector <Triangle> &triangles)
{
    ...
    for (it_add = m_Addmodifications.begin(); it_add != m_Addmodifications.end(); it_add++)
    {
        ...
        sofa::type::vector <Index> &trianglesAroundVertex = m_container->getTrianglesAroundVertexForModification((*it_add).first);
        sofa::type::vector <Index> &edgesAroundVertex = m_container->getEdgesAroundVertexForModification((*it_add).first);

        //	  msg_info() << "trianglesAroundVertex: " <<  trianglesAroundVertex;
        //	  msg_info() << "edgesAroundVertex: " <<  edgesAroundVertex;
        sofa::type::vector <Index> triShellTmp;
        sofa::type::vector <Index> edgeShellTmp;
...
//        unsigned int bord2 = 0;
        ...
        for (unsigned int i = 0; i <triShellTmp.size(); i++)
        {
            ...
            else
            {
                ...
                if (before)
                {
                    ...
                    auto &trianglesAroundEdge = m_container->getTrianglesAroundEdgeForModification(edgeShellTmp[i]);
                    Index tmp = trianglesAroundEdge[0];
        ...
        if ( ((*it_add).second).size() != edgesAroundVertex.size()) // we are on the border, one edge is missing
        {
            ...
            sofa::type::vector <Index>::iterator it;
...
void ManifoldTriangleSetTopologyModifier::addRemoveTriangles (const sofa::Size nTri2Add,
        ...
        sofa::type::vector< Index >& trianglesIndex2remove)
{
    // I - Create ROI to remesh: step 1/2
    sofa::type::vector <Index> vertexROI2Remesh;
    ...
    sofa::type::vector <Index> trianglesFinalList;
    trianglesFinalList = trianglesIndex2Add;

    std::sort( trianglesIndex2remove.begin(), trianglesIndex2remove.end(), std::greater<Index>() );
    ...
    if (!topo) //IN DEVELOPMENT (probably only in NDEBUG
    {
        ...
        sofa::type::vector <Index> allmesh;
        for (int i = 0; i <m_container->getNbPoints(); i++)
            allmesh.push_back (i);
...
void ManifoldTriangleSetTopologyModifier::reorderingEdge(const Index edgeIndex)
{
    if(m_container->hasEdges() && m_container->hasTrianglesAroundEdge())
    {
        ...
        helper::ReadAccessor< Data< sofa::type::vector<Triangle> > > m_triangle = m_container->d_triangle;

//        Edge the_edge = m_edge[edgeIndex];
        Index triangleIndex, edgeIndexInTriangle;
        EdgesInTriangle EdgesInTriangleArray;
...
void ManifoldTriangleSetTopologyModifier::reorderingTrianglesAroundVertex (const Index vertexIndex)
{
...
void ManifoldTriangleSetTopologyModifier::reorderingEdgesAroundVertex (const Index vertexIndex)
{
...
void ManifoldTriangleSetTopologyModifier::reorderingTopologyOnROI (const sofa::type::vector <Index>& listVertex)
{
    //To use this function, all shells should have already been created.

    //Finding edges concerned
    for (Index vertexIndex = 0; vertexIndex < listVertex.size(); vertexIndex++)
    {
        bool doublon = false;

        // Avoid doublon
        for(Index i = 0; i<vertexIndex; i++)
        {
        ...
        auto& edgesAroundVertex = m_container->getEdgesAroundVertexForModification( listVertex[vertexIndex] );
        auto& trianglesAroundVertex = m_container->getTrianglesAroundVertexForModification( listVertex[vertexIndex] );

        sofa::type::vector < sofa::type::vector <Index> > vertexTofind;

        sofa::type::vector <Index> goodEdgeShell;
        sofa::type::vector <Index> goodTriangleShell;

        Index firstVertex =0;
        Index secondVertex =0;
        unsigned int cpt = 0;
        ...
        for (Index triangleIndex = 0; triangleIndex < trianglesAroundVertex.size(); triangleIndex++)
        {
        ...
        for (Index triangleIndex = 1; triangleIndex < trianglesAroundVertex.size(); triangleIndex++)
        {
            for (Index pathIndex = 1; pathIndex < trianglesAroundVertex.size(); pathIndex++)
            {
        ...
        if(reverse)
        {
            ...
            for (Index triangleIndex = cpt+1; triangleIndex<trianglesAroundVertex.size(); triangleIndex++)
            {
                for (Index pathIndex = 0; pathIndex<trianglesAroundVertex.size(); pathIndex++)
                {
