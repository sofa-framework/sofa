...
TBulletConvexHullModel<DataTypes>::TBulletConvexHullModel()
    ...
    , computeConvexHullDecomposition(initData(false,"computeConvexHullDecomposition","compute convex hull decomposition using HACD"))
    , drawConvexHullDecomposition(initData(false,"drawConvexHullDecomposition","draw convex hull decomposition using"))
    , CHPoints(initData("CHPoints", "points defining the convex hull"))
    , computeNormals(initData(true, "computeNormals", "set to false to disable computation of triangles normal"))
    , positionDefined(initData(false,"positionDefined","set to true if the collision model position is defined in the mechanical object" ))
    , concavityThreeshold(initData((SReal)100, "concavityThreeshold","Threeshold used in the decomposition"))
    , _mstate(NULL)
{
    bmsh = 0x0;
    enum_type = -1;
