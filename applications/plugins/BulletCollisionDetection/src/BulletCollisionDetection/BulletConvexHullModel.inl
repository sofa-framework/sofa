...
TBulletConvexHullModel<DataTypes>::TBulletConvexHullModel()
    ...
    , concavityThreeshold(initData(&concavityThreeshold, (SReal)100, "concavityThreeshold","Threshold used in the decomposition"))
    , _mstate(NULL)
{
    bmsh = 0x0;
    enum_type = -1;
...
//    _bt_cshape.setMargin(margin.getValue());
    ...
    const Quaternion & ori = _mstate->read(core::vec_id::read_access::position)->getValue()[0].getOrientation();
    const Coord & center = _mstate->read(core::vec_id::read_access::position)->getValue()[0].getCenter();
