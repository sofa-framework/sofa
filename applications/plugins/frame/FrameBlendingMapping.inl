...
template <class TIn, class TOut>
void FrameBlendingMapping<TIn, TOut>::reverseRepartition( bool& influenced, unsigned int& realIndex, const unsigned int& pointIndex, const unsigned int& frameIndex)
{
    const unsigned int& nbRef = this->nbRefs.getValue();
    const vector<unsigned int>& m_reps = this->repartition.getValue();
    influenced = false;
...
template <class TIn, class TOut>
void FrameBlendingMapping<TIn, TOut>::QtoR(Mat33& M, const Quat& q) const
{
    // q to M
...
template <class TIn, class TOut>
void FrameBlendingMapping<TIn, TOut>::ComputeL(Mat76& L, const Quat& q) const
{
    L[0][0]=1;
    L[0][1]=0;
    L[0][2]=0;
...
template <class TIn, class TOut>
void FrameBlendingMapping<TIn, TOut>::ComputeQ(Mat37& Q, const Quat& q, const Vec3& p) const
{
    Q[0][0]=1;
    Q[0][1]=0;
    Q[0][2]=0;
    Q[0][3]=2*(q[1]*p[1]+q[2]*p[2]);
    Q[0][4]=2*(-2*q[1]*p[0]+q[0]*p[1]+q[3]*p[2]);
    Q[0][5]=2*(-2*q[2]*p[0]-q[3]*p[1]+q[0]*p[2]);
    Q[0][6]=2*(-q[2]*p[1]+q[1]*p[2]);
    Q[1][0]=0;
    Q[1][1]=1;
    Q[1][2]=0;
    Q[1][3]=2*(q[1]*p[0]-2*q[0]*p[1]-q[3]*p[2]);
    Q[1][4]=2*(q[0]*p[0]+q[2]*p[2]);
    Q[1][5]=2*(q[3]*p[0]-2*q[2]*p[1]+q[1]*p[2]);
    Q[1][6]=2*(q[2]*p[0]-q[0]*p[2]);
    Q[2][0]=0;
    Q[2][1]=0;
    Q[2][2]=1;
    Q[2][3]=2*(q[2]*p[0]+q[3]*p[1]-2*q[0]*p[2]);
    Q[2][4]=2*(-q[3]*p[0]+q[2]*p[1]-2*q[1]*p[2]);
    Q[2][5]=2*(q[0]*p[0]+q[1]*p[1]);
    Q[2][6]=2*(-q[1]*p[0]+q[0]*p[1]);
}


template <class TIn, class TOut>
void FrameBlendingMapping<TIn, TOut>::ComputeMa(Mat33& M, const Quat& q) const
{
    M[0][0]=0;
    M[0][1]=q[1];
    M[0][2]=q[2];
...
template <class TIn, class TOut>
void FrameBlendingMapping<TIn, TOut>::ComputeMb(Mat33& M, const Quat& q) const
{
    M[0][0]=-2*q[1];
    M[0][1]=q[0];
    M[0][2]=q[3];
...
template <class TIn, class TOut>
void FrameBlendingMapping<TIn, TOut>::ComputeMc(Mat33& M, const Quat& q) const
{
    M[0][0]=-2*q[2];
    M[0][1]=-q[3];
    M[0][2]=q[0];
...
template <class TIn, class TOut>
void FrameBlendingMapping<TIn, TOut>::ComputeMw(Mat33& M, const Quat& q) const
{
    M[0][0]=0;
    M[0][1]=-q[2];
    M[0][2]=q[1];
