...
void CylinderMesh<DataTypes>::draw(const sofa::core::visual::VisualParams* vparams)
{
    ...
    if (m_viewPoints.getValue())
    {
        ...
        for (int i = 0; i < m_nbVertices; ++i)
            points[i] = coords[i];
        vparams->drawTool()->drawPoints(points,8,sofa::type::RGBAColor(0.0, 0.0, 1.0,1));
        points.clear();
        ...
        vparams->drawTool()->drawPoints(points,8,sofa::type::RGBAColor(1.0, 0.0, 0.0,1));
        points.clear();
        // Boundary centers
        ...
        vparams->drawTool()->drawPoints(points,8,sofa::type::RGBAColor(0.0, 1.0, 0.0,1));
    ...
    if (m_viewTetras.getValue())
    {
        ...
        vparams->drawTool()->drawLines(points,1,sofa::type::RGBAColor(1.0, 1.0, 1.0,1));
        vparams->drawTool()->enableLighting();
    }
