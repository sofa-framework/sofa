...
inline void InvertibleFVMForceField<DataTypes>::addForce (const core::MechanicalParams* /*mparams*/ /* PARAMS FIRST */, DataVecDeriv& d_f, const DataVecCoord& d_x, const DataVecDeriv& /* d_v */)
{
    ...
    for( it=_indexedTetra->begin(), elementIndex=0 ; it!=_indexedTetra->end() ; ++it,++elementIndex )
    {
        ...
        //serr<<"InvertibleFVMForceField F_diagonal² "<<F_diagonal<<sendl;
        ...
        if( _verbose.getValue() ) serr<<"InvertibleFVMForceField F_diagonal "<<F_diagonal<<sendl;
        ...
        if( _verbose.getValue() ) serr<<"InvertibleFVMForceField degeneratedF "<<degeneratedF<<sendl;
        ...
        // if the element is inverted, un-invert it by negating a column to make U a rotation
        if( determinant(U) < 0 )
        {
            if( _verbose.getValue() ) serr << "INVERTED\n";
            U[0][Forder[0]] *= -1;
            U[1][Forder[0]] *= -1;
            U[2][Forder[0]] *= -1;
            F_diagonal[Forder[0]] *= -1;
        }
        ...
        //if( fabs( determinant(U) -1 ) > 1e-5  ) serr<<"U is not rotation\n";
        //if( fabs( determinant(V) -1 ) > 1e-5  ) serr<<"V is not rotation\n";


        if( _verbose.getValue() ) serr<<"InvertibleFVMForceField U "<<U<<sendl;
        if( _verbose.getValue() ) serr<<"InvertibleFVMForceField V "<<V<<sendl;
