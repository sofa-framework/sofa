...
NewOmniDriver::NewOmniDriver()
    ...
    , scale(initData(100.0, "scale","Default scale applied to the Phantom Coordinates. "))
    ...
    , posDevice(initData("posDevice", "position of the base of the part of the device"))
    , posStylus(initData("posStylus", "position of the base of the stylus"))
    , locDOF(initData("locDOF","localisation of the DOFs MechanicalObject"))
    , deviceName(initData(std::string("Default PHANToM"),"deviceName","name of the device"))
    ...
    , openSpeedTool(initData(0.1,"openSpeedTool","openSpeedTool value"))
    , closeSpeedTool(initData(0.1,"closeSpeedTool","closeSpeedTool value"))
    , useScheduler(initData(false,"useScheduler","Enable use of OpenHaptics Scheduler methods to synchronize haptics thread"))
    , setRestShape(initData(false, "setRestShape", "True to control the rest position instead of the current position directly"))
    , applyMappings(initData(true, "applyMappings", "True to enable applying the mappings after setting the position"))
    , alignOmniWithCamera(initData(true, "alignOmniWithCamera", "True to keep the Omni's movements in the same reference frame as the camera"))
	, stateButton1(initData(false, "stateButton1", "True if the First button of the Omni is pressed"))
	, stateButton2(initData(false, "stateButton2", "True if the Second button of the Omni is pressed"))
{
