...
/** Base class for the ProjectiveConstraintSet tests.
  Specific test cases can be created using a derived class instantiated on the class to test,

  Francois Faure, INRIA, 2013
  */

template< class _ProjectiveConstraintSet>
struct ProjectiveConstraintSet_test: public Sofa_test<typename _ProjectiveConstraintSet::Real>
{
    typedef _ProjectiveConstraintSet ProjectiveConstraintSet;
    typedef typename ProjectiveConstraintSet::In In;
    ...
    typedef typename ProjectiveConstraintSet::Out Out;
    ...
    core::ProjectiveConstraintSet<In,Out>* ProjectiveConstraintSet; ///< the ProjectiveConstraintSet to be tested
    typename InDOFs::SPtr  inDofs;  ///< ProjectiveConstraintSet input
    typename OutDOFs::SPtr outDofs; ///< ProjectiveConstraintSet output
    ...
    ProjectiveConstraintSet_test():deltaMax(1000),errorMax(10)
    {
        sofa::component::init();
        ...
        ProjectiveConstraintSet = addNew<ProjectiveConstraintSet>(root).get();
        ProjectiveConstraintSet->setModels(inDofs.get(),outDofs.get());
    }
    ...
    /** Test the ProjectiveConstraintSet using the given values and small changes.
     * Return true in case of success, if all errors are below maxError*epsilon.
     * The ProjectiveConstraintSet is initialized using the two first parameters,
                  ...
                  const OutVecCoord expectedChildNew)
    {
        ...
        ProjectiveConstraintSet->apply(&mparams, core::VecCoordId::position(), core::VecCoordId::position());
        ProjectiveConstraintSet->applyJ(&mparams, core::VecDerivId::velocity(), core::VecDerivId::velocity());
...
//        cout<<"random child forces  fc = "<<fc<<endl;
        ...
        ProjectiveConstraintSet->applyJT( &mparams, core::VecDerivId::force(), core::VecDerivId::force() );
...
//        cout<<"child velocity vc = " << vc << endl;
        ...
        ProjectiveConstraintSet->applyDJT( &mparams, core::VecDerivId::force(), core::VecDerivId::force() );
...
//        cout<<"dfp = " << dfp << endl;
        ...
        EigenSparseMatrix* J = this->getMatrix(ProjectiveConstraintSet->getJs());
//        cout<<"J = "<< endl << *J << endl;
        OutVecDeriv Jv(Nc);
        J->mult(Jv,vp);
...
//        cout<<"fp2 - fp = " << fp12 << endl;
    ...
    virtual ~ProjectiveConstraintSet_test()
    {
        if (root!=NULL)
    ...
    {
        if( matrices->size() != 1 ){
            ADD_FAILURE()<< "Matrix list should have size == 1 in simple ProjectiveConstraintSets";
        }
