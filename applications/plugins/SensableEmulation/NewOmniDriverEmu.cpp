...
NewOmniDriverEmu::NewOmniDriverEmu()
    ...
    , positionTool(initData(Vec3d(0,0,0), "positionTool","Position of the tool in the omni end effector frame"))
    , orientationTool(initData(Quat(0,0,0,1), "orientationTool","Orientation of the tool in the omni end effector frame"))
    , permanent(initData(false, "permanent" , "Apply the force feedback permanently"))
    , omniVisu(initData(false, "omniVisu", "Visualize the position of the interface in the virtual scene"))
    , simuFreq(initData(1000, "simuFreq", "frequency of the \"simulated Omni\""))
    , simulateTranslation(initData(false, "simulateTranslation", "do very naive \"translation simulation\" of omni, with constant orientation <0 0 0 1>"))
    , trajPts(initData("trajPoints","Trajectory positions"))
    , trajTim(initData("trajTiming","Trajectory timing"))
    , visu_base(NULL)
    , visu_end(NULL)
{
