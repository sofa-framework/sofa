...
CarvingManager::CarvingManager()
    : d_toolModelPath( initData("toolModelPath", "Tool model path"))
    , d_surfaceModelPath( initData("surfaceModelPath", "TriangleSetModel or SphereCollisionModel<sofa::defaulttype::Vec3Types> path"))
    , d_carvingDistance( initData(0.0, "carvingDistance", "Collision distance at which cavring will start. Equal to contactDistance by default."))
    , d_active( initData(false, "active", "Activate this object.\nNote that this can be dynamically controlled by using a key") )
    , d_keyEvent( initData('1', "key", "key to press to activate this object until the key is released") )
    , d_keySwitchEvent( initData('4', "keySwitch", "key to activate this object until the key is pressed again") )
    , d_mouseEvent( initData(true, "mouseEvent", "Activate carving with middle mouse button") )
    , d_omniEvent( initData(true, "omniEvent", "Activate carving with omni button") )
    , d_activatorName(initData("button1", "activatorName", "Name to active the script event parsing. Will look for 'pressed' or 'release' keyword. For example: 'button1_pressed'"))
