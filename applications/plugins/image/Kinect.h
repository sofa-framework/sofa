...
public:
    ...
    Kinect() : Inherited()
        ...
        , depthImage(initData(DepthTypes(),"depthImage","depth map"))
        , depthTransform(initData(TransformType(), "depthTransform" , ""))
        , deviceID ( initData((unsigned int)0,"deviceID","device ID" ) )
        , resolution ( initData("resolution","resolution" ) )
        , videoMode ( initData("videoMode","video mode" ) )
        , depthMode ( initData("depthMode","depth mode" ) )
        , ledMode ( initData("ledMode","led mode" ) )
        , tiltAngle(initData(0,"tiltAngle","tilt angle in [-30,30]"))
        , accelerometer(initData(Vector3(0,0,0),"accelerometer","Accelerometer data"))
        , drawBB(initData(true,"drawBB","draw bounding box"))
        , drawGravity(initData(true,"drawGravity","draw acceleration"))
        , showArrowSize(initData(0.1f,"showArrowSize","size of the axis"))
        , die(0)
        , got_rgb(0)
        , got_depth(0)
    {
