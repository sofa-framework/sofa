...
public:
    ...
    ImageSampler()    :   Inherited()
      ...
      , method ( initData("method","method (param)" ) )
      , computeRecursive(initData(false,"computeRecursive","if true: insert nodes recursively and build the graph"))
      , param ( initData("param","Parameters" ) )
      , position(initData(SeqPositions(),"position","output positions"))
      , fixedPosition(initData(SeqPositions(),"fixedPosition","user defined sample positions"))
      , edges(initData(SeqEdges(),"edges","edges connecting neighboring nodes"))
      , graphEdges(initData(SeqEdges(),"graphEdges","oriented graph connecting parent to child nodes"))
      , hexahedra(initData(SeqHexahedra(),"hexahedra","output hexahedra"))
      , distances(initData(DistTypes(),"distances",""))
      , voronoi(initData(VorTypes(),"voronoi",""))
      , f_clearData(initData(true,"clearData","clear distance image after computation"))
      , showSamplesScale(initData(0.0f,"showSamplesScale","show samples"))
      , drawMode(initData(0,"drawMode","0: points, 1: spheres"))
      , showEdges(initData(false,"showEdges","show edges"))
      , showGraph(initData(false,"showGraph","show graph"))
      , showFaces(initData(false,"showFaces","show the faces of cubes"))
      , time((unsigned int)0)
    {
