...
#else
	#if DiffusionSolver_EXPORTS
		#define EXPORT_DYNAMIC_LIBRARY __declspec( dllexport )
	#else 
		#define EXPORT_DYNAMIC_LIBRARY __declspec( dllimport )
	#endif
...
struct EXPORT_DYNAMIC_LIBRARY DiffusionSolver
{
    /// the scalar type
    typedef _Real Real;
...
EXPORT_DYNAMIC_LIBRARY char init_DiffusionSolver_module();
#endif
