...
public:
    ...
    Data<bool> computeVolumes;
    Data< helper::vector<Real> > volume;
    ...
    ImageToBranchingImageConverter()    :   Inherited()
      ...
      , outputMappingImage(initData(&outputMappingImage,ImageLabelTypes(),"outputMappingImage","A regular with the same size as inputImage, its associated transform is inputTransform. Each pixel stores the index of the corresponding branching grid voxel"))
      , computeVolumes(initData(&computeVolumes,false,"computeVolumes","Will the list of voxel volumes be computed based on the fine image ?"))
      , volume(initData(&volume,"volume","output list of voxel volumes"))
    {
...
protected:
    ...
    virtual void update()
    {
        ...
        if( coarseningLevels.getValue()<1 )
        {
...

            {
                ...
                for( unsigned t=0 ; t<labelDimension[BranchingImageTypes::DIMENSION_T] ; ++t )
                        cimg_forXYZ(input.getCImg(t),x,y,z)
                            if(input.getCImg(t)(x,y,z)) labelImages.getCImg(t)(x,y,z)=count++; else labelImages.getCImg(t)(x,y,z)=0;

                //for( unsigned t=0 ; t<labelDimension[BranchingImageTypes::DIMENSION_T] ; ++t ) labelImages.getCImg(t).fill( 0 );
            }
        }
        else
        {
            ...
            for( unsigned t=0 ; t<outputDimension[BranchingImageTypes::DIMENSION_T] ; ++t )
            {
                                ...
                                case 2: // ratio (nb fine in superimposed voxel / total fine)
                                {
                                    for( unsigned v=0 ; v<output_t[index1d].size() ; ++v )
                                    {
                                        ...
                                        cimg_foroff( subLabelImage, off )
                                                if( subLabelImage(off) == (LabelTypes)(v+1) )
                                        {
                                            nbLabelsV++;
                                        }
                                        for( unsigned s=0 ; s<outputDimension[BranchingImageTypes::DIMENSION_S] ; ++s )
                ...
                cimg_foroff( labelImage, off ) labelImage[off]--; // convert labels to offsets in the superimposed vector

                // replace labels by indices of the corresponding branching grid voxels
                {
                    unsigned int count=0;
                    for( unsigned z=0 ; z<output.dimension[BranchingImageTypes::DIMENSION_Z] ; ++z )
                        for( unsigned y=0 ; y<output.dimension[BranchingImageTypes::DIMENSION_Y] ; ++y )
                            for( unsigned x=0 ; x<output.dimension[BranchingImageTypes::DIMENSION_X] ; ++x )
                            {
                                unsigned int index1d=output.index3Dto1D(x,y,z);
                                if(output_t[index1d].size())
                                {
                                    unsigned finex = x*nb, finexend = std::min( (x+1)*nb, inputDimension[BranchingImageTypes::DIMENSION_X] ) - 1;
                                    unsigned finey = y*nb, fineyend = std::min( (y+1)*nb, inputDimension[BranchingImageTypes::DIMENSION_Y] ) - 1;
                                    unsigned finez = z*nb, finezend = std::min( (z+1)*nb, inputDimension[BranchingImageTypes::DIMENSION_Z] ) - 1;
                                    for( unsigned zf=finez ; zf<=finezend ; ++zf )
                                        for( unsigned yf=finey ; yf<=fineyend ; ++yf )
                                            for( unsigned xf=finex ; xf<=finexend ; ++xf )
                                                if(labelImage(xf,yf,zf)!=-1)
                                                    labelImage(xf,yf,zf)+=count;
                                    count+=output_t[index1d].size();
                                }
                            }
                }
        ...
        if( computeVolumes.getValue() )
        {
            helper::WriteAccessor<Data< helper::vector<Real> > > vol(this->volume);

            Real voxelVol=this->inputTransform.getValue().getScale()[0]*this->inputTransform.getValue().getScale()[1]*this->inputTransform.getValue().getScale()[2];

            const unsigned int maxlabel=labelImages.getCImg(0).max();

            vol.resize(maxlabel+1); for(unsigned int i=0;i<maxlabel+1;i++) vol[i]=0;

            cimg_forXYZ(labelImages.getCImg(0),x,y,z)
                    if(labelImages.getCImg(0)(x,y,z)>=0)
                            vol[(unsigned int)labelImages.getCImg(0)(x,y,z)]+=voxelVol;
/*
            // count number of voxels in branching grid at time t=0
            unsigned int num_vox=0; for(unsigned int i=0 ; i<output.imageSize; i++ ) num_vox+=output.imgList[0][i].size();
            vol.resize(num_vox); for(unsigned int i=0;i<num_vox;i++) vol[i]=0;

            // assign volumes based on fine grid
            typename ImageLabelTypes::CImgT& labelImage = labelImages.getCImg(0);
            const typename ImageTypes::imCoord inputDimension = input.getDimensions();
            unsigned nb = pow( (float)2, (int)coarseningLevels.getValue() ); // nb fine voxels in a coarse voxel in each direction x/y/z
            num_vox=0;
            for( unsigned z=0 ; z<output.dimension[BranchingImageTypes::DIMENSION_Z] ; ++z )
                for( unsigned y=0 ; y<output.dimension[BranchingImageTypes::DIMENSION_Y] ; ++y )
                    for( unsigned x=0 ; x<output.dimension[BranchingImageTypes::DIMENSION_X] ; ++x )
                    {
                        ...
                        unsigned finey = y*nb, fineyend = std::min( (y+1)*nb, inputDimension[BranchingImageTypes::DIMENSION_Y] ) - 1;
                        unsigned finez = z*nb, finezend = std::min( (z+1)*nb, inputDimension[BranchingImageTypes::DIMENSION_Z] ) - 1;
                        if(output.imgList[0][index1d].size())
                            for( unsigned zf=finez ; zf<=finezend ; ++zf )
                                for( unsigned yf=finey ; yf<=fineyend ; ++yf )
                                    for( unsigned xf=finex ; xf<=finexend ; ++xf )
                                    {
                                        vol[num_vox+(unsigned int)labelImage(xf,yf,zf)]+=voxelVol;
                                    }
                        num_vox+=output.imgList[0][index1d].size();
                    }
                    */
        }
