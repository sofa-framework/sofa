...
//          std::cerr<<flatDim<<std::endl;
          ...
          branchingImage.toImage( flatImage2 );
      }
...
//                    const typename BranchingImageTypes::Voxels& voxels = iml[index1d];
                    ...
                    if( neighbourTest )
                    {
                        ...
                        if( x>0 && ( ( cimgl.get_vector_at(x-1,y,z).magnitude(1)==0 ) == voxels[0].connections[sofa::defaulttype::BranchingImageB::LEFT].empty() ) ) return false;
                        if( (unsigned)x<flatImg.getDimensions()[0]-1 && ( ( cimgl.get_vector_at(x+1,y,z).magnitude(1)==0 ) == voxels[0].connections[sofa::defaulttype::BranchingImageB::RIGHT].empty() ) ) return false;
                        if( y>0 && ( ( cimgl.get_vector_at(x,y-1,z).magnitude(1)==0 ) == voxels[0].connections[sofa::defaulttype::BranchingImageB::BOTTOM].empty() ) ) return false;
                        if( (unsigned)y<flatImg.getDimensions()[1]-1 && ( ( cimgl.get_vector_at(x,y+1,z).magnitude(1)==0 ) == voxels[0].connections[sofa::defaulttype::BranchingImageB::TOP].empty() ) ) return false;
                        if( z>0 && ( ( cimgl.get_vector_at(x,y,z-1).magnitude(1)==0 ) == voxels[0].connections[sofa::defaulttype::BranchingImageB::BACK].empty() ) ) return false;
                        if( (unsigned)z<flatImg.getDimensions()[2]-1 && ( ( cimgl.get_vector_at(x,y,z+1).magnitude(1)==0 ) == voxels[0].connections[sofa::defaulttype::BranchingImageB::FRONT].empty() ) ) return false;
                    }
                }
          }
          return true;
