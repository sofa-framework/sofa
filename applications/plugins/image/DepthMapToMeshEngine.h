...
public:
    ...
    DepthMapToMeshEngine()    :   Inherited()
        , depthFactor(initData((Real)(1.0),"depthFactor","Intensity to depth factor"))
        , minThreshold(initData((Real)(0.1),"minThreshold","minimal depth for point creation"))
        , diffThreshold(initData((Real)(1.0),"diffThreshold","maximal depth variation for triangle creation"))
        , image(initData(ImageTypes(),"image",""))
        , transform(initData(TransformType(),"transform",""))
        , texImage(initData(TextureTypes(),"texImage",""))
        , position(initData(SeqPositions(),"position","output positions"))
        , texCoord(initData(SeqTexCoords(),"texCoord","output texture coordinates"))
        , texOffset(initData(TexCoord(0.0,0.0),"texOffset","texture offsets (in [0,1])"))
