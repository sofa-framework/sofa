...
extern "C" {
    ...
    bool SOFACUDA_CORE_API CudaScanGPUAvailable(unsigned int size, ScanType type);
    bool SOFACUDA_CORE_API CudaScanGPU(const void* input, void* output, unsigned int size, ScanType type);
