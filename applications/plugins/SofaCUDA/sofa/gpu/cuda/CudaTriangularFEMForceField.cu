...
*/
__global__ void TriangularFEMForceFieldCuda3f_addForce_kernel(int size, CudaVec3<float>* f, const CudaVec3<float>* x, const CudaVec3<float>* v,
...
__global__ void TriangularFEMForceFieldCuda3f_addDForce_kernel(int size, CudaVec3<float>* df, const CudaVec3<float>* dx, float kFactor,
    ...
    float gamma, float mu)
{
    ...
    {TriangularFEMForceFieldCuda3f_addForce_kernel<<< grid, threads >>>(size, (CudaVec3<float>*)f, (const CudaVec3<float>*)x, (const CudaVec3<float>*)v,
                                                                                                    (TriangleState*) triangleState,
                                                                                                    (const TriangleInfo*) triangleInfo,
                                                                                                    nbTriangles,
                                                                                                    (const GPUTriangleInfo*) gpuTriangleInfo,
                                                                                                    gamma, mu
        ); mycudaDebugError("TriangularFEMForceFieldCuda3f_addForce_kernel");}
}

void TriangularFEMForceFieldCuda3f_addDForce(unsigned int size, void* df, const void* dx, float kFactor,
    const void* triangleState, const void* triangleInfo,
    unsigned int nbTriangles,
    const void* gpuTriangleInfo,
    float gamma, float mu) //, const void* dfdx)
{
    dim3 threads(BSIZE,1);
    dim3 grid((nbTriangles+BSIZE-1)/BSIZE,1);
    {TriangularFEMForceFieldCuda3f_addDForce_kernel<<< grid, threads>>>(size, (CudaVec3<float>*)df, (const CudaVec3<float>*)dx, kFactor,
                                                                                                    (const TriangleState*) triangleState,
                                                                                                    (const TriangleInfo*) triangleInfo,
                                                                                                    nbTriangles,
                                                                                                    (const GPUTriangleInfo*) gpuTriangleInfo,
                                                                                                    gamma, mu
        ); mycudaDebugError("TriangularFEMForceFieldCuda3f_addDForce_kernel");}
}
