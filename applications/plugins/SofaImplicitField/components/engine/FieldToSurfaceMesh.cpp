...
{
    addInputs({&mStep, &mIsoValue, &mGridMin, &mGridMax});
    addOutputs({&d_out_triangles, &d_out_points});
}
...
void FieldToSurfaceMesh::init()
{
    ...
    addInputs({&l_field->d_componentState});
    d_componentState = core::objectmodel::ComponentState::Valid;
    update();
}

void FieldToSurfaceMesh::doUpdate(){
...
void FieldToSurfaceMesh::updateMeshIfNeeded(){
    ...
    if(workFinished.load()){
        std::cout << "COPY VALUE INTO OUT MEHS" << std::endl;
...
void FieldToSurfaceMesh::draw(const VisualParams* vparams)
{
    ...
    auto gridMin = mGridMin.getValue();
    auto gridMax = mGridMax.getValue();

    dt->drawBoundingBox(gridMin, gridMax) ;
