...
IHPDriver::IHPDriver()
    ...
    , forceScale(initData(0.0001, "forceScale","Default scale applied to the force feedback. "))
    , permanent(initData(true, "permanent" , "Apply the force feedback permanently"))
    , indexTool(initData((int)0,"toolIndex", "index of the tool to simulate (if more than 1). Index 0 correspond to first tool."))
    , graspThreshold(initData(0.2, "graspThreshold","Threshold value under which grasping will launch an event."))
    , showToolStates(initData(false, "showToolStates" , "Display states and forces from the tool."))
    , testFF(initData(false, "testFF" , "If true will add force when closing handle. As if tool was entering an elastic body."))
    , RefreshFrequency(initData((int)500,"RefreshFrequency", "Frequency of the haptic loop."))
    , xitactVisu(initData(false, "xitactVisu", "Visualize the position of the interface in the virtual scene"))
    , positionBase(initData("positionBase", "position of the base of the device"))
    , locPosBati(initData(std::string("nodeBati/posBati"),"locPosBati","localisation of the restPosition of the bati"))
    , deviceIndex(initData(1,"deviceIndex","index of the device"))
    , openTool(initData("openTool","opening of the tool"))
    , maxTool(initData(1.0,"maxTool","maxTool value"))
    , minTool(initData(0.0,"minTool","minTool value"))
{
