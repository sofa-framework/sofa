...
public:
    ...
    */
    ...
    cimg_library::CImg<bool> get_slicedModels(const unsigned int coord,const unsigned int axis,const Mat<2,3,unsigned int>& ROI,const helper::vector<type::Vec<3,Real> >& position, const helper::vector< component::visualmodel::VisualModelImpl::VisualTriangle >& triangle, const helper::vector< component::visualmodel::VisualModelImpl::VisualQuad >& quad) const
    {
        ...
        else
        {
            ...
            for (unsigned int i = 0; i < triangle.size() ; i++)  // box/ triangle intersection -> polygon with a maximum of 5 edges, to draw
            {
                ...
                helper::vector<type::Vec<3,int> > pts;
