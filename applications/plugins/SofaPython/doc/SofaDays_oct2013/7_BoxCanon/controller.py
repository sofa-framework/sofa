...
class Tuto7(Sofa.PythonScriptController):
	...
	def onKeyPressed(self,k):
		...
		if ord(k)==18:
			...
			obj = ObjectCreator.createCube(self.rootNode,'CubeCollisionModel',-50,50,0,60,fireAngle,0,'blue')
			#obj.findData('restVelocity').value=[0.0, 20.0, 0.0 ]
		# RIGHT key : right
		if ord(k)==20:
			print 'Cube launched (right)!'
			obj = ObjectCreator.createCube(self.rootNode,'CubeCollisionModel',50,50,0,-60,fireAngle,0,'magenta')
			#obj.findData('restVelocity').value=[0.0, 20.0, 0.0 ]
		# UP key : front
		if ord(k)==19:
			print 'Cube launched (front)!'
			obj = ObjectCreator.createCube(self.rootNode,'CubeCollisionModel',0,50,50,0,fireAngle,-60,'yellow')
			#obj.findData('restVelocity').value=[0.0, 20.0, 0.0 ]
		# DOWN key : rear
		if ord(k)==21:
			print 'Cube launched (back)!'
			obj = ObjectCreator.createCube(self.rootNode,'CubeCollisionModel',0,50,-50,0,fireAngle,60,'cyan')
			#obj.findData('restVelocity').value=[0.0, 20.0, 0.0 ]
		return 0 
