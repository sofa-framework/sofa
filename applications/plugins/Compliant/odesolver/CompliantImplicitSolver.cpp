...
//        integrate( sop, posId, velId );
                                                  ...
                                                  bool stabilizing )
    {
        ...
        const bool useVelocity = use_velocity.getValue();

        SReal f_factor, lambda_factor, m_factor, b_factor, k_factor;
        ...
        {
            f_factor = h;
            lambda_factor = ( useVelocity ? h : 1.0 );
        }
        else
        {
           f_factor = lambda_factor = ( useVelocity ? h : 1.0 );
        }

        if( useVelocity ) // c = h.f + Mv
        {
            // M v_k
            m_factor = 1;
            // h (1-alpha) B v_k
        ...
        if( !neglecting_compliance_forces_in_geometric_stiffness.getValue() )
        {
            ...
            simulation::MechanicalAddComplianceForce lvis( &sop.mparams(), f, lagrange, lambda_factor ); // f += fc   f += lambda / dt
            send( lvis );
...

        {
            ...
            sop.mop.addMBKv( c, m_factor, b_factor, k_factor );
        }
