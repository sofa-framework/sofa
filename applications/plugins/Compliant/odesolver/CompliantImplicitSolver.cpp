...
//        integrate( sop, posId, velId );
                                         ...
                                         core::behavior::MultiVecDeriv& c )
    {
        ...
        if( use_velocity.getValue() ) // c = h.f + Mv
        {
        ...
        else // acceleration: c = f + hKv + Dv
        {
            ...
            {
                scoped::timer substep("f += fc");
                simulation::MechanicalAddComplianceForce lvis( &sop.mparams(), f, lagrange, 1.0 ); // f += fc   f += lambda
                send( lvis );
            }

            sop.mop.addMBKv( c, 0, 1.0, h*alpha.getValue() );
...
//            vop.print( lagrange,std::cerr, "BEGIN lambda: ", "\n" );
        ...
        // ready to solve
        {
            ...
            if( sys.n && stabilizationType==PRE_STABILIZATION && useVelocity ) // does not work in acceleration
            {
...
//        vop.print( lagrange,std::cerr, "END lambda: ", "\n" );
    ...
    void CompliantImplicitSolver::post_stabilization( SolverOperations& sop,
                                              ...
                                              bool fullAssembly )
    {
        if( !sys.n || !use_velocity.getValue() ) return; // does not work in acceleration
