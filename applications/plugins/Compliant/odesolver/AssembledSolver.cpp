...
//         state->vInit(mparams, out.getId(state), in.getId(state));
                                         ...
                                         core::behavior::MultiVecDeriv& c )
    {
        ...
        MultiVecDeriv fk( &vop ); fk.realloc( &vop, false, true ); // forces generated by stiffness force fields
        MultiVecDeriv fc( &vop ); fc.realloc( &vop, false, true ); // forces generated by compliant force fields
        simulation::MechanicalComputeForcesVisitor fvis( &params, fk, fc, f ); // f = fk + fc
        send( fvis );
        vop.v_free( fc.id(), false, true ); // note that we do not need fc explicitly but the visitor computing the forces would be more complicated
