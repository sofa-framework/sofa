  ...
  public:
	...
	virtual void assemble( const typename self::in_pos_type& in ) {
        ...
        for(size_t k = 0; k < size; ++k)
        {
            ...
            const Real t1 = v[2] * v[2];
            const Real t2 = v[0] * v[0];
            const Real t3 = v[1] * v[1];
            Real t4 = t1 + t2 + t3;
            const Real t5 = 1. / ( t4 * sqrt(t4) ); //pow(t4, -0.3e1 / 0.2e1);
            t4 = t4 * t5;
            const Real t6 = v[1] * t5 * v[0];
            Real t7 = v[2] * t5;
            const Real t8 = t7 * v[0];
            t7 = t7 * v[1];
...
//    {
//        The code is generated by maple, so it should be ok, but does not correspond to the finite differences performed in Mapping_test
