...
\end{equation}
%
As symmetric \emph{indefinite} systems, they tend to be more difficult
to solve than positive definite ones: CG and Cholesky $LDL^T$
factorizations may breakdown on such systems. However, when the $(1,
1)$ block (here, matrix $Q$) is invertible, one can use
\emph{pivoting} to obtain the following smaller system:
%
\begin{align}
...
\end{equation}
%
The Schur complement becomes $S + D$ and corresponds to:
%
\begin{equation}
...
towards zero. It is a form of \emph{constraint relaxation}. Under
their most general form, KKT systems include unilateral and
complementarity constraints. We will develop these in more details in
section \ref{sec:unilateral-constraints}.
%
\section{KKT Systems, Optimization and Relaxation}
\label{sec:optimization}
%
We have seen that KKT systems correspond to Quadratic Programs:
%
\begin{equation}
   \mat{Q & -A^T \\ -A & 0} \mat{x \\ \lambda} = \mat{c \\ b}
\end{equation}
%
corresponds to the following convex QP:
%
\begin{equation}
  \argmin{Ax = b}\quad \half x^T Q x - c^T x
\end{equation}

%
The compliant KKT system obtained in section \ref{sec:compliance} is
of the form:
%
\begin{equation}
  \mat{Q & -A^T \\ -A & -D} \mat{x \\ \lambda} = \mat{c \\ b}
\end{equation}
%
where both $Q$ and $C$ are PSD. 

 TODO: effect of $C$ on the schur complement, lagrangian
relaxation
%
