...
\end{array}\right) 
\end{equation}
We assemble the $7$ terms of equation~\ref{eq abstract system} separately. Figure~\ref{fig example system} shows an example of mechanical system and matrix.
The independent DOFs are $X_{a}$ and $X_d$. 
\begin{figure}
 \includegraphics[width=0.29\linewidth]{system-graph.png}
 \includegraphics[width=0.7\linewidth]{system-bigmatrix.png}
\label{fig example system}
\caption{A mechanical system and its matrix equation. Left: scene graph. Right: block view of the corresponding equation system, with non-null blocks highlighted in yellow.}
\end{figure}
...
The corresponding equation system has the block structure shown in the right of Figure~\ref{fig example system}.

The $8$ parts of the equation system are shown in Figure~\ref{fig system matrices}. The $J$ matrices are the mapping matrices. The bottom row has two mappings, since the state $X_e$ impacted by compliance $\beta$ depends on two parent states.
\begin{figure}
 \includegraphics[width=0.7\linewidth]{system-8parts.png}
\label{fig system matrices}
\caption{Top: the $8$ parts of the equation system. Bottom: the offset matrices used in the assembly.}
\end{figure}
Offset matrices $J_{*0}$ are shown in the bottom of the figure. They are composed of identity matrices (represented with a diagonal in a block) and null blocks. These matrices are used to shift the indices of local vector and matrices to global numbering.
