...
void QSofaViewer::toggleShowPointed()
{
//    cerr<<"QSofaViewer::toggleShowPointed()" << endl;
    if( _showPointed )
    {
        _showPointed = false;
...
void QSofaViewer::mouseMoveEvent ( QMouseEvent * event )
{
    ...
    if( _showPointed ){ // cast a ray to figure out what is under the pointer

        sofa::component::collision::RayTriangleVisitor rtv;
        // Set the direction. Note that any direction would work. Setting it helps debugging.
        _sofaGL->getPickDirection(&rtv.direction[0],&rtv.direction[1],&rtv.direction[2],event->x(),event->y());

        Vec3 origin(_camera.eye()[0],_camera.eye()[1],_camera.eye()[2]);
        rtv.origin = origin;

        // If clipping plane (assuming there can not be more than one) change the origin of the ray
        // TODO: make this more general, by handling the clipping planes in the visitor
        using sofa::component::visualmodel::ClipPlane;
        ClipPlane* clipPlane = _sofaScene->groot()->get<ClipPlane>(sofa::core::objectmodel::BaseContext::SearchDown);
        if( clipPlane != NULL )
        {
            // find the intersection of the ray with the plane
            Vec3 N =clipPlane->normal.getValue();

            // find intersection point
            SReal NdotRayDir = N*rtv.direction;
            SReal eps = std::numeric_limits<SReal>::epsilon() * 100;
            if( fabs(NdotRayDir)>eps) {// ray not parallel to plane.
                SReal d= N*clipPlane->position.getValue();
                SReal t= -(N*origin + d)/NdotRayDir;
                if( NdotRayDir < 0 ) { // the triangle is forward
                    rtv.origin = origin + t * rtv.direction;
//                    cerr<<"origin moved to the clipping plane"<< endl;
                }
            }
        }

        // go !
        _sofaScene->groot()->executeVisitor(&rtv);
        sofa::core::objectmodel::BaseObject* cm = rtv.embeddingModel();
        if( cm!=NULL ){
//            cerr<<"Inside " << cm->getName() << endl;
            setToolTip(cm->getName().c_str());
        }
        else {
//            cerr<<"Outside all" << endl;
            setToolTip("Outside all");
        }
