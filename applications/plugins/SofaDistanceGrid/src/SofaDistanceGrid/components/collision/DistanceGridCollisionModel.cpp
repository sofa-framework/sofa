...
RigidDistanceGridCollisionModel::RigidDistanceGridCollisionModel()
    : modified(true)
    , fileRigidDistanceGrid( initData("filename", "Load distance grid from specified file"))
    , scale( initData(1.0, "scale", "scaling factor for input file"))
    , translation( initData("translation", "translation to apply to input file"))
    , rotation( initData("rotation", "rotation to apply to input file"))
    ...
    , flipNormals( initData(false, "flipNormals", "reverse surface direction, i.e. points are considered in collision if they move outside of the object instead of inside"))
    , showMeshPoints( initData(true, "showMeshPoints", "Enable rendering of mesh points"))
    , showGridPoints( initData(false, "showGridPoints", "Enable rendering of grid points"))
    , showMinDist ( initData(0.0, "showMinDist", "Min distance to render gradients"))
    , showMaxDist ( initData(0.0, "showMaxDist", "Max distance to render gradients"))
{
    rigid = NULL;
...
FFDDistanceGridCollisionModel::FFDDistanceGridCollisionModel()
    : fileFFDDistanceGrid( initData("filename", "Load distance grid from specified file"))
    , scale( initData(1.0, "scale", "scaling factor for input file"))
    , sampling( initData(0.0, "sampling", "if not zero: sample the surface with points approximately separated by the given sampling distance (expressed in voxels if the value is negative)"))
    , box( initData("box", "Field bounding box defined by xmin,ymin,zmin, xmax,ymax,zmax") )
    , nx( initData(64, "nx", "number of values on X axis") )
    ...
    , dumpfilename( initData("dumpfilename","write distance grid to specified file"))
    , usePoints( initData(true, "usePoints", "use mesh vertices for collision detection"))
    , singleContact( initData(false, "singleContact", "keep only the deepest contact in each cell"))
{
    ffd = NULL;
    ffdMesh = NULL;
    ffdRGrid = NULL;
