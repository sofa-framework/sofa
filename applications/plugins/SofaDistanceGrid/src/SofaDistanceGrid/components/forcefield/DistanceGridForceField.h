...
protected:
    DistanceGridForceField()
        : grid(NULL)
        , fileDistanceGrid( initData("filename", "load distance grid from specified file"))
        ...
        , stiffnessIn(initData((Real)500, "stiffnessIn", "force stiffness when inside of the object"))
        , stiffnessOut(initData((Real)0, "stiffnessOut", "force stiffness when outside of the object"))
        , damping(initData((Real)0.01, "damping", "force damping coefficient"))
        , maxDist(initData((Real)1.0, "maxdist", "max distance of the surface after which no more force is applied"))
        , minArea(initData((Real)0, "minArea", "minimal area for each triangle, as seen from the direction of the local surface (i.e. a flipped triangle will have a negative area)"))
        , stiffnessArea(initData((Real)100, "stiffnessArea", "force stiffness if a triangle have an area less than minArea"))
        , minVolume(initData((Real)0, "minVolume", "minimal volume for each tetrahedron (a flipped triangle will have a negative volume)"))
        , stiffnessVolume(initData((Real)0, "stiffnessVolume", "force stiffness if a tetrahedron have an volume less than minVolume"))
        , color(initData(sofa::type::RGBAColor(0.0f,0.5f,0.2f,1.0f), "color", "display color.(default=[0.0,0.5,0.2,1.0])"))
        , bDraw(initData(false, "draw", "enable/disable drawing of distancegrid"))
        , drawPoints(initData(false, "drawPoints", "enable/disable drawing of distancegrid"))
        , drawSize(initData((Real)10.0f, "drawSize", "display size if draw is enabled"))
