...
GeomagicDriver::GeomagicDriver()
    : d_deviceName(initData(std::string("Default Device"), "deviceName","Name of device Configuration"))
    , d_positionBase(initData(Vec3(0,0,0), "positionBase","Position of the device base in the SOFA scene world coordinates"))
    , d_orientationBase(initData(Quat(0,0,0,1), "orientationBase","Orientation of the device base in the SOFA scene world coordinates"))
    , d_orientationTool(initData(Quat(0,0,0,1), "orientationTool","Orientation of the tool in the SOFA scene world coordinates"))
    , d_scale(initData(1.0, "scale", "Default scale applied to the Device coordinates"))
    , d_forceScale(initData(1.0, "forceScale", "Default forceScale applied to the force feedback. "))
    , d_maxInputForceFeedback(initData(double(1.0), "maxInputForceFeedback", "Maximum value of the normed input force feedback for device security"))
    , d_inputForceFeedback(initData(Vec3(0, 0, 0), "inputForceFeedback", "Input force feedback in case of no LCPForceFeedback is found (manual setting)"))
    , d_manualStart(initData(false, "manualStart", "If true, will not automatically initDevice at component init phase."))
    , d_emitButtonEvent(initData(false, "emitButtonEvent", "If true, will send event through the graph when button are pushed/released"))
    , d_frameVisu(initData(false, "drawDeviceFrame", "Visualize the frame corresponding to the device tooltip"))
    , d_omniVisu(initData(false, "drawDevice", "Visualize the Geomagic device in the virtual scene"))    
    , d_posDevice(initData("positionDevice", "position of the base of the part of the device"))
    , d_angle(initData("angle", "Angluar values of joint (rad)"))
    , d_button_1(initData("button1","Button state 1"))
    , d_button_2(initData("button2","Button state 2"))    
