...
GlobalModification::GlobalModification(const InternalStorage &c, GraphHistoryManager* h): components(c), historyManager(h)
{
    ...
    sofa::helper::set< std::string > allNames;
    sofa::helper::set< std::string > allAliases;
    ...
    for (sofa::helper::set< std::string >::const_iterator it=allNames.begin(); it!=allNames.end(); ++it)
        listDataName << it->c_str();

    for (sofa::helper::set< std::string >::const_iterator it=allAliases.begin(); it!=allAliases.end(); ++it)
