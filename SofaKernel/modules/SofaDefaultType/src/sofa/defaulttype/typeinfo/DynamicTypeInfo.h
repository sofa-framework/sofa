...
/** Information about the type of a value stored in a Data.
    ...
    %BaseTypeInfo is part of the introspection/reflection capabilities of
    ...
    const BaseTypeInfo *typeinfo = data->getValueTypeInfo();
...
public:
    ...
    virtual const BaseTypeInfo* getBaseType() const = 0;

    /// Returns the TypeInfo for the type of the values accessible by the
    /// get*Value() functions.
    /// For example, if the type is `fixed_array<fixed_array<int, 2> 3>`, it
    /// returns the TypeInfo for `int`.
    virtual const BaseTypeInfo* getValueType() const = 0;
...
private: // copy constructor or operator forbidden
    BaseTypeInfo(const BaseTypeInfo&) {}
    void operator=(const BaseTypeInfo&) {}
