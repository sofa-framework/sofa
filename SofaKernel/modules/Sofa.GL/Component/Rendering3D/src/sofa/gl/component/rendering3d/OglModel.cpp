...
OglModel::OglModel()
    : blendTransparency(initData( true, "blendTranslucency", "Blend transparent parts"))
    , premultipliedAlpha(initData( false, "premultipliedAlpha", "is alpha premultiplied ?"))
    , writeZTransparent(initData( false, "writeZTransparent", "Write into Z Buffer for Transparent Object"))
    , alphaBlend(initData( false, "alphaBlend", "Enable alpha blending"))
    , depthTest(initData( true, "depthTest", "Enable depth testing"))
    , cullFace(initData( 0, "cullFace", "Face culling (0 = no culling, 1 = cull back faces, 2 = cull front faces)"))
    , lineWidth(initData( 1.0f, "lineWidth", "Line width (set if != 1, only for lines rendering)"))
    , pointSize(initData( 1.0f, "pointSize", "Point size (set if != 1, only for points rendering)"))
    , lineSmooth(initData( false, "lineSmooth", "Enable smooth line rendering"))
    , pointSmooth(initData( false, "pointSmooth", "Enable smooth point rendering"))
    , isEnabled( initData( true, "isEnabled", "Activate/deactive the component."))
    , primitiveType( initData( "primitiveType", "Select types of primitives to send (necessary for some shader types such as geometry or tesselation)"))
    , blendEquation( initData( "blendEquation", "if alpha blending is enabled this specifies how source and destination colors are combined") )
    , sourceFactor( initData( "sfactor", "if alpha blending is enabled this specifies how the red, green, blue, and alpha source blending factors are computed") )
    , destFactor( initData( "dfactor", "if alpha blending is enabled this specifies how the red, green, blue, and alpha destination blending factors are computed") )
    , tex(nullptr)
