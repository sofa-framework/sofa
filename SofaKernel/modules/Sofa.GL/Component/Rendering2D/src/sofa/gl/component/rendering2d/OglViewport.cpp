...
OglViewport::OglViewport()
    :p_screenPosition(initData( "screenPosition", "Viewport position"))
    ,p_screenSize(initData( "screenSize", "Viewport size"))
    ,p_cameraPosition(initData( Vec3f(0.0,0.0,0.0), "cameraPosition", "Camera's position in eye's space"))
    ,p_cameraOrientation(initData(Quat<SReal>(), "cameraOrientation", "Camera's orientation"))
    ,p_cameraRigid(initData( "cameraRigid", "Camera's rigid coord"))
    ,p_zNear(initData( "zNear", "Camera's ZNear"))
    ,p_zFar(initData( "zFar", "Camera's ZFar"))
    ,p_fovy(initData( (double) 60.0, "fovy", "Field of View (Y axis)"))
    ,p_enabled(initData( true, "enabled", "Enable visibility of the viewport"))
    ,p_advancedRendering(initData( false, "advancedRendering", "If true, viewport will be hidden if advancedRendering visual flag is not enabled"))
    ,p_useFBO(initData( true, "useFBO", "Use a FBO to render the viewport"))
    ,p_swapMainView(initData( false, "swapMainView", "Swap this viewport with the main view"))
    ,p_drawCamera(initData( false, "drawCamera", "Draw a frame representing the camera (see it in main viewport)"))
{
}
