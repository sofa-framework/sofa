...
bool TetrahedronSetGeometryAlgorithms< DataTypes >::isPointInTetrahedron(const TetraID ind_t, const sofa::type::Vec<3,Real>& pTest) const
{
    ...
    SReal v0 = tripleProduct(t1-pTest, t2-pTest, t3-pTest);
    SReal v1 = tripleProduct(pTest-t0, t2-t0, t3-t0);
    SReal v2 = tripleProduct(t1-t0, pTest-t0, t3-t0);
    SReal v3 = tripleProduct(t1-t0, t2-t0, pTest-t0);

    SReal V = tripleProduct(t1-t0, t2-t0, t3-t0);
    if(fabs(V)>ZERO)
...
void TetrahedronSetGeometryAlgorithms<DataTypes>::writeMSHfile(const char *filename) const
{
    ...
    {
        SReal x = (SReal) vect_c[i][0];
        SReal y = (SReal) vect_c[i][1];
        SReal z = (SReal) vect_c[i][2];
