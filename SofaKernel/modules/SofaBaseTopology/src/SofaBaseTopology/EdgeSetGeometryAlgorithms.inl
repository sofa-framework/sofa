...
template<class DataTypes>
bool EdgeSetGeometryAlgorithms<DataTypes>::isPointOnEdge(const sofa::type::Vec<3,SReal> &pt, const EdgeID ind_e) const
{
    const SReal ZERO = 1e-12;

    sofa::type::Vec<3,SReal> p0 = pt;

    Coord vertices[2];
    getEdgeVertexCoordinates(ind_e, vertices);

    sofa::type::Vec<3,SReal> p1; //(vertices[0][0], vertices[0][1], vertices[0][2]);
    sofa::type::Vec<3,SReal> p2; //(vertices[1][0], vertices[1][1], vertices[1][2]);
    DataTypes::get(p1[0], p1[1], p1[2], vertices[0]);
    DataTypes::get(p2[0], p2[1], p2[2], vertices[1]);

    sofa::type::Vec<3,SReal> v = (p0 - p1).cross(p0 - p2);
...
//
template<class DataTypes>
sofa::type::vector< SReal > EdgeSetGeometryAlgorithms<DataTypes>::compute2PointsBarycoefs(
    ...
    PointID ind_p2) const
{
    ...
    sofa::type::Vec<3,SReal> a; DataTypes::get(a[0], a[1], a[2], c0);
    sofa::type::Vec<3,SReal> b; DataTypes::get(b[0], b[1], b[2], c1);
...
void EdgeSetGeometryAlgorithms<DataTypes>::writeMSHfile(const char *filename) const
{
    ...
    {
        SReal x=0,y=0,z=0; DataTypes::get(x,y,z, vect_c[i]);
...
template<class Vec>
sofa::type::vector< SReal > compute_2points_barycoefs(const Vec& p, const Vec& a, const Vec& b)
{
    const SReal ZERO = 1e-6;
...
template<class DataTypes>
sofa::type::vector< SReal > EdgeSetGeometryAlgorithms<DataTypes>::computePointProjectionOnEdge (const EdgeID edgeIndex,
        sofa::type::Vec<3, SReal> c,
        bool& intersected)
{
    ...
    sofa::type::Vec<3,SReal> AB; DataTypes::get(AB[0], AB[1], AB[2], coord_AB);
    sofa::type::Vec<3,SReal> AC; DataTypes::get(AC[0], AC[1], AC[2], coord_AC);
    sofa::type::Vec<3,SReal> ortho_ABC = cross (AB, AC)*1000;
    sofa::type::Vec<3,SReal> coef_CH = cross (ortho_ABC, AB)*1000;
    ...
    sofa::type::Vec<3,SReal> h; DataTypes::get(h[0], h[1], h[2], coord_H);

    sofa::type::vector< SReal > barycoord = compute2PointsBarycoefs(h, theEdge[0], theEdge[1]);
    return barycoord;
...
bool EdgeSetGeometryAlgorithms<DataTypes>::computeEdgePlaneIntersection (EdgeID edgeID, sofa::type::Vec<3,Real> pointOnPlane, sofa::type::Vec<3,Real> normalOfPlane, sofa::type::Vec<3,Real>& intersection)
{
    ...
    SReal d=normalOfPlane*pointOnPlane;
    SReal t=(d-normalOfPlane*p1)/(normalOfPlane*(p2-p1));
        ...
        if ( (vec1[i] > epsilon) || (vec1[i] < -epsilon) )
        {
            ...
            for (unsigned int j = 0; j<Coord::spatial_dimensions; j++)
                if ( (vec2[j] > epsilon || vec2[j] < -epsilon) && (j != i))
                {
                    ...
                    // Solving system:
                    SReal coef_lambda = vec1[ind1] - ( vec1[ind2]*vec2[ind1]/vec2[ind2] );
