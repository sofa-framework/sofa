...
sofa::type::Vec<3, SReal> GridTopology::getPoint(Index i) const
{
...
sofa::type::Vec<3, SReal> GridTopology::getPointInGrid(int i, int j, int k) const
{
    Index id = this->getIndex(i, j, k);
    if (id < seqPoints.getValue().size())
        return seqPoints.getValue()[id];
    else
        return sofa::type::Vec<3, SReal>();
}
