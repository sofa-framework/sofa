...
public:
     ...
     */
    virtual Index findPoint(const Vector3& position) const;
     ...
     * @return The index of the node, or -1 if no such node exists at that position.
     */
    virtual Index findPoint(const Vector3& position, SReal epsilon) const;
    ...
    Vector3 getCubeCoordinate(RegularGridTopology::Index i ) const;
    ...
    virtual Index findCube(const Vector3& pos);
    Index findHexa(const Vector3& pos) { return findCube(pos); }

    /// return the nearest cube (or -1 if not found).
    virtual Index findNearestCube(const Vector3& pos);
    Index findNearestHexa(const Vector3& pos) { return findNearestCube(pos); }

    /// return the cube containing the given point (or -1 if not found),
    /// as well as deplacements from its first corner in terms of dx, dy, dz (i.e. barycentric coordinates).
    virtual Index findCube(const Vector3& pos, SReal& fx, SReal &fy, SReal &fz);
    Index findHexa(const Vector3& pos, SReal& fx, SReal &fy, SReal &fz) { return findCube(pos, fx, fy, fz); }

    /// return the cube containing the given point (or -1 if not found),
    /// as well as deplacements from its first corner in terms of dx, dy, dz (i.e. barycentric coordinates).
    virtual Index findNearestCube(const Vector3& pos, SReal& fx, SReal &fy, SReal &fz);
    Index findNearestHexa(const Vector3& pos, SReal& fx, SReal &fy, SReal &fz) { return findNearestCube(pos, fx, fy, fz); }
