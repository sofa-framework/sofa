...
void HexahedronSetGeometryAlgorithms< DataTypes >::findNearestElements(const VecCoord& pos,
        ...
        helper::vector<type::Vector3>& baryC,
...
template< class DataTypes>
void HexahedronSetGeometryAlgorithms< DataTypes >::findNearestElementsInRestPos( const VecCoord& pos, helper::vector<int>& elem, helper::vector<type::Vector3>& baryC, helper::vector<Real>& dist) const
{
    for(size_t i=0; i<pos.size(); ++i)
    {
...
void HexahedronSetGeometryAlgorithms<DataTypes>::draw(const core::visual::VisualParams* vparams)
{
    ...
    if (d_drawHexahedra.getValue() && this->m_topology->getNbHexahedra() != 0)
    {
        ...
        const VecCoord& coords =(this->object->read(core::ConstVecCoordId::position())->getValue());
        sofa::helper::vector <sofa::type::Vector3> hexaCoords;
