...
struct Multi2Mapping_test : public BaseSimulationTest, NumericTest<typename _MultiMapping::In::Real>
{
    ...
    */
    bool runTest(const vector<In1VecCoord>& in1Coords, const vector<In2VecCoord>& in2Coords, const OutVecCoord& expectedChildCoords)
    {
        ...
        for(Index p=0; p<Np1.size(); p++) {
            WriteIn1VecDeriv fin = in1Dofs[p]->writeForces();
            sofa::testing::copyToData( fin, fIn1p2[p] );  // reset parent forces before accumulating child forces
        }
        for(Index p=0; p<Np2.size(); p++) {
            WriteIn2VecDeriv fin = in2Dofs[p]->writeForces();
            sofa::testing::copyToData( fin, fIn2p2[p] );  // reset parent forces before accumulating child forces
        }
        mapping->applyJT( &mparams, core::VecDerivId::force(), core::VecDerivId::force() );
        for(Index i=0; i<Np1.size(); i++)
        {
            sofa::testing::copyFromData( fIn1p[i], in1Dofs[i]->readForces() );
        ...
        {
            sofa::testing::copyFromData( fIn2p[i], in2Dofs[i]->readForces() );
