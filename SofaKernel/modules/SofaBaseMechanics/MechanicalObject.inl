...
void MechanicalObject<DataTypes>::parse ( sofa::core::objectmodel::BaseObjectDescription* arg )
{
    ...
    if (!arg->getAttribute("size").empty())
    {
    ...
    if (!arg->getAttribute("sx").empty() || !arg->getAttribute("sy").empty() || !arg->getAttribute("sz").empty())
    {
    ...
    if (!arg->getAttribute("rx2").empty() || !arg->getAttribute("ry2").empty() || !arg->getAttribute("rz2").empty())
    {
    ...
    if (!arg->getAttribute("dx2").empty() || !arg->getAttribute("dy2").empty() || !arg->getAttribute("dz2").empty())
    {
