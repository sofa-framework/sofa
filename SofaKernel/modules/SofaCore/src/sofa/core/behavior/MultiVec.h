...
public:
    ...
    TMultiVec( BaseVectorOperations* vop, bool dynamic=true, const std::string& idLabel = {}) : vop(vop), v(MyMultiVecId::null()), dynamic(dynamic)
    {
        static_assert(vtype == V_COORD || vtype == V_DERIV, "");
        vop->v_alloc( v, idLabel);
    }
    ...
    void realloc( BaseVectorOperations* _vop, bool interactionForceField=false, bool propagate=false, const std::string& idLabel = {} )
    {
        vop = _vop;
        vop->v_realloc(v, interactionForceField, propagate, idLabel);
    }
