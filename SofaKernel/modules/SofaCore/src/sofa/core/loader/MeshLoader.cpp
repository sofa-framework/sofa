...
void MeshLoader::updateElements()
{
    if (d_triangulate.getValue())
    {
        ...
        for (size_type i = 0; i < waQuads.size() ; i++)
        {
    ...
    if (d_hexahedra.getValue().size() > 0 && d_createSubelements.getValue())
    {
        ...
        std::set<Quad > eSet;
        for (size_type i = 0; i < quads.size(); ++i)
        {
            eSet.insert(uniqueOrder(quads[i]));
        }
        int nbnew = 0;
        for (size_type i = 0; i < hexahedra.size(); ++i)
        {
    ...
    if (d_pentahedra.getValue().size() > 0 && d_createSubelements.getValue())
    {
        ...
        for (size_type i = 0; i < triangles.size(); ++i)
        {
            eSetTri.insert(uniqueOrder(triangles[i]));
        }
        int nbnewTri = 0;

        for (size_type i = 0; i < pentahedra.size(); ++i)
        {
    ...
    if (d_pyramids.getValue().size() > 0 && d_createSubelements.getValue())
    {
        ...
        for (size_type i = 0; i < pyramids.size(); ++i)
        {
    ...
    if (d_tetrahedra.getValue().size() > 0 && d_createSubelements.getValue())
    {
        ...
        int nbnew = 0;
        for (size_type i = 0; i < tetrahedra.size(); ++i)
        {
    ...
    if (d_onlyAttachedPoints.getValue())
    {
        std::set<Topology::ElemID> attachedPoints;
        {
            ...
            for (size_type i = 0; i < elems.size(); ++i)
                for (size_type j = 0; j < elems[i].size(); ++j)
                {
        ...
        const size_type newsize = size_type(attachedPoints.size());
...
void MeshLoader::updateNormals()
{
    ...
    for (size_type i = 0; i < raTriangles.size() ; i++)
    {
    ...
    for (size_type i = 0; i < raQuads.size() ; i++)
    {
    ...
    for (size_type i = 0; i < waNormals.size(); i++)
    {
...
void MeshLoader::applyTransformation(Matrix4 const& T)
{
    ...
    for (size_type i = 0; i < my_positions.size(); i++)
    {
