...
struct SOFA_HELPER_API NumericTest : public virtual BaseTest
{
    ...
    template< size_type N, typename Real, typename Vector2>
    ...
    template< size_type N, typename Real>
...
//            cout<< c2[i]-c1[i] << " ";
    ...
    /// Return the maximum difference between corresponding entries, or the infinity if the matrices have different sizes
    template<size_type M, size_type N, typename Real, typename Matrix2>
...
template <sofa::size_type L, sofa::size_type C, class real>
void EXPECT_MAT_DOUBLE_EQ(sofa::defaulttype::Mat<L,C,real> const& expected, sofa::defaulttype::Mat<L,C,real> const& actual) {
    ...
    for (sofa::size_type i=0; i<expected.nbLines; ++i)
        for (sofa::size_type j=0; j<expected.nbCols; ++j)
