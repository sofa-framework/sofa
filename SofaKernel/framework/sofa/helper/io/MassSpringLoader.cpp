                     ...
                     sofa::core::objectmodel::Base* context)
{
    ...
    {
        msg_error(context) << "cannot read file '" << filename << "'" ;
        return false;
    }

    dmsg_info(context) << "Loading model '" << filename << "'" ;
    ...
    while (massAndSpringSet != 2 && fscanf(file, cmdScanFormat.str().c_str(), cmd) != EOF )
    {
        if (!strcmp(cmd,"numm"))
        {
            if (fscanf(file, "%d", &totalNumMasses) == EOF){
                msg_error(context) << "fscanf function has encountered an error." ;
                data.setNumMasses(0);
            ...
            msg_warning(context) << "Unable to process Xsp command '"<< cmd << "'" ;
            skipToEOL(file);
        }
    }

    if(massAndSpringSet!=2){
        msg_error(context) << "Unable to load punctual masses from file. "
    ...
    while (fscanf(file, cmdScanFormat.str().c_str(), cmd) != EOF)
    {
        ...
        else if (!strcmp(cmd,"lspg"))	// linear springs connector
        {
            ...
            {
                msg_error(context) << "incorrect mass indexes in spring "<<index<<" "<<m1+1<<" "<<m2+1;
            }
            else
            {
...
#ifndef NDEBUG
                    dmsg_info(context) << "spring "<<m1<<" "<<m2<<" "<<ks<<" "<<kd<<" "<<initpos ;
#endif
                }
