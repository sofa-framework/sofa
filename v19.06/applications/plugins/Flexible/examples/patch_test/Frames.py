...
#	rootNode.createObject('ImageViewer',src="@rasterizer")
	...
	###########################################################
	simNode = rootNode.createChild('Affine')
	...
	simNode.createObject('VoronoiShapeFunction', name="SF", position="@parent.rest_position", src="@../rasterizer", useDijkstra="true", method="0", nbRef="4")
	...
	childNode.createObject('LinearMapping', template="Affine,Vec3d")
	...
	childNode.createObject('LinearMapping', template="Affine,F331", showDeformationGradientScale="1")

	simNode.createObject('PythonScriptController',filename="Frames.py", classname="Controller")


	###########################################################
	simNode = rootNode.createChild('Affine Laplace')

	simNode.createObject('MechanicalObject', template="Affine", name="parent", showObject="1", rest_position=concatAffineinString(restpos,restAffinepos), position=concatAffineinString(pos,Affinepos))
	simNode.createObject('VoronoiShapeFunction', name="SF", position="@parent.rest_position", src="@../rasterizer", useDijkstra="true", method="1", nbRef="4")
	...
	###########################################################
	simNode = rootNode.createChild('Affine Sibson')

	simNode.createObject('MechanicalObject', template="Affine", name="parent", showObject="1", rest_position=concatAffineinString(restpos,restAffinepos), position=concatAffineinString(pos,Affinepos))
	simNode.createObject('VoronoiShapeFunction', name="SF", position="@parent.rest_position", src="@../rasterizer", useDijkstra="true", method="2", nbRef="4")
