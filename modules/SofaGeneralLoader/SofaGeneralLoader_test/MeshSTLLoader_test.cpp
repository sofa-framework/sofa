...
protected:
  ...
  struct MeshSTLTestData
  {
      ...
      void testBench()
      {
          ...
          const sofa::type::vector< type::vector < type::vector <int> > > & facets = mesh.getFacets();
          unsigned int fileNbLines, fileNbTriangles, fileNbQuads;
          fileNbLines = fileNbTriangles = fileNbQuads = 0;
          for (unsigned int i = 0; i < facets.size(); i++)
          {
              const type::vector<type::vector<int> >& vtnFacet = facets[i];
              if (vtnFacet[0].size() > 0)
              {
                  const type::vector<int>& facet = vtnFacet[0];
                  switch (facet.size())
                  {
                  case 2:
                      fileNbLines++;
