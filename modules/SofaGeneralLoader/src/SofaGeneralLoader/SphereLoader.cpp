...
SphereLoader::SphereLoader()
    :BaseLoader(),
     ...
     d_scale(initData(&d_scale, type::Vec<3, SReal>(1.0, 1.0, 1.0), "scale","Scale applied to sphere positions & radius")),
     d_rotation(initData(&d_rotation, type::Vec<3, SReal>(), "rotation", "Rotation of the DOFs")),
     d_translation(initData(&d_translation, type::Vec<3, SReal>(),"translation","Translation applied to sphere positions"))
{
...
void SphereLoader::applyTransform()
{
    ...
    if (scale != type::Vec<3, SReal>(1.0, 1.0, 1.0) || rotation != type::Vec<3, SReal>(0.0, 0.0, 0.0) || translation != type::Vec<3, SReal>(0.0, 0.0, 0.0))
    {
        if(scale != type::Vec<3, SReal>(1.0, 1.0, 1.0)) {
