...
{
	setInputTransformation(TRANSLATION, NULL_VEC, NULL_SCALE);
	testOutput(OUTPUT_TRANSLATION_POS);
}

// test rotation
TYPED_TEST( TransformEngine_test , rotation )
{
    setInputTransformation(NULL_VEC, ROTATION, NULL_SCALE);
	testOutput(OUTPUT_ROTATION_POS, OUTPUT_ROTATION_QUAT);
}

// test scale
TYPED_TEST( TransformEngine_test , scale )
{
    setInputTransformation(NULL_VEC, NULL_VEC, SCALE);
	testOutput(OUTPUT_SCALE_POS);
}

// test translation-rotation composite
TYPED_TEST( TransformEngine_test , translationRotation )
{
    setInputTransformation(TRANSLATION, ROTATION, NULL_SCALE);
	testOutput(OUTPUT_ROTATION_POS + TRANSLATION, OUTPUT_ROTATION_QUAT);
}

// test translation-scale composite
TYPED_TEST( TransformEngine_test , translationScale )
{
    setInputTransformation( TRANSLATION, NULL_VEC, SCALE );
	testOutput(OUTPUT_SCALE_POS + TRANSLATION);
}

// test rotation-scale composite
TYPED_TEST( TransformEngine_test , rotationScale )
{
    setInputTransformation( NULL_VEC, ROTATION, SCALE );
	testOutput(OUTPUT_ROTATION_SCALE_POS, OUTPUT_ROTATION_QUAT);
}

// test translation-rotation-scale composite
TYPED_TEST( TransformEngine_test , translationRotationScale )
{
    setInputTransformation( TRANSLATION, ROTATION, SCALE );
	testOutput(OUTPUT_ROTATION_SCALE_POS + TRANSLATION, OUTPUT_ROTATION_QUAT);
}
