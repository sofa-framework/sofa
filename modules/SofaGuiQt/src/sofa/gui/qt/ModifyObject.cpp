...
#if SOFAGUIQT_HAVE_QT5_CHARTS
        //Energy Widget
        if (simulation::Node* real_node = sofa::simulation::node::getFrom(node))
        {
...
void ModifyObject::updateValues()
{
    ...
    if (node)
    {
        bool isNode =( sofa::simulation::node::getFrom(node) != nullptr);
        ...
        {
            simulation::Node* current_node = sofa::simulation::node::getFrom(node);
        ...
        if (dialogFlags_.REINIT_FLAG)
        {
            ...
            else if (simulation::Node *n = sofa::simulation::node::getFrom(node)) n->reinit(sofa::core::ExecParams::defaultInstance());
        }
