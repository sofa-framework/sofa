...
bool StateTester<DataTypes>::wantsToSleep(core::behavior::BaseMechanicalState* baseState, SReal speedThreshold, SReal /*rotationThreshold*/)
{
	...
	SReal maxVelocity = 0;
	for (unsigned int i = 0, nb = (unsigned int)vecVel.size(); i < nb; ++i)
...
void SleepController::init()
{
	...
	for (unsigned int i = 0, nbStates = (unsigned int)tempStates.size(); i < nbStates; ++i)
	{
		core::behavior::BaseMechanicalState* state = tempStates[i];
		bool found = false;
		for (unsigned int j = 0, nbTesters = (unsigned int)m_stateTesters.size(); j < nbTesters; ++j)
		{
	...
	{
		for (unsigned int i = 0, nbContexts = (unsigned int)m_contextsThatCanSleep.size(); i < nbContexts; ++i)
...
void SleepController::reset()
{
	...
	for (unsigned int i = 0, nb = (unsigned int)m_timeSinceWakeUp.size(); i < nb; ++i)
...
void SleepController::putNodesToSleep()
{
	...
	for (unsigned int i = 0, nbStates = (unsigned int)m_statesThatCanSleep.size(); i < nbStates; ++i)
	{
...
/*	if (f_printLog.getValue())
	...
	{
		changed = false;
		unsigned int nbContexts = (unsigned int)m_contextsThatCanSleep.size();
		for (unsigned int i = 0; i < nbContexts; ++i)
		{
			...
			for (unsigned int j = 0, nbLinks = (unsigned int)wakeupPair.size(); j < nbLinks; ++j)
			{
...
void SleepController::collectWakeupPairs(std::vector<BaseContexts>& wakeupPairs)
{
	...
	for (unsigned int i = 0, nbContacts = (unsigned int)contacts.size(); i < nbContacts; ++i)
	{
...
void SleepController::addWakeupPair(std::vector<BaseContexts>& wakeupPairs, core::objectmodel::BaseContext* context1, bool moving1, core::objectmodel::BaseContext* context2, bool moving2)
{
	...
	if (moving2)
	{
		...
		if (iter != contextsEnd)
		{
			int index = (int)(iter - contextsBegin);
