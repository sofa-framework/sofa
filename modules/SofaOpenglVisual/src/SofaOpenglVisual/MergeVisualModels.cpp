...
MergeVisualModels::MergeVisualModels()
...
void MergeVisualModels::parse ( sofa::core::objectmodel::BaseObjectDescription* arg )
{
    vl_input.parseSizeLinks(arg, d_nbInput);
    Inherit1::parse(arg);
}

void MergeVisualModels::parseFields ( const std::map<std::string,std::string*>& str )
{
    vl_input.parseFieldsSizeLinks(str, d_nbInput);
    Inherit1::parseFields(str);
}

void MergeVisualModels::init()
{
    vl_input.resize(d_nbInput.getValue());
    Inherit1::init();
    update();
