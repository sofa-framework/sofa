...
Monitor<DataTypes>::Monitor()
    : d_indices ( initData( "indices", "MechanicalObject points indices to monitor" ) )
    , d_saveXToGnuplot ( initData( false, "ExportPositions", "export Monitored positions as gnuplot file" ) )
    , d_saveVToGnuplot ( initData( false, "ExportVelocities", "export Monitored velocities as gnuplot file" ) )
    , d_saveFToGnuplot ( initData( false, "ExportForces", "export Monitored forces as gnuplot file" ) )
    ,d_showPositions (initData( false, "showPositions", "see the Monitored positions"))
    ,d_positionsColor (initData( "PositionsColor", "define the color of positions"))
    ,d_showVelocities (initData( false, "showVelocities", "see the Monitored velocities"))
    ,d_velocitiesColor(initData( "VelocitiesColor", "define the color of velocities"))
    ,d_showForces (initData( false, "showForces", "see the Monitored forces"))
    ,d_forcesColor (initData( "ForcesColor", "define the color of forces"))
    ,d_showMinThreshold (initData( 0.01 ,"showMinThreshold", "under this value, vectors are not represented"))
    ,d_showTrajectories (initData( false ,"showTrajectories", "print the trajectory of Monitored particles"))
    ,d_trajectoriesPrecision (initData( 0.1,"TrajectoriesPrecision", "set the dt between to save of positions"))
    ,d_trajectoriesColor(initData( "TrajectoriesColor", "define the color of the trajectories"))
    ,d_showSizeFactor(initData( 1.0, "sizeFactor", "factor to multiply to arrows"))
    ,d_fileName(initData( "fileName", "name of the plot files to be generated"))
