...
EvalPointsDistance<DataTypes>::EvalPointsDistance()
    : f_draw( initData( true, "draw", "activate rendering of lines between associated points"))
    , isToPrint( initData( false, "isToPrint", "suppress somes data before using save as function"))
    , f_filename( initData( "filename", "output file name"))
    , f_period( initData( 0.0, "period", "period between outputs"))
    , dist( initData( "distance", "distances (OUTPUT)"))
    , distMean( initData( 1.0, "distMean", "mean distance (OUTPUT)"))
    , distMin( initData( 1.0, "distMin", "min distance (OUTPUT)"))
    , distMax( initData( 1.0, "distMax", "max distance (OUTPUT)"))
    , distDev( initData( 1.0, "distDev", "distance standard deviation (OUTPUT)"))
    , rdistMean( initData( 1.0, "rdistMean", "mean relative distance (OUTPUT)"))
    , rdistMin( initData( 1.0, "rdistMin", "min relative distance (OUTPUT)"))
    , rdistMax( initData( 1.0, "rdistMax", "max relative distance (OUTPUT)"))
    , rdistDev( initData( 1.0, "rdistDev", "relative distance standard deviation (OUTPUT)"))
