...
void LinearMovementConstraint<DataTypes>::interpolatePosition(Real cT, typename boost::disable_if<std::is_same<MyCoord, defaulttype::RigidCoord<3, Real> >, VecCoord>::type& x)
{
    const SetIndexArray & indices = m_indices.getValue();
...
template <class DataTypes>
template <class MyCoord>
void LinearMovementConstraint<DataTypes>::interpolatePosition(Real cT, typename boost::enable_if<std::is_same<MyCoord, defaulttype::RigidCoord<3, Real> >, VecCoord>::type& x)
{
    const SetIndexArray & indices = m_indices.getValue();
