...
******************************************************************************/
#include <OpenglVisual/OglVariable.inl>
...
void OglIntVectorVariable::initVisual()
{
    ...
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setIntVector(idShader, idstr.c_str(), (int)v.size(), vptr);
}
...
void OglIntVector2Variable::initVisual()
{
    ...
    const GLint* vptr = v.empty() ? NULL : &(v[0]);
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setIntVector2(idShader, idstr.c_str(), (int)v.size()/2, vptr);
}
...
void OglIntVector3Variable::initVisual()
{
    ...
    const GLint* vptr = v.empty() ? NULL : &(v[0]);
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setIntVector3(idShader, idstr.c_str(), (int)v.size()/3, vptr);
}
...
void OglIntVector4Variable::initVisual()
{
    ...
    const GLint* vptr = v.empty() ? NULL : &(v[0]);
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setIntVector4(idShader, idstr.c_str(), (int)v.size()/4, vptr);
}
...
void OglFloatVectorVariable::initVisual()
{
    ...
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setFloatVector(idShader, idstr.c_str(), (int)v.size(), vptr);
}
...
void OglFloatVector2Variable::initVisual()
{
    ...
    const float* vptr = v.empty() ? NULL : &(v[0]);
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setFloatVector2(idShader, idstr.c_str(), (int)v.size()/2, vptr);
}
...
void OglFloatVector3Variable::initVisual()
{
    ...
    const float* vptr = v.empty() ? NULL : &(v[0]);
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setFloatVector3(idShader, idstr.c_str(), (int)v.size()/3, vptr);
}
...
void OglFloatVector4Variable::initVisual()
{
    ...
    const float* vptr = v.empty() ? NULL : &(v[0]);
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setFloatVector4(idShader, idstr.c_str(), (int)v.size()/4, vptr);
}
...
void OglMatrix2Variable::initVisual()
{
    ...
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setMatrix2(idShader, idstr.c_str(), (int)v.size()/4, transp, vptr);
}
...
void OglMatrix3Variable::initVisual()
{
    ...
    const bool transp = transpose.getValue();
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setMatrix3(idShader, idstr.c_str(), (int)v.size()/9, transp, vptr);
}
...
void OglMatrix4Variable::initVisual()
{
    ...
    const bool transp = transpose.getValue();
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setMatrix4(idShader, idstr.c_str(), (int)v.size()/16, transp, vptr);
}
...
void OglMatrix2x3Variable::initVisual()
{
    ...
    const bool transp = transpose.getValue();
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setMatrix2x3(idShader, idstr.c_str(), (int)v.size()/6, transp, vptr);
}
...
void OglMatrix3x2Variable::initVisual()
{
    ...
    const bool transp = transpose.getValue();
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setMatrix3x2(idShader, idstr.c_str(), (int)v.size()/6, transp, vptr);
}
...
void OglMatrix2x4Variable::initVisual()
{
    ...
    const bool transp = transpose.getValue();
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setMatrix2x4(idShader, idstr.c_str(), (int)v.size()/8, transp, vptr);
}
...
void OglMatrix4x2Variable::initVisual()
{
    ...
    const bool transp = transpose.getValue();
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setMatrix4x2(idShader, idstr.c_str(), (int)v.size()/8, transp, vptr);
}
...
void OglMatrix3x4Variable::initVisual()
{
    ...
    const bool transp = transpose.getValue();
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setMatrix3x4(idShader, idstr.c_str(), (int)v.size()/12, transp, vptr);
}
...
void OglMatrix4x3Variable::initVisual()
{
    ...
    const bool transp = transpose.getValue();
    for(std::set<OglShader*>::iterator it = shaders.begin(), iend = shaders.end(); it!=iend; it++)
        (*it)->setMatrix4x3(idShader, idstr.c_str(), (int)v.size()/12, transp, vptr);
}
