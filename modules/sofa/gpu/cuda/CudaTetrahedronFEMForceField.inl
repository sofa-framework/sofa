...
template <>
void TetrahedronFEMForceField<CudaVec3f1Types>::reinit()
{
    _strainDisplacements.resize( _indexedElements->size() );
    _materialsStiffnesses.resize(_indexedElements->size() );
...
void TetrahedronFEMForceField<gpu::cuda::CudaVec3f1Types>::addForce (VecDeriv& f, const VecCoord& x, const VecDeriv& v)
{
    ...
    f.resize(x.size());
    TetrahedronFEMForceFieldCuda3f1_addForce(
        data.size(),
        data.nbVertex,
        data.nbElementPerVertex,
        data.elems.deviceRead(),
    ...
    for (unsigned int i=0; i<elems.size(); i++)
    {
        ...
        Transformation Rt;
        computeRotationLarge(Rt, x, a, b, c);
        const TetrahedronFEMForceFieldInternalData<gpu::cuda::CudaVec3f1Types>::GPUElementState& s = data.state[i];
        const TetrahedronFEMForceFieldInternalData<gpu::cuda::CudaVec3f1Types>::GPUElement& e = data.elems[i];
        Mat3x3f Rdiff = Rt-s.Rt;
        if ((Rdiff[0].norm2()+Rdiff[1].norm2()+Rdiff[2].norm2()) > 0.000001f)
        {
            std::cout << "CPU Rt "<<i<<" = "<<Rt<<std::endl;
...
{
    df.resize(dx.size());
    TetrahedronFEMForceFieldCuda3f1_addDForce(
        data.size(),
        data.nbVertex,
        data.nbElementPerVertex,
        data.elems.deviceRead(),
