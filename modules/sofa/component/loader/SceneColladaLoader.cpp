...
******************************************************************************/
#include <sofa/component/loader/SceneColladaLoader.h>
...
bool SceneColladaLoader::readDAE (std::ifstream &file, const char* filename)
{
    ...
    std::cout << "SceneColladaLoader::readDAE" << std::endl;
    ...
    if(!currentAiScene)
    {
        std::cout << "Collada import failed : " << importer.GetErrorString() << sendl;
        return false;
    }
    ...
    if(currentAiScene->mRootNode)
    {
        ...
        std::cout << "//////////////" << std::endl;
        std::cout << std::string(currentAiScene->mRootNode->mName.data, currentAiScene->mRootNode->mName.length) << std::endl;
        std::cout << "//////////////" << std::endl;
            ...
            if(0 == childIndex)
            {
                ...
                for(unsigned int j = 0; j < currentAiNode->mNumMeshes; ++j)
                {
                    ...
                    MechanicalObject<Rigid3fTypes>::SPtr currentMechanicalObject = sofa::core::objectmodel::New<MechanicalObject<Rigid3fTypes> >();
                    {
                        ...
                        meshGNode->addObject(currentMechanicalObject);
...
// 					}
                    ...
                    OglModel::SPtr currentOglModel = sofa::core::objectmodel::New<OglModel>();
                    {
                        ...
                        meshGNode->addObject(currentOglModel);
...
// 						vertex / triangle / quad array are automatically filled up with the MeshTopology data
                    ...
                    if(currentAiMesh->HasBones())
                    {
                        ...
                        MechanicalObject<Rigid3fTypes>::SPtr currentSkinningMechanicalObject = sofa::core::objectmodel::New<MechanicalObject<Rigid3fTypes> >();
                        {
                            // adding the generated MechanicalObject to its parent GNode
                            meshGNode->addObject(currentSkinningMechanicalObject);

                            std::stringstream nameStream(meshName);
                            if(meshName.empty())
                                nameStream << componentIndex++;
                            currentSkinningMechanicalObject->setName(nameStream.str());

                            // filling up position coordinate array
                            currentSkinningMechanicalObject->resize(currentAiMesh->mNumBones);

                            {
                                Data<vector<Rigid3fTypes::Coord> >* d_x = currentSkinningMechanicalObject->write(core::VecCoordId::position());
                                vector<Rigid3fTypes::Coord> &x = *d_x->beginEdit();
                        ...
                        SkinningMapping<Rigid3fTypes, ExtVec3fTypes>::SPtr currentSkinningMapping = sofa::core::objectmodel::New<SkinningMapping<Rigid3fTypes, ExtVec3fTypes> >();
                        {
                            ...
                            meshGNode->addObject(currentSkinningMapping);
                            ...
                            currentSkinningMapping->setModels(currentSkinningMechanicalObject.get(), currentOglModel.get());
                            ...
                            for(unsigned int k = 0; k < currentAiMesh->mNumBones; ++k)
                            {
                                ...
                                for(unsigned int l = 0; l < bone->mNumWeights; ++l)
                                {
                                    ...
                                    {
                                        std::cout << "Error: SceneColladaLoader::readDAE, a mesh could not be load : " << nameStream.str() << " - in node : " << currentGNode->getName() << std::endl;
                                        return false;
                                    }
                    ...
                    else
                    {
                        IdentityMapping<Rigid3fTypes, ExtVec3fTypes>::SPtr currentIdentityMapping = sofa::core::objectmodel::New<IdentityMapping<Rigid3fTypes, ExtVec3fTypes> >();
                        {
                            ...
                            meshGNode->addObject(currentIdentityMapping);
...
bool SceneColladaLoader::fillSkeletalInfo(const aiScene* scene, aiNode* meshParentNode, aiNode* meshNode, aiMatrix4x4 meshTransformation, aiMesh* mesh, std::vector<SkeletonJoint<Rigid3fTypes> >& skeletonJoints, std::vector<SkeletonBone>& skeletonBones) const
{
    ...
    for(unsigned int j = 0; j < scene->mNumAnimations; ++j)
    {
        ...
        for(unsigned int k = 0; k < animation->mNumChannels; ++k)
        {
            ...
            for(int l = 0; l < numKey; ++l)
            {
                ...
                // 							{
                // 								std::cout << l << std::endl;
                // 								std::cout << transformation.a1 << " - " << transformation.a2 << " - " << transformation.a3 << " - " << transformation.a4 << std::endl;
                // 								std::cout << transformation.b1 << " - " << transformation.b2 << " - " << transformation.b3 << " - " << transformation.b4 << std::endl;
                // 								std::cout << transformation.c1 << " - " << transformation.c2 << " - " << transformation.c3 << " - " << transformation.c4 << std::endl;
                // 								std::cout << transformation.d1 << " - " << transformation.d2 << " - " << transformation.d3 << " - " << transformation.d4 << std::endl;
                // 								std::cout << std::endl;
                // 							}
            }

            //std::cout << "registering SkeletonJoint : " << std::string(nodeName.data, nodeName.length) << std::endl;
        }
    }
    ...
    for(unsigned int i = 0; i < mesh->mNumBones; ++i)
    {
        ...
        //std::cout << "registering bone : " << std::string(boneName.data, boneName.length) << std::endl;
    }
    ...
    std::cout << "fillSkeletalInfo : end" << std::endl;
