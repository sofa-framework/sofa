...
int CapsuleIntTool::computeIntersection(Capsule & cap, TSphere<DataTypes> & sph,double alarmDist,double contactDist,OutputVector* contacts){
    ...
    SReal theory_contactDist = cap_rad + sph_rad + contactDist;
    SReal contact_exists = cap_rad + sph_rad + alarmDist;
    SReal alpha = (AB * AC)/AB.norm2();//projection of the sphere center on the capsule segment
