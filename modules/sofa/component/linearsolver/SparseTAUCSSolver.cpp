...
void SparseTAUCSSolver<TMatrix,TVector>::invert(Matrix& M)
{
    ...
    matrix_taucs.n = M.rowSize();
    matrix_taucs.m = M.colSize();
    matrix_taucs.flags = get_taucs_flags<Real>();
    matrix_taucs.colptr = (int *) &(M.getRowBegin()[0]);
    matrix_taucs.rowind = (int *) &(M.getColsIndex()[0]);
    matrix_taucs.values.v = (void*) &(M.getColsValue()[0]);
    ...
    opts.push_back(NULL);
    if (taucs_linsolve(&matrix_taucs, &factorization, 0, NULL, NULL, &(opts[0]), NULL) != TAUCS_SUCCESS)
    {
        serr << "TAUCS factorization failed." << sendl;
    }
...
void SparseTAUCSSolver<TMatrix,TVector>::solve (Matrix& /*M*/, Vector& z, Vector& r)
{
    ...
    for (unsigned int i=0; i<options.size(); ++i)
        opts.push_back((char*)options[i].c_str());
    opts.push_back(NULL);
    if (taucs_linsolve(&matrix_taucs, &factorization, 1, z.ptr(), r.ptr(), &(opts[0]), NULL) != TAUCS_SUCCESS)
    {
        serr << "TAUCS solve failed." << sendl;
    }
}
