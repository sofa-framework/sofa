...
//    }
    ...
    /** Create a block in the current row, which must be previously initialized using beginBlockRow(unsigned row).
        ...
        If the block already exists, they will be added.
        */
...
public:
    ...
    void multVector( OutVecDeriv& output, const InVecDeriv& input ){
        Eigen::Map<VectorEigenOut> mo(&output[0],output.size());
        Eigen::Map<const VectorEigenIn> mi(&input[0],input.size());
        compress();
...
//                        cerr<<"copyFrom,  insert at " << r + blRow*Nout << ", " << c + blCol*Nin << endl;
    ...
    void mult( OutVecDeriv& result, const InVecDeriv& data ) const
    {
        // use optimized product if possible
        if(canCast(data)){
            const Eigen::Map<VectorEigenOut> d(const_cast<OutReal*>(&data[0][0]),data.size()*Nin);
            Eigen::Map<VectorEigenOut> r(&result[0][0],result.size()*Nout);
...
//    }
    ...
    void addMultTranspose( InVecDeriv& result, const OutVecDeriv& data ) const
    {
        // use optimized product if possible
        if(canCast(result)){
            const Eigen::Map<VectorEigenOut> d(const_cast<OutReal*>(&data[0][0]),data.size()*Nout);
            Eigen::Map<VectorEigenOut> r(&result[0][0],result.size()*Nin);
