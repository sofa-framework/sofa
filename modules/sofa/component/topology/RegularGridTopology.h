...
public:
    ...
    int findHexa(const Vector3& pos, Real_Sofa& fx, Real_Sofa &fy, Real_Sofa &fz) { return findCube(pos, fx, fy, fz); }

    /// return the cube containing the given point (or -1 if not found),
    /// as well as deplacements from its first corner in terms of dx, dy, dz (i.e. barycentric coordinates).
    virtual int findNearestCube(const Vector3& pos, Real_Sofa& fx, Real_Sofa &fy, Real_Sofa &fz);
    int findNearestHexa(const Vector3& pos, Real_Sofa& fx, Real_Sofa &fy, Real_Sofa &fz) { return findNearestCube(pos, fx, fy, fz); }
