        ...
        sofa::helper::vector<unsigned int > &closest_vertices)
{
    ...
    unsigned int ind_ta_test;
    unsigned int ind_tb_test;
    ...
    if(is_first_cut)
    {
        ...
        if(is_a_inside)
        {
            this->sout << "a is inside" <<  this->sendl;
        }
        else
        {
            this->sout << "a is NOT inside !!!" <<  this->sendl;
            if(ind_ta_new == ind_ta_test) // fail
            {
                //this->sout << "fail !!!" <<  this->sendl;
                return false;
            }
            else
            {
    ...
    if (ind_tb != (unsigned)-1)
    {
        ...
        if(is_b_inside)
        {
            this->sout << "b is inside" <<  this->sendl;
        }
        else
        {
            this->sout << "b is NOT inside !!!" <<  this->sendl;
    ...
    if (ind_tb == (unsigned)-1)
    {
        this->sout << "Final triangle is " << ind_tb_final <<" : " << m_container->getTriangle(ind_tb_final) << this->sendl;
        ind_tb = ind_tb_final;
    }
    ...
    if(elem_size>0) // intersection successfull
    {
        ...
        Edge e = m_container->getEdge(edges_list[0]);
        unsigned int p1_a = e[0];
        unsigned int p2_a = e[1];
        ...
        //this->sout << "a_baryCoefs = " << a_baryCoefs[0] << ", " << a_baryCoefs[1] << ", " << a_baryCoefs[2] <<  this->sendl;
        ...
        for (unsigned int i=0; i<edges_list.size(); i++)
        {
            ...
            new_points.push_back(acc_nb_points-1);
            new_points.push_back(acc_nb_points);
            closest_vertices.push_back(e[0]);
            closest_vertices.push_back(e[1]);
            ...
            if(i>0) // not to treat particular case of first extremitiy
            {
                ...
                if(is_snapping_p && i<edges_list.size()-1) // not to treat particular case of last extremitiy
                {
                    ...
                    // this->sout << "INFO_print : DO is_snapping_p, i = " << i << " on vertex " << ind_p <<  this->sendl;
                ...
                Vec<3,Real> point_created=(Vec<3,double>) m_geometryAlgorithms->computeBaryEdgePoint(m_container->getEdge(edges_list[i-1]), coords_list[i-1]);
                Vec<3,Real> point_to_create=(Vec<3,double>) m_geometryAlgorithms->computeBaryEdgePoint(m_container->getEdge(edges_list[i]), coords_list[i]);
        ...
        if(is_intersected || !is_on_boundary)
        {
            ...
            Edge e = m_container->getEdge(edges_list[edges_list.size()-1]);
            unsigned int p1_b=e[0];
            unsigned int p2_b=e[1];

            b_i123_last.clear();
            b_i123_last.push_back(p1_b); // OUTPUT
            b_i123_last.push_back(p2_b); // OUTPUT
            ...
            //this->sout << "b_baryCoefs = " << b_baryCoefs[0] << ", " << b_baryCoefs[1] << ", " << b_baryCoefs[2] <<  this->sendl;
        ...
        //this->sout<<"INFO, number to create = "<< triangles_to_create.size() <<this->sendl;
        ...
        //this->sout<<"INFO, number to remove = "<< triangles_to_remove.size() <<this->sendl;
        ...
        const sofa::helper::vector<double>& coords_list, sofa::helper::vector<EdgeID>& new_edges)
{
    ...
    //// STEP 1c : Create start point if necessary
