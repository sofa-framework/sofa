...
\begin{center}
 \includegraphics[width=0.5\linewidth]{slides-fig-ftopdown}
...
\begin{frame}
 \frametitle{Example of action (continued)}
...
\end{itemize}
\begin{center}
 \includegraphics[width=0.5\linewidth]{slides-fig-fbottomup}
\end{center}
...
\end{itemize}
\begin{center}
 \includegraphics[width=0.5\linewidth]{slides-fig-accFromF}
\end{center}
...
\begin{frame}[fragile]
 \frametitle{Using state vectors}
\begin{itemize}
 \item Reserved indices are used to denote x,v,f,dx
 \item Auxiliary vectors can be used, e.g. in the beginning of Runge-Kutta2:
\end{itemize}
\begin{code_cpp}
...
At each time step:
\begin{enumerate}
 \item Collisions are modeled by springs using a CollisionAction
...
Weaknesses:
\begin{itemize}
 \item Documentation is not complete 
 \item Main loop is not yet customizable 
 \item Constraint-based methods are difficult to implement
 \item Still a lot to do: friction, haptics, faster collision detection...
\end{itemize}
Future work:
\begin{itemize}
 \item get rid of weaknesses
 \item parallelisation (GPU, clusters)
 \item easy debugging
\end{itemize}
