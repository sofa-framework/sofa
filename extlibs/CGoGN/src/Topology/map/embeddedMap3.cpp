...
Dart EmbeddedMap3::deleteEdge(Dart d)
{
    ...
    if (isOrbitEmbedded<EDGE>()) {
        const unsigned int edgeID = getEmbedding<EDGE>(d) ;
...
void EmbeddedMap3::deleteVolume(Dart d, bool withBoundary, std::vector<Dart> *removedVertices) {
    std::cerr << "Calling EmbeddedMap3::deleteVolume..." << std::endl;
    ...
    for (Dart dv = traV.begin() ; dv != traV.end() ; dv = traV.next() ) {
        ...
        unsigned int i = 0u;
        for (Dart dW = traW.begin() ; (dW != traW.end()) && ( i<= 1u ) ; dW = traW.next() )
            ++i;
        if ( i == 1u ) {
            const unsigned int vertexIdx = getEmbedding<VERTEX>(dv) ;
            verticesToRemove.push_back(vertexIdx);
            if ( removedVertices != NULL )
                removedVertices->push_back(dv);
        }
    }
...
{
    std::cerr << "nb vertex orbits : " << getNbOrbits<VERTEX>() << std::endl ;
    std::cerr << "nb vertex cells : " << m_attribs[VERTEX].size() << std::endl ;

    std::cerr << "nb edge orbits : " << getNbOrbits<EDGE>() << std::endl ;
    std::cerr << "nb edge cells : " << m_attribs[EDGE].size() << std::endl ;

    std::cerr << "nb face orbits : " << getNbOrbits<FACE>() << std::endl ;
    std::cerr << "nb face cells : " << m_attribs[FACE].size() << std::endl ;

    std::cerr << "nb volume orbits : " << getNbOrbits<VOLUME>() << std::endl ;
