...
template <typename MAP, unsigned int ORBIT>
TraversorCell<MAP, ORBIT>::TraversorCell(const TraversorCell& tracell)
    : m(tracell.m)
    , dmark(NULL)
    , cmark(NULL)
    , quickTraversal(tracell.quickTraversal)
    , current(tracell.current)
    , firstTraversal(true)
{
    std::cerr << "TraversorCell copy constructor (should not happen)..." << std::endl;
    if(quickTraversal != NULL){
        cont = &(m.template getAttributeContainer<ORBIT>()) ;
    } else {
        if(tracell.cmark != NULL)
            cmark = new CellMarker<ORBIT>(m, tracell.cmark->getThread()) ;
        else
            dmark = new DartMarker(m, tracell.dmark->getThread()) ;
    }
}
...
template <unsigned int ORBIT>
TraversorCell<GenericMap, ORBIT>::TraversorCell(const TraversorCell& tracell)
    : m(tracell.m)
    , dmark(NULL)
    , cmark(NULL)
    , quickTraversal(tracell.quickTraversal)
    ...
    , firstTraversal(true)
{
    std::cerr << "TraversorCell specialized copy constructor (should not happen)..." << std::endl;
    if(quickTraversal != NULL){
        cont = &(m.template getAttributeContainer<ORBIT>()) ;
    } else {
        if(tracell.cmark != NULL)
