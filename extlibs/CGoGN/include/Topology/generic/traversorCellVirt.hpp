...
template <unsigned int ORBIT>
VTraversorCell<GenericMap, ORBIT>::~VTraversorCell()
{
	if(dmark)
		delete dmark ;
	else if(cmark)
...
template <unsigned int ORBIT>
Dart VTraversorCell<GenericMap, ORBIT>::begin()
{
	if(quickTraversal != NULL)
	{
		qCurrent = cont->begin() ;
...
template <unsigned int ORBIT>
Dart VTraversorCell<GenericMap, ORBIT>::end()
{
	return NIL ;
}

template <unsigned int ORBIT>
Dart VTraversorCell<GenericMap, ORBIT>::next()
{
	if(current != NIL)
	{
		if(quickTraversal != NULL)
...
template <unsigned int ORBIT>
void VTraversorCell<GenericMap, ORBIT>::skip(Dart d)
{
	if(dmark)
		dmark->markOrbit<ORBIT>(d) ;
	else
