...
//                foreach_adjacent2<EDGE>(map, f, [&] (Face ff)
        AdjacentTrav2<typename PFP::MAP, FACE,EDGE> trav(map,f);
        for (Face c = trav.t.begin(), e = trav.t.end(); c != e; c = trav.t.next())
        {
            if(!mark.isMarked(c)) // not already marked
            {
                visitedFaces.push_back(c) ;
                mark.markOrbit(c) ;
            }
        }
    }
