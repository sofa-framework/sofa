...
class Plane3D
{
	public:
        ...
        Plane3D(const sofa::defaulttype::Vec<3,T>& n, T d);

		// construct the plane with normal vector n and going through p
        Plane3D(const sofa::defaulttype::Vec<3,T>& n, const sofa::defaulttype::Vec<3,T>& p);

		// construct the plane going through p1, p2 and p3
        Plane3D(const sofa::defaulttype::Vec<3,T>& p1, const sofa::defaulttype::Vec<3,T>& p2, const sofa::defaulttype::Vec<3,T>& p3);

		/**********************************************/
		/*                 ACCESSORS                  */
		/**********************************************/

        sofa::defaulttype::Vec<3,T>& normal();

        const sofa::defaulttype::Vec<3,T>& normal() const;
        ...
        T distance(const sofa::defaulttype::Vec<3,T>& p) const;

		// project the point p onto the plane
        void project(sofa::defaulttype::Vec<3,T>& p) const;

		// return on/over/under according to the side of the plane where point p is
        Orientation3D orient(const sofa::defaulttype::Vec<3,T>& p) const;

		// return on/over/under according to the side of the plane where point p is
		// return the distance to the point in dist reference
        Orientation3D orient(const sofa::defaulttype::Vec<3,T>& p, T& dist) const;
	...
	private:
        sofa::defaulttype::Vec<3,T> m_normal ;
		T m_d ;
} ;
