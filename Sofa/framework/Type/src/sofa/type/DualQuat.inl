...
real DualQuatCoord3<real>::norm2() const
{
    real r = (real)0.;
    for (int i = 0; i < 4; i++)
        r += orientation[i] * orientation[i] + dual[i] * dual[i];
    return r;
}
...
void DualQuatCoord3<real>::velocity_getJ(type::Mat<4, 3, real>& J0, type::Mat<4, 3, real>& JE)
{
    ...
    J0[0][0] = orientation[3];
    J0[0][1] = orientation[2];
    J0[0][2] = -orientation[1];
    J0[1][0] = -orientation[2];
    J0[1][1] = orientation[3];
    J0[1][2] = orientation[0];
    J0[2][0] = orientation[1];
    J0[2][1] = -orientation[0];
    J0[2][2] = orientation[3];
    J0[3][0] = -orientation[0];
    J0[3][1] = -orientation[1];
    J0[3][2] = -orientation[2];
    J0 *= (real)0.5;

    type::Vec<3, real> t = getTranslation();
    JE[0][0] = dual[3] + orientation[1] * t[1] + orientation[2] * t[2];
    JE[0][1] = dual[2] - orientation[1] * t[0] - orientation[3] * t[2];
    JE[0][2] = -dual[1] - orientation[2] * t[0] + orientation[3] * t[1];
    JE[1][0] = -dual[2] - orientation[0] * t[1] + orientation[3] * t[2];
    JE[1][1] = dual[3] + orientation[0] * t[0] + orientation[2] * t[2];
    JE[1][2] = dual[0] - orientation[3] * t[0] - orientation[2] * t[1];
    JE[2][0] = dual[1] - orientation[3] * t[1] - orientation[0] * t[2];
    JE[2][1] = -dual[0] + orientation[3] * t[0] - orientation[1] * t[2];
    JE[2][2] = dual[3] + orientation[0] * t[0] + orientation[1] * t[1];
    JE[3][0] = -dual[0] + orientation[2] * t[1] - orientation[1] * t[2];
    JE[3][1] = -dual[1] - orientation[2] * t[0] + orientation[0] * t[2];
    JE[3][2] = -dual[2] + orientation[1] * t[0] - orientation[0] * t[1];
    JE *= (real)0.5;
}
...
void DualQuatCoord3<real>::normalize_getJ(type::Mat<4, 4, real>& J0, type::Mat<4, 4, real>& JE)
{
    ...
    qn.dual *= Q0inv;
    for (i = 0; i < 4; i++)
        J0[i][i] = (real)1. - qn.orientation[i] * qn.orientation[i];
    for (i = 0; i < 4; i++)
        for (j = 0; j < i; j++)
            J0[i][j] = J0[j][i] = -qn.orientation[j] * qn.orientation[i];
    for (i = 0; i < 4; i++)
        for (j = 0; j <= i; j++)
            JE[i][j] = JE[j][i] = -qn.dual[j] * qn.orientation[i] - qn.dual[i] * qn.orientation[j];
    J0 *= Q0inv;
    JE -= J0 * Q0QE * Q0inv;
    JE *= Q0inv;
}
...
void DualQuatCoord3<real>::multRight_getJ(type::Mat<4, 4, real>& J0, type::Mat<4, 4, real>& JE)
{
    ...
    J0[0][1] = -orientation[2];
    J0[0][2] = orientation[1];
    J0[0][3] = orientation[0];
    J0[1][0] = orientation[2];
    J0[1][1] = orientation[3];
    J0[1][2] = -orientation[0];
    J0[1][3] = orientation[1];
    J0[2][0] = -orientation[1];
    J0[2][1] = orientation[0];
    J0[2][2] = orientation[3];
    J0[2][3] = orientation[2];
    J0[3][0] = -orientation[0];
    J0[3][1] = -orientation[1];
    J0[3][2] = -orientation[2];
    J0[3][3] = orientation[3];
    JE[0][0] = dual[3];
    JE[0][1] = -dual[2];
    JE[0][2] = dual[1];
    JE[0][3] = dual[0];
...
void DualQuatCoord3<real>::pointToParent_getJ(type::Mat<3, 4, real>& J0, type::Mat<3, 4, real>& JE, const type::Vec<3, real>& p)
{
    J0.fill(0);
    JE.fill(0);
    J0[0][0] = (real)2. * (-dual[3] + orientation[0] * p[0] + orientation[1] * p[1] + orientation[2] * p[2]);
    J0[0][1] = (real)2. * (dual[2] - orientation[1] * p[0] + orientation[0] * p[1] + orientation[3] * p[2]);
    J0[0][2] = (real)2. * (-dual[1] - orientation[2] * p[0] - orientation[3] * p[1] + orientation[0] * p[2]);
    J0[0][3] = (real)2. * (dual[0] + orientation[3] * p[0] - orientation[2] * p[1] + orientation[1] * p[2]);
    J0[1][0] = -J0[0][1];
    J0[1][1] = J0[0][0];
    J0[1][2] = J0[0][3];
    J0[1][3] = -J0[0][2];
    ...
    J0[2][2] = J0[0][0];
    J0[2][3] = J0[0][1];
    JE[0][0] = (real)2. * orientation[3];
    JE[0][1] = -(real)2. * orientation[2];
    JE[0][2] = (real)2. * orientation[1];
    JE[0][3] = -(real)2. * orientation[0];
    JE[1][0] = -JE[0][1];
    JE[1][1] = JE[0][0];
    JE[1][2] = JE[0][3];
    JE[1][3] = -JE[0][2];
...
type::Mat<3, 8, real> DualQuatCoord3<real>::pointToParent_applyH(const DualQuatCoord3<real>& dq, const type::Vec<3, real>& p)
{
    ...
    dJ.fill(0);
    dJ[0][0] = (real)2. * (-dq.dual[3] + dq.orientation[0] * p[0] + dq.orientation[1] * p[1] + dq.orientation[2] * p[2]);
    dJ[0][1] = (real)2. * (dq.dual[2] - dq.orientation[1] * p[0] + dq.orientation[0] * p[1] + dq.orientation[3] * p[2]);
    dJ[0][2] = (real)2. * (-dq.dual[1] - dq.orientation[2] * p[0] - dq.orientation[3] * p[1] + dq.orientation[0] * p[2]);
    dJ[0][3] = (real)2. * (dq.dual[0] + dq.orientation[3] * p[0] - dq.orientation[2] * p[1] + dq.orientation[1] * p[2]);
    dJ[1][0] = -dJ[0][1];
    dJ[1][1] = dJ[0][0];
    dJ[1][2] = dJ[0][3];
    dJ[1][3] = -dJ[0][2];
    ...
    dJ[2][2] = dJ[0][0];
    dJ[2][3] = dJ[0][1];
    dJ[0][4] = (real)2. * dq.orientation[3];
    dJ[0][5] = -(real)2. * dq.orientation[2];
    dJ[0][6] = (real)2. * dq.orientation[1];
    dJ[0][7] = -(real)2. * dq.orientation[0];
    dJ[1][4] = -dJ[0][5];
    dJ[1][5] = dJ[0][4];
    dJ[1][6] = dJ[0][7];
    dJ[1][7] = -dJ[0][6];
