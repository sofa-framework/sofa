    ...
    **/
    void clearRowCol(Index i) override
    {
        ...
        else
        {
            ...
            Index rowId = 0;
            if (!this->searchRow(i, rowId)) return;
            ...
            for (Index xj = rowRange.begin(); xj < rowRange.end(); ++xj)
            {
                ...
                Index jRowId = 0;
                if (!this->searchRow(j, jRowId)) continue;

                Index colId = 0;
                if (!this->searchColInRange(Range(this->rowBegin[jRowId], this->rowBegin[jRowId+1]), i, colId)) continue;
...
public:
    ...
    BlockConstAccessor blockGet(Index i, Index j) const
    {
        ...
        if (this->searchRow(i, rowId))
        {
            Range rowRange(this->rowBegin[rowId], this->rowBegin[rowId+1]);
            Index colId = 0;
            if (this->searchColInRange(rowRange, j, colId))
            {
