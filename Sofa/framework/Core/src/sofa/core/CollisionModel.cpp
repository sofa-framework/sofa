...
CollisionModel::CollisionModel()
    : bActive(initData( true, "active", "flag indicating if this collision model is active and should be included in default collision detections"))
    , bMoving(initData( true, "moving", "flag indicating if this object is changing position between iterations"))
    , bSimulated(initData( true, "simulated", "flag indicating if this object is controlled by a simulation"))
    , bSelfCollision(initData( false, "selfCollision", "flag indication if the object can self collide"))
    , proximity(initData( (SReal)0.0, "proximity", "Distance to the actual (visual) surface"))
    , contactStiffness(initData( (SReal)10.0, "contactStiffness", "Contact stiffness"))
    , contactFriction(initData( (SReal)0.0, "contactFriction", "Contact friction coefficient (dry or viscous or unused depending on the contact method)"))
    , contactRestitution(initData( (SReal)0.0, "contactRestitution", "Contact coefficient of restitution"))
    , contactResponse(initData( "contactResponse", "if set, indicate to the ContactManager that this model should use the given class of contacts.\nNote that this is only indicative, and in particular if both collision models specify a different class it is up to the manager to choose."))
    , color(initData( sofa::type::RGBAColor(1,0,0,1), "color", "color used to display the collision model if requested"))
    , group(initData("group","IDs of the groups containing this model. No collision can occur between collision models included in a common group (e.g. allowing the same object to have multiple collision models)"))
    , size(0)
    , d_numberOfContacts(initData( (Size)0, "numberOfContacts", "Number of collision models this collision model is currently attached to"))
