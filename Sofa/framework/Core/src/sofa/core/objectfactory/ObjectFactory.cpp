...
ClassEntry& ObjectFactory::getEntry(std::string classname)
{
    if (registry.find(classname) == registry.end()) {
        registry[classname] = ClassEntrySPtr(new ClassEntry);
                             ...
                             ClassEntrySPtr* previous)
{
    ...
    const ClassEntrySPtr& pointedEntry = it->second;
    ClassEntrySPtr& aliasEntry = registry[name];
    ...
    if (previous) {
        const ClassEntrySPtr& entry = aliasEntry;
        *previous = entry;
    }
...
void ObjectFactory::resetAlias(std::string name, ClassEntrySPtr previous)
{
...
objectmodel::BaseObject::SPtr ObjectFactory::createObject(objectmodel::BaseContext* context, objectmodel::BaseObjectDescription* arg)
{
    ...
    std::vector< std::pair<std::string, BaseObjectCreatorSPtr> > creators;
    ...
    if (it != registry.end()) // Found the classname
    {
        ...
        BaseObjectCreatorMap::iterator it2 = entry->creatorMap.find(templatename);
        if (it2 != entry->creatorMap.end())
        {
            BaseObjectCreatorSPtr c = it2->second;
        ...
        if (creators.empty())
        {
            BaseObjectCreatorMap::iterator it3;
            for (it3 = entry->creatorMap.begin(); it3 != entry->creatorMap.end(); ++it3)
            {
                ...
                BaseObjectCreatorSPtr c = it3->second;
                if (c->canCreate(context, arg)){
...
void ObjectFactory::getEntriesFromTarget(std::vector<ClassEntrySPtr>& result, std::string target)
{
        ...
        it != itEnd; ++it)
    {
        ...
        if(entry->className == it->first)
        {
            ...
            bool inTarget = false;
            for (BaseObjectCreatorMap::iterator itc = entry->creatorMap.begin(), itcend = entry->creatorMap.end(); itc != itcend; ++itc)
            {
                const BaseObjectCreatorSPtr c = itc->second;
