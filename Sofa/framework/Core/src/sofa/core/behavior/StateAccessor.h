 ...
 * as a derived type.
 */
class SOFA_CORE_API StateAccessor : public virtual objectmodel::BaseComponent
{
public:
    SOFA_ABSTRACT_CLASS(StateAccessor, objectmodel::BaseComponent);
...
inline void StateAccessor::computeBBox(const core::ExecParams* params, bool onlyVisible)
{
    ...
    for (const auto mstate : l_mechanicalStates)
    {
        if (mstate)
        {
            const auto& mstateBbox = mstate->f_bbox.getValue();
            bbox.include(mstateBbox);
            anyMState = true;
        }
    }
