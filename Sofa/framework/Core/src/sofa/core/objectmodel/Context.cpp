...
Context::Context()
    : is_activated(initData( true, "activated", "To Activate a node"))
    , worldGravity_(initData( Vec3(SReal(0),SReal(-9.81),SReal(0)),"gravity","Gravity in the world coordinate system"))
    , dt_(initData(SReal(0.01),"dt","Time step"))
    , time_(initData(SReal(0.),"time","Current time"))
    , animate_(initData(false,"animate","Animate the Simulation(applied at initialization only)"))
	, d_isSleeping(initData( false, "sleeping", "The node is sleeping, and thus ignored by visitors."))
	, d_canChangeSleepingState(initData( false, "canChangeSleepingState", "The node can change its sleeping state."))
{
