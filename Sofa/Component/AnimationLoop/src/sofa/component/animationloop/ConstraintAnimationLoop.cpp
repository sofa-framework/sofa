...
ConstraintAnimationLoop::ConstraintAnimationLoop(simulation::Node* gnode)
    : Inherit(gnode)
    , d_displayTime(initData( false, "displayTime","Display time for each important step of ConstraintAnimationLoop."))
    , d_tol( initData( 0.00001, "tolerance", "Tolerance of the Gauss-Seidel"))
    , d_maxIt( initData( 1000, "maxIterations", "Maximum number of iterations of the Gauss-Seidel"))
    , d_doCollisionsFirst(initData( false, "doCollisionsFirst","Compute the collisions first (to support penality-based contacts)"))
    , d_doubleBuffer( initData( false, "doubleBuffer","Buffer the constraint problem in a double buffer to be accessible with an other thread"))
    , d_scaleTolerance( initData( true, "scaleTolerance","Scale the error tolerance with the number of constraints"))
    , d_allVerified( initData( false, "allVerified","All contraints must be verified (each constraint's error < tolerance)"))
    , d_sor( initData( 1.0, "sor","Successive Over Relaxation parameter (0-2)"))
    , d_schemeCorrection( initData( false, "schemeCorrection","Apply new scheme where compliance is progressively corrected"))
    , d_realTimeCompensation( initData( false, "realTimeCompensation","If the total computational time T < dt, sleep(dt-T)"))
    , d_graphErrors( initData("graphErrors","Sum of the constraints' errors at each iteration"))
    , d_graphConstraints( initData("graphConstraints","Graph of each constraint's error at the end of the resolution"))
    , d_graphForces( initData("graphForces","Graph of each constraint's force at each step of the resolution"))
{
    bufCP1 = false;
