    ...
    sofa::type::vector<Real>& baryCoefs) const
{
    ...
    const Vec3 pa_proj = a - v_normal * dot(a - triP[0], v_normal);
    const Vec3 pb_proj = b - v_normal * dot(b - triP[0], v_normal);

    const sofa::type::fixed_array<EdgeID, 3>& edgesInTri = this->m_topology->getEdgesInTriangle(ind_t);
    ...
    sofa::type::vector< EdgeID >& new_edges, Real epsilonSnapPath, Real epsilonSnapBorder)
{
    ...
    for (size_t i = 0; i < nb_points; i++)
    {
            ...
            if ((epsilonSnapPath != 0.0) || (!points2Snap.empty()))
                for (size_t j = 0; j < points2Snap.size(); j++)
                    if (points2Snap[j][0] == intersected_indices[i])
                    {
                        ...
                        if (intersected_topoElements[i - 1] == core::topology::TopologyElementType::POINT) //second dof has to be moved, first acestor must be pa
        ...
        case core::topology::TopologyElementType::EDGE:
        {
            ...
            srcElems.push_back(core::topology::PointAncestorElem(core::topology::TopologyElementType::EDGE, intersected_indices[i],
        ...
        case core::topology::TopologyElementType::TRIANGLE:
        {
            ...
            srcElems.push_back(core::topology::PointAncestorElem(core::topology::TopologyElementType::TRIANGLE, intersected_indices[i],
...
template<class DataTypes>
void TriangleSetGeometryAlgorithms<DataTypes>::SnapAlongPath(sofa::type::vector< sofa::core::topology::TopologyElementType>& intersected_topoElements,
    ...
    Real epsilonSnapPath)
{
    ...
    for (it = map_point2snap.begin(); it != map_point2snap.end(); ++it)
    {
        ...
        intersected_topoElements[((*it).second)[0]] = core::topology::TopologyElementType::POINT;
    ...
    Real epsilonSnapBorder)
{
    ...
    if (!snap_a  && intersected_topoElements[0] == core::topology::TopologyElementType::TRIANGLE) // this means a is not close to a point, but could be close to an edge
    {
        for (unsigned int i = 0; i < 3; i++)
        {
            if (intersected_barycoefs[0][i] < epsilon)
            {
                ...
                if ((m_container->getTrianglesAroundEdge(theEdge)).size() > 1) //snap to point and not edge
                {
                    for (unsigned int j = 0; j < 3; j++)
                        if (intersected_barycoefs[0][j] > 1 - epsilon)
                        {
                            ...
                            intersected_topoElements[0] = core::topology::TopologyElementType::POINT;
                    ...
                    while (find)
                    {
                        pointDone = true;
                        allDone = true;
                        if (intersected_topoElements[1] == core::topology::TopologyElementType::EDGE) // just remove or need to projection?
                        {
                        ...
                        else if (intersected_topoElements[1] == core::topology::TopologyElementType::POINT)
                        {
                ...
                if ((intersected_indices[1] == theEdge) && (intersected_topoElements[1] == core::topology::TopologyElementType::EDGE)) // Only keep this one? or need to project?
                {
                ...
                else // need to create this point by projection
                {
                    ...
                    intersected_topoElements[0] = core::topology::TopologyElementType::EDGE;
    ...
    // Same for last point
    if (!snap_b  && intersected_topoElements.back() == core::topology::TopologyElementType::TRIANGLE) // this means a is not close to a point, but could be close to an edge
    {
        for (unsigned int i = 0; i < 3; i++)
        {
            if (intersected_barycoefs.back()[i] < epsilon)
            {
                ...
                if ((m_container->getTrianglesAroundEdge(theEdge)).size() > 1) //snap to point and not edge
                {
                    for (unsigned int j = 0; j < 3; j++)
                        if (intersected_barycoefs.back()[j] > 1 - epsilon)
                        {
                            ...
                            intersected_topoElements.back() = core::topology::TopologyElementType::POINT;
                    ...
                    while (find)
                    {
                        ...
                        pointDone = true;
                        allDone = true;
                        if (intersected_topoElements[pos] == core::topology::TopologyElementType::EDGE) // just remove or need to projection?
                        {
                            const sofa::type::vector<EdgeID> &shell = m_container->getEdgesAroundVertex(thePoint);
                            for (size_t k = 0; k < shell.size(); k++)
                            {
                        ...
                        else if (intersected_topoElements[pos] == core::topology::TopologyElementType::POINT)
                        {
                ...
                if ((intersected_indices[intersected_indices.size() - 2] == theEdge) && (intersected_topoElements[intersected_topoElements.size() - 2] == core::topology::TopologyElementType::EDGE)) // Only keep this one? or need to projection?
                {
                ...
                else
                {
                    ...
                    intersected_topoElements.back() = core::topology::TopologyElementType::EDGE;
