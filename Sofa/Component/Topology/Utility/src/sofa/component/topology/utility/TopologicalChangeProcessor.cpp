...
TopologicalChangeProcessor::TopologicalChangeProcessor()
    : m_filename( initData( "filename", "input file name for topological changes."))
    , m_listChanges (initData( "listChanges", "0 for adding, 1 for removing, 2 for cutting and associated indices."))
    , m_interval( initData( 0.0, "interval", "time duration between 2 actions"))
    , m_shift( initData( 0.0, "shift", "shift between times in the file and times when they will be read"))
    , m_loop( initData( false, "loop", "set to 'true' to re-read the file when reaching the end"))
    , m_useDataInputs( initData( false, "useDataInputs", "If true, will perform operation using Data input lists rather than text file."))
    , m_timeToRemove( initData( 0.0, "timeToRemove", "If using option useDataInputs, time at which will be done the operations. Possibility to use the interval Data also."))
    , m_pointsToRemove(initData( "pointsToRemove", "List of point IDs to be removed."))
    , m_edgesToRemove (initData( "edgesToRemove", "List of edge IDs to be removed."))
    , m_trianglesToRemove (initData( "trianglesToRemove", "List of triangle IDs to be removed."))
    , m_quadsToRemove (initData( "quadsToRemove", "List of quad IDs to be removed."))
    , m_tetrahedraToRemove (initData( "tetrahedraToRemove", "List of tetrahedron IDs to be removed."))
    , m_hexahedraToRemove (initData( "hexahedraToRemove", "List of hexahedron IDs to be removed."))
    , m_saveIndicesAtInit( initData( false, "saveIndicesAtInit", "set to 'true' to save the incision to do in the init to incise even after a movement"))
    , m_epsilonSnapPath( initData( (SReal)0.1, "epsilonSnapPath", "epsilon snap path"))
    , m_epsilonSnapBorder( initData( (SReal)0.25, "epsilonSnapBorder", "epsilon snap path"))
    , m_draw( initData( false, "draw", "draw information"))
