...
SubsetTopologicalMapping::SubsetTopologicalMapping()
    : samePoints(initData(false,"samePoints", "True if the same set of points is used in both topologies"))
    , handleEdges(initData(false,"handleEdges", "True if edges events and mapping should be handled"))
    , handleTriangles(initData(false,"handleTriangles", "True if triangles events and mapping should be handled"))
    , handleQuads(initData(false,"handleQuads", "True if quads events and mapping should be handled"))
    , handleTetrahedra(initData(false,"handleTetrahedra", "True if tetrahedra events and mapping should be handled"))
    , handleHexahedra(initData(false,"handleHexahedra", "True if hexahedra events and mapping should be handled"))
    , pointS2D(initData("pointS2D", "Internal source -> destination topology points map"))
    , pointD2S(initData("pointD2S", "Internal destination -> source topology points map (link to SubsetMapping::indices to handle the mechanical-side of the mapping"))
    , edgeS2D(initData("edgeS2D", "Internal source -> destination topology edges map"))
    , edgeD2S(initData("edgeD2S", "Internal destination -> source topology edges map"))
    , triangleS2D(initData("triangleS2D", "Internal source -> destination topology triangles map"))
    , triangleD2S(initData("triangleD2S", "Internal destination -> source topology triangles map"))
    , quadS2D(initData("quadS2D", "Internal source -> destination topology quads map"))
    , quadD2S(initData("quadD2S", "Internal destination -> source topology quads map"))
    , tetrahedronS2D(initData("tetrahedronS2D", "Internal source -> destination topology tetrahedra map"))
    , tetrahedronD2S(initData("tetrahedronD2S", "Internal destination -> source topology tetrahedra map"))
    , hexahedronS2D(initData("hexahedronS2D", "Internal source -> destination topology hexahedra map"))
    , hexahedronD2S(initData("hexahedronD2S", "Internal destination -> source topology hexahedra map"))
{
}
