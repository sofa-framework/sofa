...
SubsetTopologicalMapping::SubsetTopologicalMapping()
    ...
    , d_handleEdges(initData(&d_handleEdges, false, "d_handleEdges", "True if edges events and mapping should be handled"))
    ...
    , d_handleQuads(initData(&d_handleQuads, false, "d_handleQuads", "True if quads events and mapping should be handled"))
    , d_handleTetrahedra(initData(&d_handleTetrahedra, false, "d_handleTetrahedra", "True if tetrahedra events and mapping should be handled"))
    , d_handleHexahedra(initData(&d_handleHexahedra, false, "d_handleHexahedra", "True if hexahedra events and mapping should be handled"))
    , d_pointS2D(initData(&d_pointS2D, "d_pointS2D", "Internal source -> destination topology points map"))
    , d_pointD2S(initData(&d_pointD2S, "d_pointD2S", "Internal destination -> source topology points map (link to SubsetMapping::indices to handle the mechanical-side of the mapping"))
    , d_edgeS2D(initData(&d_edgeS2D, "d_edgeS2D", "Internal source -> destination topology edges map"))
    , d_edgeD2S(initData(&d_edgeD2S, "d_edgeD2S", "Internal destination -> source topology edges map"))
    , d_triangleS2D(initData(&d_triangleS2D, "d_triangleS2D", "Internal source -> destination topology triangles map"))
    , d_triangleD2S(initData(&d_triangleD2S, "d_triangleD2S", "Internal destination -> source topology triangles map"))
    , d_quadS2D(initData(&d_quadS2D, "d_quadS2D", "Internal source -> destination topology quads map"))
    , d_quadD2S(initData(&d_quadD2S, "d_quadD2S", "Internal destination -> source topology quads map"))
    , d_tetrahedronS2D(initData(&d_tetrahedronS2D, "d_tetrahedronS2D", "Internal source -> destination topology tetrahedra map"))
    , d_tetrahedronD2S(initData(&d_tetrahedronD2S, "d_tetrahedronD2S", "Internal destination -> source topology tetrahedra map"))
    , d_hexahedronS2D(initData(&d_hexahedronS2D, "d_hexahedronS2D", "Internal source -> destination topology hexahedra map"))
    , d_hexahedronD2S(initData(&d_hexahedronD2S, "d_hexahedronD2S", "Internal destination -> source topology hexahedra map"))
{
