...
VectorSpringForceField<DataTypes>::VectorSpringForceField(MechanicalState* _object1, MechanicalState* _object2)
    ...
    , springArray( initData( "springs", "springs data"))
    , m_filename( initData(std::string(""),"filename","File name from which the spring informations are loaded") )
    , m_stiffness( initData(SReal(1.0),"stiffness","Default edge stiffness used in absence of file information") )
    , m_viscosity( initData( SReal(1.0),"viscosity","Default edge viscosity used in absence of file information") )
    , m_useTopology( initData( false, "useTopology", "Activate/Desactivate topology mode of the component (springs on each edge)"))
