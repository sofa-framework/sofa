...
PolynomialSpringsForceField<DataTypes>::PolynomialSpringsForceField(MechanicalState* mstate1, MechanicalState* mstate2)
    ...
    , d_firstObjectPoints(initData( "firstObjectPoints", "points related to the first object"))
    , d_secondObjectPoints(initData( "secondObjectPoints", "points related to the second object"))
    ...
    , d_computeZeroLength(initData( 1, "computeZeroLength", "flag to compute initial length for springs"))
    , d_zeroLength(initData( "zeroLength", "initial length for springs"))
    , d_recomputeIndices(initData( false, "recompute_indices", "Recompute indices (should be false for BBOX)"))
    , d_compressible(initData( false, "compressible", "Indicates if object compresses without reactio force"))
    , d_drawMode(initData( 0, "drawMode", "The way springs will be drawn:\n- 0: Line\n- 1:Cylinder\n- 2: Arrow"))
    , d_showArrowSize(initData( 0.01f, "showArrowSize","size of the axis"))
