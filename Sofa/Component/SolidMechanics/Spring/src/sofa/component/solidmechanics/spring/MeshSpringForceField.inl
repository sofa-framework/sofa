...
MeshSpringForceField<DataTypes>::MeshSpringForceField()
    : d_linesStiffness(initData(Real(0),"linesStiffness","Stiffness for the Lines",true))
    , d_linesDamping(initData(Real(0),"linesDamping","Damping for the Lines",true))
    , d_trianglesStiffness(initData(Real(0),"trianglesStiffness","Stiffness for the Triangles",true))
    , d_trianglesDamping(initData(Real(0),"trianglesDamping","Damping for the Triangles",true))
    , d_quadsStiffness(initData(Real(0),"quadsStiffness","Stiffness for the Quads",true))
    , d_quadsDamping(initData(Real(0),"quadsDamping","Damping for the Quads",true))
    , d_tetrahedraStiffness(initData(Real(0),"tetrahedraStiffness","Stiffness for the Tetrahedra",true))
    , d_tetrahedraDamping(initData(Real(0),"tetrahedraDamping","Damping for the Tetrahedra",true))
    , d_cubesStiffness(initData(Real(0),"cubesStiffness","Stiffness for the Cubes",true))
    , d_cubesDamping(initData(Real(0),"cubesDamping","Damping for the Cubes",true))
    , d_noCompression( initData( false, "noCompression", "Only consider elongation", false))
    , d_drawMinElongationRange(initData( Real(8.), "drawMinElongationRange","Min range of elongation (red eongation - blue neutral - green compression)"))
    , d_drawMaxElongationRange(initData( Real(15.), "drawMaxElongationRange","Max range of elongation (red eongation - blue neutral - green compression)"))
    , d_drawSpringSize(initData( Real(8.), "drawSpringSize","Size of drawed lines"))
    , d_localRange( initData( type::Vec<2, sofa::Index>(sofa::InvalidID, sofa::InvalidID), "localRange", "optional range of local DOF indices. Any computation involving only indices outside of this range are discarded (useful for parallelization using mesh partitionning)" ) )
