...
JointSpringForceField<DataTypes>::JointSpringForceField(MechanicalState* object1, MechanicalState* object2)
    ...
    , m_infile(nullptr)
    , m_outfile(nullptr)
    , f_outfilename( initData( "outfile", "output file name"))
    , f_infilename( initData( "infile", "input file containing constant joint force"))
    ...
    , d_springs(initData("spring","pairs of indices, stiffness, damping, rest length"))
    , d_showLawfulTorsion(initData( false, "showLawfulTorsion", "display the lawful part of the joint rotation"))
    , d_showExtraTorsion(initData( false, "showExtraTorsion", "display the illicit part of the joint rotation"))
    , d_showFactorSize(initData( (Real)1.0, "showFactorSize", "modify the size of the debug information of a given factor" ))
{
}
