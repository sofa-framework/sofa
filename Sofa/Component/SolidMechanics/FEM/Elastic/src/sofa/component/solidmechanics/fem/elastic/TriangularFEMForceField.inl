...
TriangularFEMForceField<DataTypes>::TriangularFEMForceField()
    ...
    , vertexInfo(initData( "vertexInfo", "Internal point data"))
    , m_topology(nullptr)
    , method(LARGE)
    , f_method(initData( std::string("large"), "method", "large: large displacements, small: small displacements"))
    , f_poisson(initData( type::vector<Real>(1, static_cast<Real>(0.3)), "poissonRatio", "Poisson ratio in Hooke's law (vector)"))
    , f_young(initData( type::vector<Real>(1, static_cast<Real>(1000.0)), "youngModulus", "Young modulus in Hooke's law (vector)"))
    , m_rotatedInitialElements(initData( "rotatedInitialElements", "Flag activating rendering of stress directions within each triangle"))
    , m_initialTransformation(initData( "initialTransformation", "Flag activating rendering of stress directions within each triangle"))
    , hosfordExponant(initData( (Real)1.0, "hosfordExponant", "Exponant in the Hosford yield criteria"))
    , criteriaValue(initData( (Real)1e15, "criteriaValue", "Fracturable threshold used to draw fracturable triangles"))
    , showStressValue(initData( true, "showStressValue", "Flag activating rendering of stress values as a color in each triangle"))
    , showStressVector(initData( false, "showStressVector", "Flag activating rendering of stress directions within each triangle"))
    , showFracturableTriangles(initData( false, "showFracturableTriangles", "Flag activating rendering of triangles to fracture"))
    , f_computePrincipalStress(initData( false, "computePrincipalStress", "Compute principal stress for each triangle"))
    , l_topology(initLink("topology", "link to the topology container"))
#ifdef PLOT_CURVE
    , elementID(initData( (Real)0, "id", "element id to follow in the graphs"))
    , f_graphStress(initData( "graphMaxStress", "Graph of max stress corresponding to the element id"))
    , f_graphCriteria(initData( "graphCriteria", "Graph of the fracture criteria corresponding to the element id"))
    , f_graphOrientation(initData( "graphOrientation", "Graph of the orientation of the principal stress direction corresponding to the element id"))
#endif
