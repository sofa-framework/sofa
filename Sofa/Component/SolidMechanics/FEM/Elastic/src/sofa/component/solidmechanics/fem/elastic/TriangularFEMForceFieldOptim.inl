...
TriangularFEMForceFieldOptim<DataTypes>::TriangularFEMForceFieldOptim()
    : d_triangleInfo(initData( "triangleInfo", "Internal triangle data (persistent)"))
    , d_triangleState(initData( "triangleState", "Internal triangle data (time-dependent)"))
    , d_poisson(initData((Real)(0.3),"poissonRatio","Poisson ratio in Hooke's law"))
    , d_young(initData((Real)(1000.0),"youngModulus","Young modulus in Hooke's law"))
    , d_damping(initData((Real)0.,"damping","Ratio damping/stiffness"))
    , d_restScale(initData((Real)1.,"restScale","Scale factor applied to rest positions (to simulate pre-stretched materials)"))
    , d_showStressVector(initData(false,"showStressVector","Flag activating rendering of stress directions within each triangle"))
    , d_showStressMaxValue(initData((Real)0.0,"showStressMaxValue","Max value for rendering of stress values"))
