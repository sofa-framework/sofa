...
HexahedronFEMForceField<DataTypes>::HexahedronFEMForceField()
    : f_method(initData(std::string("large"),"method","\"large\" or \"polar\" or \"small\" displacements" ))
    , f_poissonRatio(initData((Real)0.45f,"poissonRatio","FEM Poisson Ratio in Hooke's law [0,0.5["))
    , f_youngModulus(initData((Real)5000,"youngModulus","FEM Young's modulus in Hooke's law"))
    , f_updateStiffnessMatrix(initData(false,"updateStiffnessMatrix",""))
    , _gatherPt(initData("gatherPt","number of dof accumulated per threads during the gather operation (Only use in GPU version)"))
    , _gatherBsize(initData("gatherBsize","number of dof accumulated per threads during the gather operation (Only use in GPU version)"))
    , f_drawing(initData(true,"drawing","draw the forcefield if true"))
    , f_drawPercentageOffset(initData((Real)0.15,"drawPercentageOffset","size of the hexa"))
    , needUpdateTopology(false)
    , l_topology(initLink("topology", "link to the topology container"))
    , _elementStiffnesses(initData("stiffnessMatrices", "Stiffness matrices per element (K_i)"))
    , m_topology(nullptr)
    , _sparseGrid(nullptr)
