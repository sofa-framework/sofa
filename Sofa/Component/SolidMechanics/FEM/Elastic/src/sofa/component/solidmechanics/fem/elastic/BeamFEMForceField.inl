...
BeamFEMForceField<DataTypes>::BeamFEMForceField(Real poissonRatio, Real youngModulus, Real radius, Real radiusInner)
    : m_beamsData(initData( "beamsData", "Internal element data"))
    , m_indexedElements(nullptr)
    , d_poissonRatio(initData((Real)poissonRatio,"poissonRatio","Poisson's Ratio"))
    , d_youngModulus(initData((Real)youngModulus,"youngModulus","Young Modulus"))
    , d_radius(initData((Real)radius,"radius","radius of the section"))
    , d_radiusInner(initData((Real)radiusInner,"radiusInner","inner radius of the section for hollow beams"))
    , d_listSegment(initData("listSegment", "apply the forcefield to a subset list of beam segments. If no segment defined, forcefield applies to the whole topology"))
    , d_useSymmetricAssembly(initData(false,"useSymmetricAssembly","use symmetric assembly of the matrix K"))
