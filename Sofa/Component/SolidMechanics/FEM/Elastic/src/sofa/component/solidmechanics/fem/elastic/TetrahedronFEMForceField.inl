...
TetrahedronFEMForceField<DataTypes>::TetrahedronFEMForceField()
    ...
    , f_method(initData(std::string("large"),"method","\"small\", \"large\" (by QR), \"polar\" or \"svd\" displacements"))
    , _poissonRatio(initData((Real)0.45f,"poissonRatio","FEM Poisson Ratio in Hooke's law [0,0.5["))
    , _youngModulus(initData("youngModulus","FEM Young's Modulus in Hooke's law"))
    , _localStiffnessFactor(initData( "localStiffnessFactor","Allow specification of different stiffness per element. If there are N element and M values are specified, the youngModulus factor for element i would be localStiffnessFactor[i*M/N]"))
    , _updateStiffnessMatrix(initData(false,"updateStiffnessMatrix",""))
    , _assembling(initData(false,"computeGlobalMatrix",""))
    , _plasticMaxThreshold(initData((Real)0.f,"plasticMaxThreshold","Plastic Max Threshold (2-norm of the strain)"))
    , _plasticYieldThreshold(initData((Real)0.0001f,"plasticYieldThreshold","Plastic Yield Threshold (2-norm of the strain)"))
    , _plasticCreep(initData((Real)0.9f,"plasticCreep","Plastic Creep Factor * dt [0,1]. Warning this factor depends on dt."))
    ...
    , drawHeterogeneousTetra(initData(false,"drawHeterogeneousTetra","Draw Heterogeneous Tetra in different color"))
    , _computeVonMisesStress(initData(0,"computeVonMisesStress","compute and display von Mises stress: 0: no computations, 1: using corotational strain, 2: using full Green strain. Set listening=1"))
    , _vonMisesPerElement(initData( "vonMisesPerElement", "von Mises Stress per element"))
    , _vonMisesPerNode(initData( "vonMisesPerNode", "von Mises Stress per node"))
    , _vonMisesStressColors(initData( "vonMisesStressColors", "Vector of colors describing the VonMises stress"))
    , _showStressColorMap(initData( std::string("Blue to Red"),"showStressColorMap", "Color map used to show stress values"))
    , _showStressAlpha(initData( 1.0f, "showStressAlpha", "Alpha for vonMises visualisation"))
    , _showVonMisesStressPerNode(initData(false,"showVonMisesStressPerNode","draw points showing vonMises stress interpolated in nodes"))
    , _showVonMisesStressPerElement(initData( false, "showVonMisesStressPerElement", "draw triangles showing vonMises stress interpolated in elements"))
    , _updateStiffness(initData(false,"updateStiffness","udpate structures (precomputed in init) using stiffness parameters in each iteration (set listening=1)"))
