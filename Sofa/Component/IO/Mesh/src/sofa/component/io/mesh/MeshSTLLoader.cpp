...
MeshSTLLoader::MeshSTLLoader() : MeshLoader()
    , _headerSize(initData( 80u, "headerSize","Size of the header binary file (just before the number of facet)."))
    , _forceBinary(initData( false, "forceBinary","Force reading in binary mode. Even in first keyword of the file is solid."))
    , d_mergePositionUsingMap(initData( true, "mergePositionUsingMap","Since positions are duplicated in a STL, they have to be merged. Using a map to do so will temporarily duplicate memory but should be more efficient. Disable it if memory is really an issue."))
{
}
