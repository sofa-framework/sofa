...
MeshOBJLoader::MeshOBJLoader()
    ...
    , d_handleSeams(initData( (bool)false, "handleSeams", "Preserve UV and normal seams information (vertices with multiple UV and/or normals)"))
    , d_loadMaterial(initData( (bool) true, "loadMaterial", "Load the related MTL file or use a default one?"))
    , d_material(initData("defaultMaterial","Default material") )
    , d_materials(initData("materials","List of materials") )
    , d_faceList(initData("faceList","List of face definitions.") )
    , d_texIndexList(initData("texcoordsIndex","Indices of textures coordinates used in faces definition."))
    , d_positionsList(initData("positionsDefinition", "Vertex positions definition"))
    , d_texCoordsList(initData("texcoordsDefinition", "Texture coordinates definition"))
    , d_normalsIndexList(initData("normalsIndex","List of normals of elements of the mesh loaded."))
    , d_normalsList(initData("normalsDefinition","Normals definition"))
    , d_texCoords(initData("texcoords","Texture coordinates of all faces, to be used as the parent data of a VisualModel texcoords data"))
    , d_computeMaterialFaces(initData( false, "computeMaterialFaces", "True to activate export of Data instances containing list of face indices for each material"))
    , d_vertPosIdx      (initData( "vertPosIdx", "If vertices have multiple normals/texcoords stores vertices position indices"))
    , d_vertNormIdx     (initData( "vertNormIdx", "If vertices have multiple normals/texcoords stores vertices normal indices"))
{
