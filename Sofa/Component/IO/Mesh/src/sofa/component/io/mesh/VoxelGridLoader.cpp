...
VoxelGridLoader::VoxelGridLoader()
    : VoxelLoader(),
      ...
      dataResolution ( initData( Vec3i ( 0,0,0 ), "resolution", "Resolution of the voxel file" ) ),
      roi ( initData( Vec6i ( 0,0,0, 0xFFFF, 0xFFFF, 0xFFFF ), "ROI", "Region of interest (xmin, ymin, zmin, xmax, ymax, zmax)" ) ),
      headerSize ( initData( 0, "header", "Header size in bytes" ) ),
      segmentationHeaderSize ( initData( 0, "segmentationHeader", "Header size in bytes" ) ),
      idxInRegularGrid(initData("idxInRegularGrid","indices of the hexa in the grid.")),
      backgroundValue ( initData( "bgValue", "Background values (to be ignored)" ) ),
      activeValue ( initData( "dataValue", "Active data values" ) ),
      generateHexa( initData( true, "generateHexa", "Interpret voxel as either hexa or points")),
      image(nullptr),
