...
VTKExporter::VTKExporter()
    ...
    , vtkFilename( initData( "filename", "output VTK file name"))
    , fileFormat( initData( (bool) true, "XMLformat", "Set to true to use XML format"))
    , position( initData( "position", "points position (will use points from topology or mechanical state if this is empty)"))
    , writeEdges( initData( (bool) true, "edges", "write edge topology"))
    , writeTriangles( initData( (bool) false, "triangles", "write triangle topology"))
    , writeQuads( initData( (bool) false, "quads", "write quad topology"))
    , writeTetras( initData( (bool) false, "tetras", "write tetra topology"))
    , writeHexas( initData( (bool) false, "hexas", "write hexa topology"))
    , dPointsDataFields( initData( "pointsDataFields", "Data to visualize (on points)"))
    , dCellsDataFields( initData( "cellsDataFields", "Data to visualize (on cells)"))
    ...
    , overwrite( initData( false, "overwrite", "overwrite the file, otherwise create a new file at each export, with suffix in the filename"))
{
}
