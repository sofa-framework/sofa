...
protected:
    BTDLinearSolver()
        : d_verbose( initData(false,"verbose","Dump system state at each iteration") )
        , d_problem(initData(false,"showProblem", "display debug informations about subpartSolve computation") )
        , d_subpartSolve(initData(false,"subpartSolve", "Allows for the computation of a subpart of the system") )
        , d_verification(initData(false,"verification", "verification of the subpartSolve"))
        , d_blockSize( initData(6,"blockSize","dimension of the blocks in the matrix") )
    {
