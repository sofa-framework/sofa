...
ShewchukPCGLinearSolver<TMatrix,TVector>::ShewchukPCGLinearSolver()
    ...
    , f_use_precond( initData(true,"use_precond","Use preconditioner") )
    , f_update_step( initData((unsigned)1,"update_step","Number of steps before the next refresh of precondtioners") )
    , f_build_precond( initData(true,"build_precond","Build the preconditioners, if false build the preconditioner only at the initial step") )
    , f_preconditioners( initData( "preconditioners", "If not empty: path to the solvers to use as preconditioners") )
