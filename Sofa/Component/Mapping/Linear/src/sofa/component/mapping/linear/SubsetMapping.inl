...
SubsetMapping<TIn, TOut>::SubsetMapping()
    : Inherit()
    , f_indices( initData( "indices", "list of input indices"))
    , f_first( initData( sofa::InvalidID, "first", "first index (use if indices are sequential)"))
    , f_last( initData( sofa::InvalidID, "last", "last index (use if indices are sequential)"))
    , f_radius( initData( (Real)1.0e-5, "radius", "search radius to find corresponding points in case no indices are given"))
    , f_handleTopologyChange( initData( true, "handleTopologyChange", "Enable support of topological changes for indices (disable if it is linked from SubsetTopologicalMapping::pointD2S)"))
    , f_ignoreNotFound( initData( false, "ignoreNotFound", "True to ignore points that are not found in the input model, they will be treated as fixed points"))
    , f_resizeToModel( initData( false, "resizeToModel", "True to resize the output MechanicalState to match the size of indices"))
