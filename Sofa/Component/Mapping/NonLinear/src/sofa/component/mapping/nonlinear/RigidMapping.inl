...
RigidMapping<TIn, TOut>::RigidMapping()
    : Inherit()
    , points(initData( "initialPoints", "Local Coordinates of the points"))
    , index(initData( (unsigned)0, "index", "input DOF index"))
    , fileRigidMapping(initData( "filename", "Xsp file where rigid mapping information can be loaded from."))
    , useX0(initData( false, "useX0", "Use x0 instead of local copy of initial positions (to support topo changes)"))
    , indexFromEnd(initData( false, "indexFromEnd", "input DOF index starts from the end of input DOFs vector"))
    , rigidIndexPerPoint(initData( "rigidIndexPerPoint", "For each mapped point, the index of the Rigid it is mapped from"))
    , globalToLocalCoords(initData( "globalToLocalCoords", "are the output DOFs initially expressed in global coordinates"))
    , geometricStiffness(initData( 0, "geometricStiffness", "assemble (and use) geometric stiffness (0=no GS, 1=non symmetric, 2=symmetrized)"))
    , matrixJ()
    , updateJ(false)
{
