...
Vec3State::Vec3State()
    ...
    , m_restPositions(initData( "restPosition", "Vertices rest coordinates"))
    , m_vnormals (initData( "normal", "Normals of the model"))
    , modified(false)
{
...
VisualModelImpl::VisualModelImpl() //const std::string &name, std::string filename, std::string loader, std::string textureName)
    ...
    , lastMeshRev(-1)
    , castShadow(true)
    , m_initRestPositions(initData( false, "initRestPositions", "True if rest positions must be initialized with initial positions"))
    , m_useNormals		(initData( true, "useNormals", "True if normal smoothing groups should be read from file"))
    , m_updateNormals   (initData( true, "updateNormals", "True if normals should be updated at each iteration"))
    , m_computeTangents (initData( false, "computeTangents", "True if tangents should be computed at startup"))
    , m_updateTangents  (initData( true, "updateTangents", "True if tangents should be updated at each iteration"))
    , m_handleDynamicTopology (initData( true, "handleDynamicTopology", "True if topological changes should be handled"))
    , m_fixMergedUVSeams (initData( true, "fixMergedUVSeams", "True if UV seams should be handled even when duplicate UVs are merged"))
    , m_keepLines (initData( false, "keepLines", "keep and draw lines (false by default)"))
    , m_vertices2       (initData( "vertices", "vertices of the model (only if vertices have multiple normals/texcoords, otherwise positions are used)"))
    , m_vtexcoords      (initData( "texcoords", "coordinates of the texture"))
    , m_vtangents       (initData( "tangents", "tangents for normal mapping"))
    , m_vbitangents     (initData( "bitangents", "tangents for normal mapping"))
    , m_edges           (initData( "edges", "edges of the model"))
    , m_triangles       (initData( "triangles", "triangles of the model"))
    , m_quads           (initData( "quads", "quads of the model"))
    , m_vertPosIdx      (initData( "vertPosIdx", "If vertices have multiple normals/texcoords stores vertices position indices"))
    , m_vertNormIdx     (initData( "vertNormIdx", "If vertices have multiple normals/texcoords stores vertices normal indices"))
    , fileMesh          (initData( "filename"," Path to an ogl model"))
    , texturename       (initData( "texturename", "Name of the Texture"))
    , m_translation     (initData( Vec3Real(), "translation", "Initial Translation of the object"))
    , m_rotation        (initData( Vec3Real(), "rotation", "Initial Rotation of the object"))
    , m_scale           (initData( Vec3Real(1.0,1.0,1.0), "scale3d", "Initial Scale of the object"))
    , m_scaleTex        (initData( TexCoord(1.f,1.f), "scaleTex", "Scale of the texture"))
    , m_translationTex  (initData( TexCoord(0.f,0.f), "translationTex", "Translation of the texture"))
    , material			(initData( "material", "Material")) // tex(nullptr)
    , putOnlyTexCoords	(initData( (bool) false, "putOnlyTexCoords", "Give Texture Coordinates without the texture binding"))
    , srgbTexturing		(initData( (bool) false, "srgbTexturing", "When sRGB rendering is enabled, is the texture in sRGB colorspace?"))
    , materials			(initData( "materials", "List of materials"))
    , groups			(initData( "groups", "Groups of triangles and quads using a given material"))
