...
RecordedCamera::RecordedCamera()
    ...
    , p_pivot(initData( 0 , "pivot", "Pivot (0 => Scene center, 1 => World Center"))
    , currentMode(RecordedCamera::NONE_MODE)
    , isMoving(false)
    , m_startTime(initData( (SReal) 0.0 , "startTime", "Time when the camera moves will start"))
    , m_endTime(initData( (SReal)200 , "endTime", "Time when the camera moves will end (or loop)"))
    , m_rotationMode(initData( (bool)false , "rotationMode", "If true, rotation will be performed"))
    , m_translationMode(initData( (bool)false , "translationMode", "If true, translation will be performed"))
    , m_navigationMode(initData( (bool)false , "navigationMode", "If true, navigation will be performed"))
    , m_rotationSpeed(initData( (SReal)0.1 , "rotationSpeed", "rotation Speed"))
    , m_rotationCenter(initData( "rotationCenter", "Rotation center coordinates"))
    , m_rotationStartPoint(initData( "rotationStartPoint", "Rotation start position coordinates"))
    , m_rotationLookAt(initData( "rotationLookAt", "Position to be focused during rotation"))
    , m_rotationAxis(initData( Vec3(0,1,0), "rotationAxis", "Rotation axis"))
    , m_cameraUp(initData( Vec3(0,0,0), "cameraUp", "Camera Up axis"))
    , p_drawRotation(initData( (bool)false , "drawRotation", "If true, will draw the rotation path"))
    , p_drawTranslation(initData( (bool)false , "drawTranslation", "If true, will draw the translation path"))
    , m_translationPositions(initData( "cameraPositions", "Intermediate camera's positions"))
    , m_translationOrientations(initData( "cameraOrientations", "Intermediate camera's orientations"))
    , m_nextStep(0.0)
    , m_angleStep(0.0)
