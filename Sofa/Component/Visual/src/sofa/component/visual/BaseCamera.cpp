...
BaseCamera::BaseCamera()
    :p_position(initData( "position", "Camera's position"))
    ,p_orientation(initData( "orientation", "Camera's orientation"))
    ,p_lookAt(initData( "lookAt", "Camera's look at"))
    ,p_distance(initData( "distance", "Distance between camera and look at"))
    ,p_fieldOfView(initData( (double) (45.0) , "fieldOfView", "Camera's FOV"))
    ,p_zNear(initData( (double) 0.01 , "zNear", "Camera's zNear"))
    ,p_zFar(initData( (double) 100.0 , "zFar", "Camera's zFar"))
    ,p_computeZClip(initData( (bool)true, "computeZClip", "Compute Z clip planes (Near and Far) according to the bounding box"))
    ,p_minBBox(initData( Vec3(0.0,0.0,0.0) , "minBBox", "minBBox"))
    ,p_maxBBox(initData( Vec3(1.0,1.0,1.0) , "maxBBox", "maxBBox"))
    ,p_widthViewport(initData( (unsigned int) 800 , "widthViewport", "widthViewport"))
    ,p_heightViewport(initData((unsigned int) 600 , "heightViewport", "heightViewport"))
    ,p_type(initData("projectionType", "Camera Type (0 = Perspective, 1 = Orthographic)"))
    ,p_activated(initData( true , "activated", "Camera activated ?"))
    ,p_fixedLookAtPoint(initData( false, "fixedLookAt", "keep the lookAt point always fixed"))
    ,p_modelViewMatrix(initData(  "modelViewMatrix", "ModelView Matrix"))
    ,p_projectionMatrix(initData(  "projectionMatrix", "Projection Matrix"))
