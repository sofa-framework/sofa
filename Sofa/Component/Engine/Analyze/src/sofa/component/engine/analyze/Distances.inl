...
Distances< DataTypes >::Distances ( sofa::component::topology::container::dynamic::DynamicSparseGridTopologyContainer* hexaTopoContainer, core::behavior::MechanicalState<DataTypes>* targetPointSet ) :
    d_showMapIndex (initData (&d_showMapIndex, (unsigned int)0, "d_showMapIndex", "Frame DOF index on which display values." ) ),
    ...
    d_showTextScaleFactor (initData (&d_showTextScaleFactor, 0.001, "d_showTextScaleFactor", "Scale to apply on the text." ) ),
    d_showGradientMap (initData (&d_showGradientMap, false, "showGradients", "show gradients for each point of the target point set." ) ),
    d_showGradientsScaleFactor (initData (&d_showGradientsScaleFactor, 0.1, "d_showGradientsScaleFactor", "scale for the gradients displayed." ) ),
    ...
    d_initTarget (initData (&d_initTarget, false, "d_initTarget", "initialize the target MechanicalObject from the grid." ) ),
    d_initTargetStep (initData (&d_initTargetStep, 1, "d_initTargetStep", "initialize the target MechanicalObject from the grid using this step." ) ),
    d_zonesFramePair (initData (&d_zonesFramePair, "d_zonesFramePair", "Correspondence between the segmented value and the frames." ) ),
    d_harmonicMaxValue (initData (&d_harmonicMaxValue, 100.0, "d_harmonicMaxValue", "Max value used to initialize the harmonic distance grid." ) ),
