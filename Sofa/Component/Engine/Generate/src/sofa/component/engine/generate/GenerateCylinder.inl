...
GenerateCylinder<DataTypes>::GenerateCylinder()
    : f_outputTetrahedraPositions ( initData( "output_TetrahedraPosition", "output array of 3d points of tetrahedra mesh") )
    , f_outputTrianglesPositions ( initData( "output_TrianglesPosition", "output array of 3d points of triangle mesh") )
    , f_tetrahedra( initData( "tetrahedra", "output mesh tetrahedra") )
    , f_triangles( initData( "triangles", "output triangular mesh") )
    , f_bezierTriangleWeight( initData( "BezierTriangleWeights", "weights of rational Bezier triangles") )
    , f_isBezierTriangleRational( initData( "isBezierTriangleRational", "booleans indicating if each Bezier triangle is rational or integral") )
    , f_bezierTriangleDegree( initData( "BezierTriangleDegree", "order of Bezier triangles") )
    , f_bezierTetrahedronWeight( initData( "BezierTetrahedronWeights", "weights of rational Bezier tetrahedra") )
    , f_isBezierTetrahedronRational( initData( "isBezierTetrahedronRational", "booleans indicating if each Bezier tetrahedron is rational or integral") )
    , f_bezierTetrahedronDegree( initData( "BezierTetrahedronDegree", "order of Bezier tetrahedra") )
    , f_radius( initData((Real)0.2, "radius", "input cylinder radius") )
    , f_height( initData((Real)1.0, "height", "input cylinder height") )
    , f_origin( initData(Coord(), "origin", "cylinder origin point") )
    , f_openSurface( initData(true, "openSurface", "if the cylinder is open at its 2 ends") )
    , f_resolutionCircumferential( initData((size_t)6, "resCircumferential", "Resolution in the circumferential direction") )
   , f_resolutionRadial( initData((size_t)3, "resRadial", "Resolution in the radial direction") )
  , f_resolutionHeight( initData((size_t)5, "resHeight", "Resolution in the height direction") )
{
