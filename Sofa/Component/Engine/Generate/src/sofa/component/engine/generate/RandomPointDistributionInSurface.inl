...
RandomPointDistributionInSurface<DataTypes>::RandomPointDistributionInSurface()
    : initialized(false)
    , randomSeed( initData( (unsigned int) 0, "randomSeed", "Set a specified seed for random generation (0 for \"true pseudo-randomness\" ") )
    , isVisible( initData( bool (true), "isVisible", "is Visible ?") )
    , drawOutputPoints( initData( bool (false), "drawOutputPoints", "Output points visible ?") )
    , minDistanceBetweenPoints( initData( Real (0.1), "minDistanceBetweenPoints", "Min Distance between 2 points (-1 for true randomness)") )
    , numberOfInPoints( initData( (unsigned int) 10, "numberOfInPoints", "Number of points inside") )
    , numberOfTests( initData( (unsigned int) 5, "numberOfTests", "Number of tests to find if the point is inside or not (odd number)") )
    , f_vertices( initData( "vertices", "Vertices") )
    , f_triangles( initData( "triangles", "Triangles indices") )
    , f_inPoints( initData( "inPoints", "Points inside the surface") )
    , f_outPoints( initData( "outPoints", "Points outside the surface") )
