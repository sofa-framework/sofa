...
MeshROI<DataTypes>::MeshROI()
    ...
    , d_X0_i( initData( "ROIposition", "ROI position coordinates of the degrees of freedom") )
    , d_edges_i(initData( "ROIedges", "ROI Edge Topology") )
    , d_triangles_i(initData( "ROItriangles", "ROI Triangle Topology") )
    , d_computeEdges( initData( true,"computeEdges","If true, will compute edge list and index list inside the ROI.") )
    , d_computeTriangles( initData( true,"computeTriangles","If true, will compute triangle list and index list inside the ROI.") )
    , d_computeTetrahedra( initData( true,"computeTetrahedra","If true, will compute tetrahedra list and index list inside the ROI.") )
    , d_computeTemplateTriangles( initData(true,"computeMeshROI","Compute with the mesh (not only bounding box)") )
    , d_box( initData( "box", "Bounding box defined by xmin,ymin,zmin, xmax,ymax,zmax") )
    ...
    , d_pointsOutROI( initData("pointsOutROI","Points not contained in the ROI") )
    , d_edgesOutROI( initData("edgesOutROI","Edges not contained in the ROI") )
    , d_trianglesOutROI( initData("trianglesOutROI","Triangles not contained in the ROI") )
    , d_tetrahedraOutROI( initData("tetrahedraOutROI","Tetrahedra not contained in the ROI") )
    , d_indicesOut( initData("indicesOut","Indices of the points not contained in the ROI") )
    , d_edgeOutIndices( initData("edgeOutIndices","Indices of the edges not contained in the ROI") )
    , d_triangleOutIndices( initData("triangleOutIndices","Indices of the triangles not contained in the ROI") )
    , d_tetrahedronOutIndices( initData("tetrahedronOutIndices","Indices of the tetrahedra not contained in the ROI") )
    , d_drawOut( initData(false,"drawOut","Draw the data not contained in the ROI") )
    , d_drawMesh( initData(false,"drawMesh","Draw Mesh used for the ROI") )
    , d_drawBox( initData(false,"drawBox","Draw the Bounding box around the mesh used for the ROI") )
    , d_drawPoints( initData(false,"drawPoints","Draw Points") )
    , d_drawEdges( initData(false,"drawEdges","Draw Edges") )
    , d_drawTriangles( initData(false,"drawTriangles","Draw Triangles") )
    , d_drawTetrahedra( initData(false,"drawTetrahedra","Draw Tetrahedra") )
    , d_drawSize( initData(0.0,"drawSize","rendering size for mesh and topological elements") )
    , d_doUpdate( initData(false,"doUpdate","Update the computation (not only at the init)") )
{
