...
NearestPointROI<DataTypes>::NearestPointROI(core::behavior::MechanicalState<DataTypes>* mm1, core::behavior::MechanicalState<DataTypes>* mm2)
    : Inherit1()
    , Inherit2(mm1, mm2)
    , d_inputIndices1( initData("inputIndices1","Indices of the points to consider on the first model") )
    , d_inputIndices2( initData("inputIndices2","Indices of the points to consider on the first model") )
    , f_radius( initData((Real)1,"radius", "Radius to search corresponding fixed point") )
    , d_useRestPosition(initData( true, "useRestPosition", "If true will use restPosition only at init"))
    , f_indices1( initData("indices1","Indices from the first model associated to a dof from the second model") )
    , f_indices2( initData("indices2","Indices from the second model associated to a dof from the first model") )
    , d_edges(initData( "edges", "List of edge indices"))
    , d_indexPairs(initData( "indexPairs", "list of couples (parent index + index in the parent)"))
    , d_distances(initData( "distances", "List of distances between pairs of points"))
{
