...
MeshSplittingEngine<DataTypes>::MeshSplittingEngine()
    : Inherited()
    , inputPosition(initData("position","input vertices"))
    , inputEdges(initData("edges","input edges"))
    , inputTriangles(initData("triangles","input triangles"))
    , inputQuads(initData("quads","input quads"))
    , inputTets(initData("tetrahedra","input tetrahedra"))
    , inputHexa(initData("hexahedra","input hexahedra"))
    , nbInputs (initData( (unsigned)0, "nbInputs", "Number of input vectors"))
    ...
    , indexPairs( initData( type::vector<unsigned>(), "indexPairs", "couples for input vertices: ROI index + index in the ROI"))
