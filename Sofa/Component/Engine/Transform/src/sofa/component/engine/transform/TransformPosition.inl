...
TransformPosition<DataTypes>::TransformPosition()
    : f_origin( initData( "origin", "A 3d point on the plane/Center of the scale") )
    , f_inputX( initData( "input_position", "input array of 3d points") )
    , f_outputX( initData( "output_position", "output array of 3d points projected on a plane") )
    , f_normal(initData( "normal", "plane normal") )
    , f_translation(initData( "translation", "translation vector ") )
    , f_rotation(initData( "rotation", "rotation vector ") )
    , f_scale(initData( Coord(1.0,1.0,1.0), "scale", "scale factor") )
    , f_affineMatrix(initData( Mat4x4::s_identity, "matrix", "4x4 affine matrix") )
    , f_method(initData( "method", "transformation method either translation or scale or rotation or random or projectOnPlane") )
    , f_seed(initData( (long) 0, "seedValue", "the seed value for the random generator") )
    , f_maxRandomDisplacement(initData( (Real) 1.0, "maxRandomDisplacement", "the maximum displacement around initial position for the random transformation") )
    , f_fixedIndices( initData("fixedIndices","Indices of the entries that are not transformed") )
    , f_filename(initData( "filename", "filename of an affine matrix. Supported extensions are: .trm, .tfm, .xfm and .txt(read as .xfm)") )
    , f_drawInput(initData( false, "drawInput", "Draw input points") )
    , f_drawOutput(initData( false, "drawOutput", "Draw output points") )
    , f_pointSize(initData( (Real)1.0, "pointSize", "Point size") )
{
