...
struct NewmarkImplicitDynamic_test : public component::integrationscheme::testing::IntegrationSchemeSpringTest
{
    ...
    void createScene(double K, double m, double l0, double beta, double gamma,  double rm=0, double rk=0)
    {
        this->prepareScene(K, m, l0);
        // add ODE Solver to test
        simpleapi::createObject(m_si.root, "NewmarkIntegrationScheme", {
            { "beta", simpleapi::str(beta)},
            { "gamma", simpleapi::str(gamma)},
    ...
    void generateDiscreteMassPositions (double h, double K, double m, double z0, double v0,double g, double finalTime, double cd, double beta, double gamma,double rm,double rk)
    {
        ...
        K*=-1; // Force goes in opposite direction w/r to elongation in a spring so we have -K in the stifness matrix

        // During t=finalTime
        if((finalTime/h) > 0)
        {
            size = int(finalTime/h);
            positionsArray.reserve(size+1);
            velocitiesArray.reserve(size+1);
            accelerationsArray.reserve(size+1);
        }
...
{
   this->createScene(100,10,1,0.25,0.5,0,0); // k,m,l0
...
{
   this->createScene(100,10,1,0.25,0.5,0.1,0.1); // k,m,l0
