...
GenericConstraintSolver::GenericConstraintSolver()
    ...
    , d_computeGraphs(initData(&d_computeGraphs, false, "d_computeGraphs", "Compute graphs of errors and forces during resolution"))
    ...
    , d_graphViolations(initData(&d_graphViolations, "d_graphViolations", "Graph of each constraint's violation at each step of the resolution"))
    , d_currentNumConstraints(initData(&d_currentNumConstraints, 0, "d_currentNumConstraints", "OUTPUT: current number of constraints"))
    , d_currentNumConstraintGroups(initData(&d_currentNumConstraintGroups, 0, "d_currentNumConstraintGroups", "OUTPUT: current number of constraints"))
    , d_currentIterations(initData(&d_currentIterations, 0, "d_currentIterations", "OUTPUT: current number of constraint groups"))
    , d_currentError(initData(&d_currentError, 0.0_sreal, "d_currentError", "OUTPUT: current error"))
    , d_reverseAccumulateOrder(initData(&d_reverseAccumulateOrder, false, "d_reverseAccumulateOrder", "True to accumulate constraints from nodes in reversed order (can be necessary when using multi-mappings or interaction constraints not following the node hierarchy)"))
