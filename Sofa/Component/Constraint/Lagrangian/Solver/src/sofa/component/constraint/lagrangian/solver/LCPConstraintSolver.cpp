...
LCPConstraintSolver::LCPConstraintSolver()
    : d_displayDebug(initData(&d_displayDebug, false, "d_displayDebug", "Display debug information."))
    ...
    , d_minW(initData(&d_minW, 0.0_sreal, "d_minW", "If not zero, constraints whose self-compliance (i.e. the corresponding value on the diagonal of W) is smaller than this threshold will be ignored"))
    , d_maxF(initData(&d_maxF, 0.0_sreal, "d_maxF", "If not zero, constraints whose response force becomes larger than this threshold will be ignored"))
    , d_multi_grid(initData(&d_multi_grid, false, "multi_grid", "activate multi_grid resolution (NOT STABLE YET)"))
    , d_multi_grid_levels(initData(&d_multi_grid_levels, 2, "d_multi_grid_levels", "if multi_grid is active: how many levels to create (>=2)"))
    , d_merge_method(initData(&d_merge_method, 0, "d_merge_method", "if multi_grid is active: which method to use to merge constraints (0 = compliance-based, 1 = spatial coordinates)"))
    , d_merge_spatial_step(initData(&d_merge_spatial_step, 2, "d_merge_spatial_step", "if d_merge_method is 1: grid size reduction between multigrid levels"))
    , d_merge_local_levels(initData(&d_merge_local_levels, 2, "d_merge_local_levels", "if d_merge_method is 1: up to the specified level of the multigrid, constraints are grouped locally, i.e. separately within each contact pairs, while on upper levels they are grouped globally independently of contact pairs."))
    ...
    , d_showLevels(initData(&d_showLevels, 0, "d_showLevels", "Number of constraint levels to display"))
    , d_showCellWidth(initData(&d_showCellWidth, "d_showCellWidth", "Distance between each constraint cells"))
    , d_showTranslation(initData(&d_showTranslation, "d_showTranslation", "Position of the first cell"))
    , d_showLevelTranslation(initData(&d_showLevelTranslation, "d_showLevelTranslation", "Translation between levels"))
    , current_cp(&lcp1)
    , last_cp(nullptr)
    , _W(&lcp1.W)
    , _dFree(&lcp1.dFree)
