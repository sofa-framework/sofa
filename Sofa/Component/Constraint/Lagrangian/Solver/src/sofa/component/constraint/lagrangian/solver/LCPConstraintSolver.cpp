...
LCPConstraintSolver::LCPConstraintSolver()
    : displayDebug(initData( false, "displayDebug","Display debug information."))
    , initial_guess(initData( true, "initial_guess","activate LCP results history to improve its resolution performances."))
    , build_lcp(initData( true, "build_lcp", "LCP is not fully built to increase performance in some case."))
    , tol( initData( 0.001, "tolerance", "residual error threshold for termination of the Gauss-Seidel algorithm"))
    , maxIt( initData( 1000, "maxIt", "maximal number of iterations of the Gauss-Seidel algorithm"))
    , mu( initData( 0.6, "mu", "Friction coefficient"))
    , minW( initData( 0.0, "minW", "If not zero, constraints whose self-compliance (i.e. the corresponding value on the diagonal of W) is smaller than this threshold will be ignored"))
    , maxF( initData( 0.0, "maxF", "If not zero, constraints whose response force becomes larger than this threshold will be ignored"))
    , multi_grid(initData( false, "multi_grid","activate multi_grid resolution (NOT STABLE YET)"))
    , multi_grid_levels(initData( 2, "multi_grid_levels","if multi_grid is active: how many levels to create (>=2)"))
    , merge_method( initData( 0, "merge_method","if multi_grid is active: which method to use to merge constraints (0 = compliance-based, 1 = spatial coordinates)"))
    , merge_spatial_step( initData( 2, "merge_spatial_step", "if merge_method is 1: grid size reduction between multigrid levels"))
    , merge_local_levels( initData( 2, "merge_local_levels", "if merge_method is 1: up to the specified level of the multigrid, constraints are grouped locally, i.e. separately within each contact pairs, while on upper levels they are grouped globally independently of contact pairs."))
    , constraintGroups( initData( "group", "list of ID of groups of constraints to be handled by this solver."))
    , f_graph( initData("graph","Graph of residuals at each iteration"))
    , showLevels( initData(0,"showLevels","Number of constraint levels to display"))
    , showCellWidth( initData( "showCellWidth", "Distance between each constraint cells"))
    , showTranslation( initData( "showTranslation", "Position of the first cell"))
    , showLevelTranslation( initData( "showLevelTranslation", "Translation between levels"))
    , _mu(0.6)
    , lcp(&lcp1)
    , last_lcp(nullptr)
    , _W(&lcp1.W)
