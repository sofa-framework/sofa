...
PrecomputedConstraintCorrection<DataTypes>::PrecomputedConstraintCorrection(sofa::core::behavior::MechanicalState<DataTypes> *mm)
    : Inherit(mm)
    , m_rotations(initData( false, "rotations", ""))
    , m_restRotations(initData( false, "restDeformations", ""))
    , recompute(initData( false, "recompute", "if true, always recompute the compliance"))
    , debugViewFrameScale(initData( 1.0, "debugViewFrameScale", "Scale on computed node's frame"))
    , f_fileCompliance(initData( "fileCompliance", "Precomputed compliance matrix data file"))
    , fileDir(initData( "fileDir", "If not empty, the compliance will be saved in this repertory"))
    , invM(nullptr)
