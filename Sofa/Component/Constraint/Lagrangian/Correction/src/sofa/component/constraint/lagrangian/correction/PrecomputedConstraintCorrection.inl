...
PrecomputedConstraintCorrection<DataTypes>::PrecomputedConstraintCorrection(sofa::core::behavior::MechanicalState<DataTypes> *mm)
    ...
    , d_debugViewFrameScale(initData(&d_debugViewFrameScale, 1.0_sreal, "d_debugViewFrameScale", "Scale on computed node's frame"))
    , d_fileCompliance(initData(&d_fileCompliance, "fileCompliance", "Precomputed compliance matrix data file"))
    , d_fileDir(initData(&d_fileDir, "d_fileDir", "If not empty, the compliance will be saved in this repertory"))
    , invM(nullptr)
