...
template<class DataTypes>
AugmentedLagrangianConstraint<DataTypes>::~AugmentedLagrangianConstraint()
{
    if(contactsStatus)
...
template<class DataTypes>
void AugmentedLagrangianConstraint<DataTypes>::clear(int reserve)
{
    contacts.clear();
    if (reserve)
...
template<class DataTypes>
void AugmentedLagrangianConstraint<DataTypes>::addContact(SReal mu, SReal epsilon, Deriv norm, Coord P, Coord Q, Real contactDistance, int m1, int m2, long id, PersistentID localid)
{
    addContact(mu, epsilon, norm, P, Q, contactDistance, m1, m2,
...
template<class DataTypes>
void AugmentedLagrangianConstraint<DataTypes>::addContact(SReal mu, SReal epsilon, Deriv norm, Real contactDistance, int m1, int m2, long id, PersistentID localid)
{
    addContact(mu, epsilon, norm,
...
template<class DataTypes>
void AugmentedLagrangianConstraint<DataTypes>::addContact(SReal mu, SReal epsilon, Deriv norm, Coord P, Coord Q, Real contactDistance, int m1, int m2, Coord /*Pfree*/, Coord /*Qfree*/, long id, PersistentID localid)
{
...
template<class DataTypes>
void AugmentedLagrangianConstraint<DataTypes>::buildConstraintMatrix(const core::ConstraintParams *, DataMatrixDeriv &c1_d, DataMatrixDeriv &c2_d, unsigned int &contactId
...
template<class DataTypes>
void AugmentedLagrangianConstraint<DataTypes>::getPositionViolation(linearalgebra::BaseVector *v)
{
...
template<class DataTypes>
void AugmentedLagrangianConstraint<DataTypes>::getVelocityViolation(linearalgebra::BaseVector *v)
{
...
template<class DataTypes>
void AugmentedLagrangianConstraint<DataTypes>::getConstraintViolation(const core::ConstraintParams *cparams, linearalgebra::BaseVector *v, const DataVecCoord &, const DataVecCoord &
        , const DataVecDeriv &, const DataVecDeriv &)
{
    ...
    default :
        msg_error() << "AugmentedLagrangianConstraint doesn't implement " << cparams->getName() << " constraint violation\n";
        break;
    }
}
...
template<class DataTypes>
void AugmentedLagrangianConstraint<DataTypes>::getConstraintInfo(const core::ConstraintParams*, VecConstraintBlockInfo& blocks, VecPersistentID& ids, VecConstCoord& /*positions*/, VecConstDeriv& directions, VecConstArea& /*areas*/)
{
    if (contacts.empty()) return;
    const bool friction = (contacts[0].mu > 0.0); /// @todo: can there be both friction-less and friction contacts in the same AugmentedLagrangianConstraint ???
...
void AugmentedLagrangianConstraint<DataTypes>::getConstraintResolution(const core::ConstraintParams *, std::vector<core::behavior::ConstraintResolution*>& resTab, unsigned int& offset)
{
    ...
    for(unsigned int i=0; i<contacts.size(); i++)
    {
        ...
        if(c.mu > 0.0)
        {
            AugmentedLagrangianResolutionWithFriction* ucrwf = new AugmentedLagrangianResolutionWithFriction(c.mu, c.epsilon, nullptr, &contactsStatus[i]);
            ...
            resTab[offset++] = new AugmentedLagrangianResolution(c.epsilon);
    }
}

template<class DataTypes>
bool AugmentedLagrangianConstraint<DataTypes>::isActive() const
{
    for(unsigned int i = 0; i < contacts.size(); i++)
...
template<class DataTypes>
void AugmentedLagrangianConstraint<DataTypes>::draw(const core::visual::VisualParams* vparams)
{
