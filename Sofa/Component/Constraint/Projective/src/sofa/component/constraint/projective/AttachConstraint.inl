...
AttachConstraint<DataTypes>::AttachConstraint(core::behavior::MechanicalState<DataTypes> *mm1, core::behavior::MechanicalState<DataTypes> *mm2)
    ...
    , f_indices1( initData("indices1","Indices of the source points on the first model") )
    , f_indices2( initData("indices2","Indices of the fixed points on the second model") )
    , f_twoWay( initData(false,"twoWay", "true if forces should be projected back from model2 to model1") )
    , f_freeRotations( initData(false,"freeRotations", "true to keep rotations free (only used for Rigid DOFs)") )
    , f_lastFreeRotation( initData(false,"lastFreeRotation", "true to keep rotation of the last attached point free (only used for Rigid DOFs)") )
    , f_restRotations( initData(false,"restRotations", "true to use rest rotations local offsets (only used for Rigid DOFs)") )
    , f_lastPos( initData("lastPos", "position at which the attach constraint should become inactive") )
    , f_lastDir( initData("lastDir", "direction from lastPos at which the attach coustraint should become inactive") )
    , f_clamp( initData( false,"clamp", "true to clamp particles at lastPos instead of freeing them.") )
    , f_minDistance( initData( static_cast<Real>(-1),"minDistance", "the constraint become inactive if the distance between the points attached is bigger than minDistance.") )
    , d_positionFactor(initData( static_cast<Real>(1.0), "positionFactor", "IN: Factor applied to projection of position"))
    , d_velocityFactor(initData( static_cast<Real>(1.0), "velocityFactor", "IN: Factor applied to projection of velocity"))
    , d_responseFactor(initData( static_cast<Real>(1.0), "responseFactor", "IN: Factor applied to projection of force/acceleration"))
    , d_constraintFactor( initData("constraintFactor","Constraint factor per pair of points constrained. 0 -> the constraint is released. 1 -> the constraint is fully constrained") )
{
