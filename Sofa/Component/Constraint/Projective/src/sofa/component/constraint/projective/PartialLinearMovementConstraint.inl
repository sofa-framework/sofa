...
#include <sofa/component/constraint/projective/PartialLinearMovementConstraint.h>
...
PartialLinearMovementConstraint<DataTypes>::PartialLinearMovementConstraint()
    ...
    , m_indices( initData("indices","Indices of the constrained points") )
    , m_keyTimes(  initData("keyTimes","key times for the movements") )
    , m_keyMovements(  initData("movements","movements corresponding to the key times") )
    , showMovement( initData( (bool)false, "showMovement", "Visualization of the movement to be applied to constrained dofs."))
    , linearMovementBetweenNodesInIndices( initData( (bool)false, "linearMovementBetweenNodesInIndices", "Take into account the linear movement between the constrained points"))
    , mainIndice( initData( "mainIndice", "The main indice node in the list of constrained nodes, it defines how to apply the linear movement between this constrained nodes "))
    , minDepIndice( initData( "minDepIndice", "The indice node in the list of constrained nodes, which is imposed the minimum displacment "))
    , maxDepIndice( initData( "maxDepIndice", "The indice node in the list of constrained nodes, which is imposed the maximum displacment "))
    , m_imposedDisplacmentOnMacroNodes(  initData("imposedDisplacmentOnMacroNodes","The imposed displacment on macro nodes") )
    , X0 ( initData( Real(0.0),"X0","Size of specimen in X-direction" ) )
    , Y0 ( initData( Real(0.0),"Y0","Size of specimen in Y-direction" ) )
    , Z0 ( initData( Real(0.0),"Z0","Size of specimen in Z-direction" ) )
    , movedDirections( initData("movedDirections","for each direction, 1 if moved, 0 if free") )
