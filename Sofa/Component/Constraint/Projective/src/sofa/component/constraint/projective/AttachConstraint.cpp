...
void AttachConstraint<Rigid3Types>::calcRestRotations()
{
    ...
    for (unsigned int i=0; i<indices2.size(); ++i)
    {
        ...
        if (indices2[i] < x0.size()-1)
        {
            ...
            dp0.normalize();
            Vector3 y = cross(dp0, Vector3(1_sreal,0_sreal,0_sreal));
            y.normalize();
            double alpha = acos(dp0[0]);
            q = Quat<SReal>(y,alpha);
            msg_info() << "restRotations x2["<<indices2[i]<<"]="<<q<<" dp0="<<dp0<<" qx="<<q.rotate(Vector3(1_sreal,0_sreal,0_sreal));
        }
        restRotations[i] = q;
    }
}

template class SOFA_COMPONENT_CONSTRAINT_PROJECTIVE_API AttachConstraint<Vec3Types>;
template class SOFA_COMPONENT_CONSTRAINT_PROJECTIVE_API AttachConstraint<Vec2Types>;
template class SOFA_COMPONENT_CONSTRAINT_PROJECTIVE_API AttachConstraint<Vec1Types>;
template class SOFA_COMPONENT_CONSTRAINT_PROJECTIVE_API AttachConstraint<Rigid3Types>;
template class SOFA_COMPONENT_CONSTRAINT_PROJECTIVE_API AttachConstraint<Rigid2Types>;
