    ...
    for (auto& [component, localMatrixMap] : matrixMaps.mappedLocalMatrix)
    {
        for (auto& [states, localMatrix] : localMatrixMap)
        {
            if (auto* sparsityPatternMatrix = dynamic_cast<SparsityPatternLocalMappedMatrix<c, SReal>*>(localMatrix))
            {
                ...
                {
                    m_stiffness[component].setMatrixAccumulator(mat.get(), mstate0, mstate1);
                }
                else if constexpr (c == Contribution::DAMPING)
                {
                    m_damping[component].setMatrixAccumulator(mat.get(), mstate0, mstate1);
                }
                else if constexpr (c == Contribution::GEOMETRIC_STIFFNESS)
                {
                    m_geometricStiffness[component].setMatrixAccumulator(mat.get(), mstate0, mstate1);
                }
                else if constexpr (c == Contribution::MASS)
                {
                    m_mass[component] = mat.get();
                }
            }
            else
            {
    ...
    for (auto& [component, localMatrixMap] : matrixMaps.componentLocalMatrix)
    {
        for (auto& [states, localMatrix] : localMatrixMap)
        {
            if (auto* sparsityPatternMatrix = dynamic_cast<SparsityPatternLocalMatrix<c>*>(localMatrix))
            {
                ...
                auto mat = sofa::core::objectmodel::New<ConstantLocalMatrix<c>>();
                configureCreatedMatrixComponent<c>(mat, component, factor, !this->notMuted());
...
void ConstantSparsityPatternSystem::reinitLocalMatrices(LocalMatrixMaps<c, Real>& matrixMaps)
{
    for (auto& [component, localMatrixMap] : matrixMaps.componentLocalMatrix)
    {
        for (auto& [states, localMatrix] : localMatrixMap)
        {
            if (auto* local = dynamic_cast<ConstantLocalMatrix<c>* >(localMatrix))
            {
                local->currentId = 0;
            }
        }
...
void ConstantSparsityPatternSystem::makeCreateDispatcher()
{
...
std::unique_ptr<CreateMatrixDispatcher<c>> ConstantSparsityPatternSystem::makeCreateDispatcher()
{
