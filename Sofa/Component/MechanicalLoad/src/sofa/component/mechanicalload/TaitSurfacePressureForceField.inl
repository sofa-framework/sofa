...
TaitSurfacePressureForceField<DataTypes>::TaitSurfacePressureForceField():
    ...
    m_p0(initData( (Real)0.0, "p0", "IN: Rest pressure when V = V0")),
    m_B(initData( (Real)0.0, "B", "IN: Bulk modulus (resistance to uniform compression)")),
    m_gamma(initData( (Real)0.0, "gamma", "IN: Bulk modulus (resistance to uniform compression)")),
    m_injectedVolume(initData( (Real)0.0, "injectedVolume", "IN: Injected (or extracted) volume since the start of the simulation")),
    m_maxInjectionRate(initData( (Real)1000.0, "maxInjectionRate", "IN: Maximum injection rate (volume per second)")),

    m_initialVolume(initData( (Real)0.0, "initialVolume", "OUT: Initial volume, as computed from the surface rest position")),
    m_currentInjectedVolume(initData( (Real)0.0, "currentInjectedVolume", "OUT: Current injected (or extracted) volume (taking into account maxInjectionRate)")),
    m_v0(initData( (Real)0.0, "v0", "OUT: Rest volume (as computed from initialVolume + injectedVolume)")),
    m_currentVolume(initData( (Real)0.0, "currentVolume", "OUT: Current volume, as computed from the last surface position")),
    m_currentPressure(initData( (Real)0.0, "currentPressure", "OUT: Current pressure, as computed from the last surface position")),
    m_currentStiffness(initData( (Real)0.0, "currentStiffness", "OUT: dP/dV at current volume and pressure")),
    m_pressureTriangles(initData( "pressureTriangles", "OUT: list of triangles where a pressure is applied (mesh triangles + tesselated quads)")),
    m_initialSurfaceArea(initData( (Real)0.0, "initialSurfaceArea", "OUT: Initial surface area, as computed from the surface rest position")),
    m_currentSurfaceArea(initData( (Real)0.0, "currentSurfaceArea", "OUT: Current surface area, as computed from the last surface position")),
    m_drawForceScale(initData( (Real)0.001, "drawForceScale", "DEBUG: scale used to render force vectors")),
    m_drawForceColor(initData( sofa::type::RGBAColor(0,1,1,1), "drawForceColor", "DEBUG: color used to render force vectors")),
    m_volumeAfterTC(initData( "volumeAfterTC", "OUT: Volume after a topology change")),
    m_surfaceAreaAfterTC(initData( (Real)0.0, "surfaceAreaAfterTC", "OUT: Surface area after a topology change")),
