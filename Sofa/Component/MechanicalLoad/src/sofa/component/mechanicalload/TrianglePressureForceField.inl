...
template <class DataTypes>  TrianglePressureForceField<DataTypes>::TrianglePressureForceField()    
		...
		, cauchyStress(initData( MatSym3(),"cauchyStress", "Cauchy Stress applied on the normal of each triangle"))
        , triangleList(initData("triangleList", "Indices of triangles separated with commas where a pressure is applied"))
        , normal(initData("normal", "Normal direction for the plane selection of triangles"))
		...
		, p_useConstantForce(initData( (bool)true, "useConstantForce", "applied force is computed as the pressure vector times the area at rest"))
