...
EdgePressureForceField<DataTypes>::EdgePressureForceField()
    : edgePressureMap(initData( "edgePressureMap", "map between edge indices and their pressure"))
    ,pressure(initData( "pressure", "Pressure force per unit area"))
    , edgeIndices(initData("edgeIndices", "Indices of edges separated with commas where a pressure is applied"))
    , edges(initData( "edges", "List of edges where a pressure is applied"))
    , normal(initData("normal", "Normal direction for the plane selection of edges"))
    ...
    , arrowSizeCoef(initData((SReal)0.0, "arrowSizeCoef", "Size of the drawn arrows (0->no arrows, sign->direction of drawing"))
    , p_intensity(initData("p_intensity", "pressure intensity on edge normal"))
    , p_binormal(initData("binormal", "Binormal of the 2D plane"))
    , p_showForces(initData( (bool)false, "showForces", "draw arrows of edge pressures"))
