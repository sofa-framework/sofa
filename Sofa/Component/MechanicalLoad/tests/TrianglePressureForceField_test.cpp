...
struct TrianglePressureForceField_test : public ForceField_test<_TrianglePressureForceField>
{
    ...
    {
        sofa::simulation::node::initNode(Inherited::node.get());

        // Do a few animation steps
        for(int k=0;k<10;k++)
        {
            sofa::simulation::node::animateNode(Inherited::node.get(), 0.5_sreal);
        }
...
TYPED_TEST_SUITE(TrianglePressureForceField_test, TestTypes);
