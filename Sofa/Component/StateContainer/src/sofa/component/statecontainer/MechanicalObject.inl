...
MechanicalObject<DataTypes>::MechanicalObject()
    : x(initData( "position", "position coordinates of the degrees of freedom"))
    , v(initData( "velocity", "velocity coordinates of the degrees of freedom"))
    , f(initData( "force", "force vector of the degrees of freedom"))
    , x0(initData( "rest_position", "rest position coordinates of the degrees of freedom"))
    , externalForces(initData( "externalForce", "externalForces vector of the degrees of freedom"))
    , dx(initData( "derivX", "dx vector of the degrees of freedom"))
    , xfree(initData( "free_position", "free position coordinates of the degrees of freedom"))
    , vfree(initData( "free_velocity", "free velocity coordinates of the degrees of freedom"))
    , c(initData( "constraint", "constraints applied to the degrees of freedom"))
    , m(initData( "mappingJacobian", "mappingJacobian applied to the degrees of freedom"))
    , reset_position(initData( "reset_position", "reset position coordinates of the degrees of freedom"))
    , reset_velocity(initData( "reset_velocity", "reset velocity coordinates of the degrees of freedom"))
    , restScale(initData( (SReal)1.0, "restScale", "optional scaling of rest position coordinates (to simulated pre-existing internal tension).(default = 1.0)"))
    , d_useTopology(initData( true, "useTopology", "Shall this object rely on any active topology to initialize its size and positions"))
    , showObject(initData( (bool) false, "showObject", "Show objects. (default=false)"))
    , showObjectScale(initData( (float) 0.1, "showObjectScale", "Scale for object display. (default=0.1)"))
    , showIndices(initData( (bool) false, "showIndices", "Show indices. (default=false)"))
    , showIndicesScale(initData( (float) 0.02, "showIndicesScale", "Scale for indices display. (default=0.02)"))
    , showVectors(initData( (bool) false, "showVectors", "Show velocity. (default=false)"))
    , showVectorsScale(initData( (float) 0.0001, "showVectorsScale", "Scale for vectors display. (default=0.0001)"))
    , drawMode(initData(0,"drawMode","The way vectors will be drawn:\n- 0: Line\n- 1:Cylinder\n- 2: Arrow.\n\nThe DOFS will be drawn:\n- 0: point\n- >1: sphere. (default=0)"))
    , d_color(initData( type::RGBAColor::white(), "showColor", "Color for object display. (default=[1 1 1 1])"))
    , translation(initData( Vector3(), "translation", "Translation of the DOFs"))
    , rotation(initData( Vector3(), "rotation", "Rotation of the DOFs"))
    , scale(initData( Vector3(1.0,1.0,1.0), "scale3d", "Scale of the DOFs in 3 dimensions"))
    , translation2(initData( Vector3(), "translation2", "Translation of the DOFs, applied after the rest position has been computed"))
    , rotation2(initData( Vector3(), "rotation2", "Rotation of the DOFs, applied the after the rest position has been computed"))
    , d_size(initData( 0, "size", "Size of the vectors"))
    , l_topology(initLink("topology","Link to the topology relevant for this object"))
    , f_reserve(initData( 0, "reserve", "Size to reserve when creating vectors. (default=0)"))
