...
InputEventReader::InputEventReader()
    : filename( initData( std::string("/dev/input/mouse2"), "filename", "input events file name"))
    , inverseSense(initData( false, "inverseSense", "inverse the sense of the mouvement"))
    , p_printEvent(initData( false, "printEvent", "Print event informations"))
//, timeout( initData( 0, "timeout", "time out to get an event from file" ))
    , p_key1(initData( '0', "key1","Key event generated when the left pedal is pressed"))
    , p_key2(initData( '1', "key2","Key event generated when the right pedal is pressed"))
    , p_writeEvents(initData( false , "writeEvents","If true, write incoming events ; if false, read events from that file (if an output filename is provided)"))
    , p_outputFilename(initData( "outputFilename","Other filename where events will be stored (or read)"))
