...
WriteState::WriteState()
    ...
    , d_writeX( initData( true, "writeX", "flag enabling output of X vector"))
    , d_writeX0( initData( false, "writeX0", "flag enabling output of X0 vector"))
    , d_writeV( initData( false, "writeV", "flag enabling output of V vector"))
    , d_writeF( initData( false, "writeF", "flag enabling output of F vector"))
    , d_time( initData( type::vector<double>(0), "time", "set time to write outputs (by default export at t=0)"))
    , d_period( initData( 0.0, "period", "period between outputs"))
    , d_DOFsX( initData( type::vector<unsigned int>(0), "DOFsX", "set the position DOFs to write"))
    , d_DOFsV( initData( type::vector<unsigned int>(0), "DOFsV", "set the velocity DOFs to write"))
    , d_stopAt( initData( 0.0, "stopAt", "stop the simulation when the given threshold is reached"))
    , d_keperiod( initData( 0.0, "keperiod", "set the period to measure the kinetic energy increase"))
    , mmodel(nullptr)
    , outfile(nullptr)
