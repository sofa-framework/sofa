    ...
    , d_drawIsEnabled(initData(&d_drawIsEnabled, false, "debugDraw", "enable/disable drawing of lights shadow textures. (default=false)"))
{
    ...
    addUpdateCallback("activateShadows",
                      {&d_shadowsEnabled},
                      [this](const core::DataTracker& tracker)
    {
        const bool b = d_shadowsEnabled.getValue();
        if (b && !m_shadowShaders.empty())
        {
    ...
    if (sofa::core::objectmodel::KeypressedEvent::checkEventType(event))
    {
        ...
        case 'L':
            msg_deprecated() << "This component was implemented with hard coded user interactions. This behavior was deprecated in PR#4163. "
                                ...
                                " c++ or python (see the example named LightManagerController.py)"
